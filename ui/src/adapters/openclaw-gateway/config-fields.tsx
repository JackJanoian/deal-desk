...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="DealDesk API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "dealDeskApiUrl",
                  String(config.dealDeskApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "dealDeskApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://dealdesk.example"
            />
          </Field>
          ...
          <Field label="Claimed API key path">
            <DraftInput
              ...
              immediate
              className={inputClass}
              placeholder="~/.openclaw/workspace/dealdesk-claimed-api-key.json"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "dealdesk"))}
                onCommit={(v) => mark("adapterConfig", "sessionKey", v || undefined)}
                immediate
                className={inputClass}
                placeholder="dealdesk"
              />
            </Field>
          )}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. DealDesk persists a device key during onboarding so pairing approvals
