 ...
 *
 * Plugin bundles are built with `external: ["@dealdesk/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__dealDeskPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__dealDeskPluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__dealDeskPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__dealDeskPluginBridge__?.sdkUi ?? {};
        function missing(name) {
          return function MissingDealDeskSdkUiComponent() {
            throw new Error('DealDesk plugin UI runtime is not initialized for "' + name + '". Ensure the host loaded the plugin bridge before rendering this UI module.');
          };
        }
 ...
 * - `import { ... } from "@dealdesk/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@dealdesk/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@dealdesk/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@dealdesk/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@dealdesk/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__dealDeskPluginBridge__) {
 ...
 * self-register via `window.dealDeskPlugins.registerReactComponent()`.
 ...
 * of `@dealdesk/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
