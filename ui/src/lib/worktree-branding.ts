...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("dealdesk-worktree-enabled") !== "true") return null;

  const name = readMetaContent("dealdesk-worktree-name");
  const color = normalizeHexColor(readMetaContent("dealdesk-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("dealdesk-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
