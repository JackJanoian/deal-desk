...
describe("issue-reference", () => {
  ...
  it("does not treat GitHub issue URLs as internal DealDesk issue links", () => {
    expect(parseIssuePathIdFromPath("https://github.com/dealdesk/dealdesk/issues/1778")).toBeNull();
    expect(parseIssueReferenceFromHref("https://github.com/dealdesk/dealdesk/issues/1778")).toBeNull();
  });
  ...
  it("preserves absolute DealDesk issue URLs so origin, port, and hash are not lost", () => {
