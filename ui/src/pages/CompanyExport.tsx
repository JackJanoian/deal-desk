...
/**
 * Filter .dealdesk.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterDealDeskYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://dealdesk.ing) package.");
  lines.push("");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("pnpm dealdesk company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [DealDesk](https://dealdesk.ing) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [DealDesk](https://dealdesk.ing) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function CompanyExport() {
  ...
  // Recompute .dealdesk.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .dealdesk.yaml
    const yamlPath = exportData.dealDeskExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterDealDeskYaml(exportData.files[yamlPath], checkedFiles);
    }
