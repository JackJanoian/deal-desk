...
import type { ExecutionWorkspace, Issue, Project, ProjectWorkspace, RoutineListItem } from "@dealdesk/shared";
...
function LegacyWorkspaceTabRedirect({ workspaceId }: { workspaceId: string }) {
  useEffect(() => {
    try {
      localStorage.removeItem(`dealdesk:execution-workspace-tab:${workspaceId}`);
    } catch {}
  }, [workspaceId]);
...
function ExecutionWorkspaceIssuesList({
  ...
  return (
    <IssuesList
      ...
      viewStateKey="dealdesk:execution-workspace-issues-view"
  ...
  const runRoutine = useMutation({
    ...
    onSuccess: async (_, { id }) => {
      ...
      pushToast({
        ...
        body: "DealDesk created a run using this execution workspace.",
        tone: "success",
      });
    },
    onSettled: () => {
            ...
            <Card className="rounded-none">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <Field label="Provision command" hint="Runs when DealDesk prepares this execution workspace">
                    <Textarea
