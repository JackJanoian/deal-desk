...
import { AGENT_ADAPTER_TYPES } from "@dealdesk/shared";
import type { AgentAdapterType, JoinRequest } from "@dealdesk/shared";
...
function mapInviteAuthFeedback(
  ...
  if (code === "INVALID_EMAIL_OR_PASSWORD") {
    return {
      tone: "error",
      message:
        "That email and password did not match an existing DealDesk account. Check both fields, or create an account first if you are new here.",
    };
  }
...
export function InviteLandingPage() {
  ...
  const companyDisplayName = companyName || "this DealDesk company";
          ...
          <section className={`${panelClassName} space-y-6`}>
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0">
                <p className="text-xs uppercase tracking-[0.24em] text-zinc-500">
                  You&apos;ve been invited to join DealDesk
                </p>
                <h1 className="mt-2 text-2xl font-semibold">
                  {invite.inviteType === "bootstrap_ceo" ? "Set up DealDesk" : `Join ${companyDisplayName}`}
                </h1>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-300">
                  {showsAgentForm
                    ...
                    : requiresHumanAccount
                      ? "Create your DealDesk account first. If you already have one, switch to sign in and continue the invite with the same email."
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div className="border border-zinc-800 p-3">
                ...
                <div className="mt-1 text-sm text-zinc-100">{invitedByUserName ?? "DealDesk board"}</div>
              </div>
              <div className="border border-zinc-800 p-3">
            ...
            {showsAgentForm ? (
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <p className="mt-1 text-sm text-zinc-400">
                    {authMode === "sign_up"
                      ? `Start with a DealDesk account. After that, you'll come right back here to accept the invite for ${companyDisplayName}.`
                      : "Use the DealDesk account that already matches this invite. If you do not have one yet, switch back to create account."}
                  </p>
                </div>
                ...
                <p className="text-xs leading-5 text-zinc-500">
                  {authMode === "sign_up"
                    ? "Already signed up before? Use the existing-account option instead so the invite lands on the right DealDesk user."
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-zinc-400">
                    {shouldAutoAcceptHumanInvite
                      ...
                      : `This will ${
                          invite.inviteType === "bootstrap_ceo" ? "finish setting up DealDesk" : `submit or complete your join request for ${companyDisplayName}`
                        }.`}
                  </p>
                </div>
                {error ? <p className="text-xs text-red-400">{error}</p> : null}
