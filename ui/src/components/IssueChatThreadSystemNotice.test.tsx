...
import type { Agent, SuccessfulRunHandoffState } from "@dealdesk/shared";
...
vi.mock("../hooks/useDealDeskIssueRuntime", () => ({
  useDealDeskIssueRuntime: () => ({}),
}));
...
describe("IssueChatThread system notice routing", () => {
  it("renders authorType=system comments as a SystemNotice rather than a user bubble", () => {
    ...
    expect(container.textContent).toContain("DealDesk needs a disposition");
  ...
  it("labels system notice source as DealDesk when no run agent can be resolved", () => {
    const comment: IssueChatComment = {
    ...
    const status = container.querySelector('[role="status"]');
    expect(status).not.toBeNull();
    expect(status?.textContent).toContain("DealDesk");
    expect(status?.textContent).not.toContain("You");
  });

  it("falls back to DealDesk in the system notice header when run agent is unknown to agentMap", () => {
    const comment: IssueChatComment = {
    ...
    expect(sourceLink?.textContent).toBe("DealDesk");
  });
  ...
  it("folds stale successful-run disposition warnings into the activity log disclosure style", () => {
    ...
    expect(row?.textContent).not.toContain("DealDesk needs a disposition before this issue can continue.");
