...
function MarkdownIssueLink({
  ...
  return (
    <Link
      ...
      className="dealdesk-markdown-issue-ref"
      title={title}
...
function createWikiLinkNode(href: string, wikiLink: ParsedWikiLink): MarkdownAstNode {
  return {
    ...
    data: {
      hProperties: {
        "data-dealdesk-wiki-link": "true",
        "data-dealdesk-wiki-target": wikiLink.target,
      },
    },
...
function CodeBlock({
  ...
  return (
    <div className="dealdesk-markdown-codeblock">
      <pre
        {...preProps}
        ref={preRef}
      ...
      <button
        ...
        title={label}
        className="dealdesk-markdown-codeblock-copy"
      ...
      >
        ...
        <span className="dealdesk-markdown-codeblock-copy-label">{label}</span>
      </button>
    </div>
  );
}
...
function MermaidDiagramBlock({ source, darkMode }: { source: string; darkMode: boolean }) {
  ...
  useEffect(() => {
    ...
    loadMermaid()
      .then(async (mermaid) => {
        ...
        const rendered = await mermaid.render(`dealdesk-mermaid-${renderId}`, source);
        if (!active) return;
        setSvg(rendered.svg);
      })
      .catch((err) => {
  ...
  return (
    <div className="dealdesk-mermaid">
      {svg ? (
        <div dangerouslySetInnerHTML={{ __html: svg }} />
      ) : (
        <>
          <p className={cn("dealdesk-mermaid-status", error && "dealdesk-mermaid-status-error")}>
            {error ? `Unable to render Mermaid diagram: ${error}` : "Rendering Mermaid diagram..."}
          </p>
          <pre className="dealdesk-mermaid-source">
...
export function MarkdownBody({
  ...
  const components: Components = {
    ...
    table: ({ node: _node, style: tableStyle, children: tableChildren, ...tableProps }) => (
      <div className="dealdesk-markdown-table-scroll" role="region" aria-label="Scrollable table" tabIndex={0}>
    ...
    a: ({ node: _node, href, style: linkStyle, children: linkChildren, ...anchorProps }) => {
      ...
      const isWikiLink = dataProps["data-dealdesk-wiki-link"] === "true";
      ...
      if (parsed) {
        ...
        return (
          <a
            href={targetHref}
            className={cn(
              "dealdesk-mention-chip",
              `dealdesk-mention-chip--${parsed.kind}`,
              parsed.kind === "project" && "dealdesk-project-mention-chip",
            )}
  ...
  return (
    <div
      className={cn(
        "dealdesk-markdown prose prose-sm min-w-0 max-w-full break-words overflow-hidden",
