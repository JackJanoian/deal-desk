...
import { buildProjectMentionHref, buildSkillMentionHref } from "@dealdesk/shared";
import {
...
describe("MarkdownEditor", () => {
  ...
  it("keeps mention queries active across spaces", () => {
    expect(findMentionMatch("Ping @DealDesk App", "Ping @DealDesk App".length)).toEqual({
      trigger: "mention",
      marker: "@",
      query: "DealDesk App",
      atPos: 5,
      endPos: "Ping @DealDesk App".length,
    });
  });
  ...
  async function openMentionMenuFor(
    ...
    range.setStart(textNode!, "@Deal".length);
    range.collapse(true);
    ...
    const option = Array.from(document.body.querySelectorAll('button[type="button"]'))
      .find((node) => node.textContent?.includes("DealDesk App")) as HTMLButtonElement | undefined;
  ...
  it("accepts mention selection from a touch tap", async () => {
    ...
    expect(handleChange).toHaveBeenCalledWith(
      `[@DealDesk App](${buildProjectMentionHref("project-123", "#336699")}) `,
    );
