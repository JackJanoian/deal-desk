...
type DealDeskSprite = (typeof DEALDESK_SPRITES)[number];
...
function spriteSize(sprite: DealDeskSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = DEALDESK_SPRITES[(baseCol + baseRow) % DEALDESK_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = DEALDESK_SPRITES[Math.floor(Math.random() * DEALDESK_SPRITES.length)]!;
