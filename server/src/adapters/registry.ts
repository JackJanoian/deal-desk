...
} from "@dealdesk/adapter-claude-local";
import {
...
} from "@dealdesk/adapter-cursor-local";
import {
...
} from "@dealdesk/adapter-cursor-cloud/server";
import { agentConfigurationDoc as cursorCloudAgentConfigurationDoc } from "@dealdesk/adapter-cursor-cloud";
import {
...
} from "@dealdesk/adapter-gemini-local";
import {
...
} from "@dealdesk/adapter-opencode-local";
import {
  execute as openclawGatewayExecute,
  testEnvironment as openclawGatewayTestEnvironment,
} from "@dealdesk/adapter-openclaw-gateway/server";
import {
  agentConfigurationDoc as openclawGatewayAgentConfigurationDoc,
  models as openclawGatewayModels,
} from "@dealdesk/adapter-openclaw-gateway";
...
} from "@dealdesk/adapter-pi-local";
import {
...
const hermesLocalAdapter: ServerAdapterModule = {
  ...
  execute: async (ctx) => {
    ...
    const explicitApiKey =
      typeof existingEnv.DEALDESK_API_KEY === "string" && existingEnv.DEALDESK_API_KEY.trim().length > 0;
    ...
    const authGuardPrompt = [
      "DealDesk API safety rule:",
      "Use Authorization: Bearer $DEALDESK_API_KEY on every DealDesk API request.",
      "Use X-DealDesk-Run-Id: $DEALDESK_RUN_ID on every DealDesk API request that writes or mutates data, including comments and issue updates.",
      "Never use a board, browser, or local-board session for DealDesk API writes.",
    ].join("\n");
    ...
    const patchedConfig: Record<string, unknown> = {
      ...
      env: {
        ...existingEnv,
        ...(!explicitApiKey ? { DEALDESK_API_KEY: normalizedCtx.authToken } : {}),
        DEALDESK_RUN_ID: normalizedCtx.runId,
      },
    };
...
const externalAdaptersReady: Promise<void> = (async () => {
  try {
    ...
    for (const externalAdapter of externalAdapters) {
      ...
      if (overriding) {
        console.log(
          `[dealdesk] External adapter "${externalAdapter.type}" overrides built-in adapter`,
        );
    ...
    console.error("[dealdesk] Failed to load external adapters:", err);
  }
})();
...
export function setOverridePaused(type: string, paused: boolean): boolean {
  ...
  if (paused && !wasPaused) {
    ...
    console.log(`[dealdesk] Override paused for "${type}" — builtin adapter restored`);
    return true;
  }
  if (!paused && wasPaused) {
    pausedOverrides.delete(type);
    console.log(`[dealdesk] Override resumed for "${type}" — external adapter active`);
    return true;
  }
  return false;
}
