...
function extractUiParserSource(
  ...
  const contractVersion = pkg.dealdesk?.adapterUiParser;
  if (contractVersion) {
    ...
    logger.info(
      { packageName },
      "Adapter has ./ui-parser export but no dealdesk.adapterUiParser version — loading anyway (future versions may require it)",
    );
  }
