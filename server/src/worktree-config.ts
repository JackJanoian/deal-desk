...
import type { DealDeskConfig } from "@dealdesk/shared";
...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by DealDesk worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.DEALDESK_IN_WORKTREE !== "true") return null;

  const configPath = resolveDealDeskConfigPath(overrideConfigPath);
  ...
  const worktreeName =
    nonEmpty(persistedEnv.DEALDESK_WORKTREE_NAME) ??
    nonEmpty(env.DEALDESK_WORKTREE_NAME) ??
    path.basename(worktreeRoot);
  const instanceId =
    nonEmpty(persistedEnv.DEALDESK_INSTANCE_ID) ??
    nonEmpty(env.DEALDESK_INSTANCE_ID) ??
    sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(persistedEnv.DEALDESK_HOME) ??
      nonEmpty(env.DEALDESK_HOME) ??
      nonEmpty(env.DEALDESK_WORKTREES_DIR) ??
      "~/.dealdesk-worktrees",
  );
...
function writeConfigFile(configPath: string, config: DealDeskConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".dealdesk", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as DealDeskConfig;
...
function buildIsolatedWorktreeConfig(
  ...
  const databasePort =
    config.database.mode === "embedded-postgres"
      ...
      : undefined;
  const nextConfig: DealDeskConfig = {
    ...config,
    database: {
      ...config.database,
      ...(config.database.mode === "embedded-postgres"
...
): { config: DealDeskConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.DEALDESK_HOME = context.homeDir;
  process.env.DEALDESK_INSTANCE_ID = context.instanceId;
  process.env.DEALDESK_CONFIG = context.configPath;
  process.env.DEALDESK_CONTEXT = context.contextPath;
  process.env.DEALDESK_WORKTREE_NAME = context.worktreeName;

  let repairedConfig = false;
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as DealDeskConfig;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    DEALDESK_HOME: context.homeDir,
    DEALDESK_INSTANCE_ID: context.instanceId,
    DEALDESK_CONFIG: context.configPath,
    DEALDESK_CONTEXT: context.contextPath,
    DEALDESK_IN_WORKTREE: "true",
    DEALDESK_WORKTREE_NAME: context.worktreeName,
  };
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: DealDeskConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as DealDeskConfig;
  } catch {
    return;
  }
