 ...
 * toggle server-side adapter code for the whole DealDesk instance.
 *
...
interface AdapterInstallRequest {
  /** npm package name (e.g., "droid-dealdesk-adapter") or local path */
  packageName: string;
...
export function adapterRoutes() {
  ...
  router.post("/adapters/install", async (req, res) => {
    ...
    // e.g. "@henkey/hermes-dealdesk-adapter@0.3.0" → packageName + version
  ...
  // without modifying DealDesk's source code.
  //
