...
import { heartbeatRuns, instanceUserRoles, invites } from "@dealdesk/db";
...
function hasDevServerStatusToken(providedToken: string | undefined) {
  const expectedToken = process.env.DEALDESK_DEV_SERVER_STATUS_TOKEN?.trim();
...
export function healthRoutes(
  ...
  router.get("/", async (req, res) => {
    ...
    const exposeDevServerDetails =
      exposeFullDetails || hasDevServerStatusToken(req.get("x-dealdesk-dev-server-status-token"));
