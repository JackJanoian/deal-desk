...
import type { DeploymentExposure, DeploymentMode, HumanCompanyMembershipRole, PermissionKey } from "@dealdesk/shared";
import {
  forbidden,
  conflict,
  notFound,
...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (
    normalized !== "dealdesk" &&
    normalized !== "dealdesk-create-agent" &&
    normalized !== "dealdesk-create-plugin" &&
    normalized !== "dealdesk-converting-plans-to-tasks" &&
...
/** Resolve the DealDesk repo skills directory (built-in / managed skills). */
function resolveDealDeskSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const dealDeskSkillsDir = resolveDealDeskSkillsDir();

  // Build set of DealDesk-managed skill names
  const dealDeskSkillNames = new Set<string>();
  if (dealDeskSkillsDir) {
    try {
      for (const entry of fs.readdirSync(dealDeskSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) dealDeskSkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isDealDeskManaged: dealDeskSkillNames.has(entry.name),
      });
    }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.dealDeskApiUrl)) {
    const legacyDealDeskApiUrl = nonEmptyTrimmedString(input.dealDeskApiUrl);
    if (legacyDealDeskApiUrl) merged.dealDeskApiUrl = legacyDealDeskApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    dealDeskApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.dealDeskApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawDealDeskApiUrl =
    typeof defaults.dealDeskApiUrl === "string"
      ? defaults.dealDeskApiUrl.trim()
      : "";
  if (rawDealDeskApiUrl) {
    try {
      const parsedDealDeskApiUrl = new URL(rawDealDeskApiUrl);
      if (
        parsedDealDeskApiUrl.protocol !== "http:" &&
        parsedDealDeskApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_dealdesk_api_url_protocol",
          level: "warn",
          message: `dealDeskApiUrl must use http:// or https:// (got ${parsedDealDeskApiUrl.protocol}).`
        });
      } else {
        normalized.dealDeskApiUrl = parsedDealDeskApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_dealdesk_api_url_configured",
          level: "info",
          message: `dealDeskApiUrl set to ${parsedDealDeskApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_dealdesk_api_url_invalid",
        level: "warn",
        message: `Invalid dealDeskApiUrl: ${rawDealDeskApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your DealDesk host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "DealDesk is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm dealdesk allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = `/api/invites/${token}/skills/dealdesk`;
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/dealdesk-claimed-api-key.json and load DEALDESK_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      inviteMessage: extractInviteMessage(invite),
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: dealDeskApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the DealDesk hostname is reachable and allowed via `pnpm dealdesk allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this DealDesk API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/dealdesk/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # DealDesk OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          dealDeskApiUrl: "http://host.docker.internal:3100",
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, DealDesk generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "dealDeskApiUrl": "https://dealdesk-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in DealDesk before key claim is allowed.
    ...
    - ~/.openclaw/workspace/dealdesk-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/dealdesk-claimed-api-key.json

    And set the DEALDESK_API_KEY and DEALDESK_API_URL in your environment variables as specified here:
    ...
    Then set DEALDESK_API_KEY and DEALDESK_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install DealDesk skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your DEALDESK_API_URL to the top of your skill and note the path to your DEALDESK_API_URL
    ...
    ${
      ...
      "Ensure DealDesk is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested DealDesk base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.dealDeskApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm dealdesk allowed-hostname <host>
      - then restart DealDesk and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@dealdesk.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (req, res) => {
    ...
    res.json({
      skills: [
        { name: "dealdesk", path: "/api/skills/dealdesk" },
        {
          name: "para-memory-files",
          path: "/api/skills/para-memory-files"
        },
        {
          name: "dealdesk-create-agent",
          path: "/api/skills/dealdesk-create-agent"
        },
        {
          name: "dealdesk-converting-plans-to-tasks",
          path: "/api/skills/dealdesk-converting-plans-to-tasks"
        }
      ]
    });
  });
  ...
  router.get("/invites/:token/skills/index", async (req, res) => {
    ...
    res.json({
      skills: [
        {
          name: "dealdesk",
          path: `/api/invites/${token}/skills/dealdesk`,
        },
      ],
    });
  });
  ...
  router.get("/invites/:token/skills/:skillName", async (req, res) => {
    ...
    const skillName = (req.params.skillName as string).trim().toLowerCase();
    if (skillName !== "dealdesk") throw notFound("Skill not found");
    const markdown = readSkillMarkdown(skillName);
    if (!markdown) throw notFound("Skill not found");
    res.type("text/markdown").send(markdown);
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              dealDeskApiUrl: req.body.dealDeskApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.dealDeskApiUrl &&
          !persistedDefaults.dealDeskApiUrl
        ) {
          missingPersistedFields.push("dealDeskApiUrl");
        }
