...
import { issues, projects, projectWorkspaces } from "@dealdesk/db";
import {
...
import type { WorkspaceRuntimeDesiredState, WorkspaceRuntimeServiceStateMap } from "@dealdesk/shared";
...
export function executionWorkspaceRoutes(db: Db) {
  ...
  async function handleExecutionWorkspaceRuntimeCommand(req: Request, res: Response) {
    ...
    if (!workspaceCwd) {
      res.status(422).json({ error: "Execution workspace needs a local path before DealDesk can run workspace commands" });
      return;
    }
      ...
      run: async () => {
        ...
        if (action === "run") {
          ...
          if (!availableWorkspace) {
            throw new Error("Execution workspace needs a local path before DealDesk can run workspace commands");
          }
        ...
        if (action === "start" || action === "restart") {
          const availableWorkspace = await ensureWorkspaceAvailable();
          if (!availableWorkspace) {
            throw new Error("Execution workspace needs a local path before DealDesk can manage local runtime services");
          }
