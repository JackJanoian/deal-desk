...
describe("agent instructions service", () => {
  const originalDealDeskHome = process.env.DEALDESK_HOME;
  const originalDealDeskInstanceId = process.env.DEALDESK_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalDealDeskHome === undefined) delete process.env.DEALDESK_HOME;
    else process.env.DEALDESK_HOME = originalDealDeskHome;
    if (originalDealDeskInstanceId === undefined) delete process.env.DEALDESK_INSTANCE_ID;
    else process.env.DEALDESK_INSTANCE_ID = originalDealDeskInstanceId;
