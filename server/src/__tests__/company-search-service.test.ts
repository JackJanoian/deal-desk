...
import { companySearchQuerySchema, COMPANY_SEARCH_MAX_QUERY_LENGTH } from "@dealdesk/shared";
import {
...
describeEmbeddedPostgres("companySearchService", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("dealdesk-company-search-");
  ...
  async function createCompany(name = "DealDesk") {
