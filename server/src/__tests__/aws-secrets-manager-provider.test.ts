...
describe("awsSecretsManagerProvider", () => {
  const previousEnv = {
    DEALDESK_SECRETS_AWS_REGION: process.env.DEALDESK_SECRETS_AWS_REGION,
    AWS_REGION: process.env.AWS_REGION,
    AWS_DEFAULT_REGION: process.env.AWS_DEFAULT_REGION,
    DEALDESK_SECRETS_AWS_DEPLOYMENT_ID: process.env.DEALDESK_SECRETS_AWS_DEPLOYMENT_ID,
    DEALDESK_SECRETS_AWS_KMS_KEY_ID: process.env.DEALDESK_SECRETS_AWS_KMS_KEY_ID,
  ...
  it("creates DealDesk-managed AWS secrets without persisting plaintext in provider material", async () => {
    ...
    const provider = createAwsSecretsManagerProvider({
      ...
      gateway: {
        async createSecret(input) {
          ...
          return {
            ARN: "arn:aws:secretsmanager:us-east-1:123456789012:secret:dealdesk/prod-use1/company-1/openai-api-key",
    ...
    expect(calls).toEqual([
      expect.objectContaining({
        ...
        input: expect.objectContaining({
          Name: "dealdesk/prod-use1/company-1/openai-api-key",
          KmsKeyId: "arn:aws:kms:us-east-1:123456789012:key/test",
        }),
      }),
    ]);
    expect(JSON.stringify(prepared)).not.toContain("super-secret-value");
    expect(prepared.externalRef).toContain("dealdesk/prod-use1/company-1/openai-api-key");
      ...
      expect.objectContaining({
        ...
        input: expect.objectContaining({
          ...
          Tags: expect.arrayContaining([
            { Key: "dealdesk:provider-owner", Value: "platform" },
            { Key: "dealdesk:environment", Value: "production" },
          ]),
        }),
      }),
    ]);
  ...
  it("signs AWS Secrets Manager JSON requests with default runtime credentials", async () => {
    ...
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValue(
      new Response(
        JSON.stringify({
          ARN: "arn:aws:secretsmanager:us-east-1:123456789012:secret:dealdesk/prod/company-1/openai-api-key",
          VersionId: "aws-version-1",
        }),
        { status: 200 },
      ),
    ...
    expect(calls).toEqual([
      {
        ...
        input: {
          ...
          VersionStages: ["DEALDESK_PENDING"],
        },
      },
    ]);
  ...
  it("rejects linked external references under the DealDesk-managed namespace", async () => {
    const provider = createAwsSecretsManagerProvider({
      config: {
        region: "us-east-1",
        endpoint: "https://secretsmanager.us-east-1.amazonaws.com",
    ...
    await expect(
      provider.linkExternalSecret({
        externalRef:
          "arn:aws:secretsmanager:us-east-1:123456789012:secret:dealdesk/prod-use1/company-2/openai-api-key",
        providerVersionRef: "linked-version-7",
      }),
    ).rejects.toThrow(/DealDesk-managed namespace/i);
  });
  ...
  it("redacts AWS provider exception text when remote listing fails", async () => {
    const rawProviderMessage =
      "AccessDeniedException: User: arn:aws:sts::123456789012:assumed-role/prod/DealDesk is not authorized to perform secretsmanager:ListSecrets on arn:aws:secretsmanager:us-east-1:123456789012:secret:prod/openai";
    const provider = createAwsSecretsManagerProvider({
      config: {
        region: "us-east-1",
        endpoint: "https://secretsmanager.us-east-1.amazonaws.com",
  ...
  it("resolves AWS secret values by provider version reference", async () => {
    ...
    const resolved = await provider.resolveVersion({
      material: {
        ...
        secretId: "arn:aws:secretsmanager:us-east-1:123456789012:secret:dealdesk/prod-use1/company-1/openai-api-key",
        versionId: "aws-version-2",
        source: "managed",
      },
      externalRef:
  ...
  it("warns when AWS provider configuration is incomplete and blocks managed writes", async () => {
    ...
    expect(health.message).toContain("missing DEALDESK_SECRETS_AWS_REGION");
    ...
    ).rejects.toThrow(/DEALDESK_SECRETS_AWS_REGION|AWS_REGION/i);
  });

  it("deletes only DealDesk-managed AWS secrets", async () => {
    const calls: Array<{ op: string; input: Record<string, unknown> }> = [];
    const provider = createAwsSecretsManagerProvider({
      config: {
        region: "us-east-1",
  ...
  it("archives pending DealDesk-managed AWS versions without deleting the secret", async () => {
    const calls: Array<{ op: string; input: Record<string, unknown> }> = [];
    const provider = createAwsSecretsManagerProvider({
      config: {
        region: "us-east-1",
