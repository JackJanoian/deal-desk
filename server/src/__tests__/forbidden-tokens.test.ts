...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "dealdesk", LOGNAME: "dealdesk", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "dealdesk" }),
      },
    );

    expect(tokens).toEqual(["dealdesk", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "dealdesk" }, {
        userInfo: () => ({ username: "dealdesk" }),
      });

      expect(tokens).toEqual(["dealdesk", "custom-token"]);
    } finally {
