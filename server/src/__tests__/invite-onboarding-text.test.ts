...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("DealDesk OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("/api/invites/token-123/skills/dealdesk");
    expect(text).toContain("Suggested DealDesk base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("dealDeskApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.dealDeskApiUrl");
    expect(text).toContain("~/.openclaw/workspace/dealdesk-claimed-api-key.json");
    expect(text).toContain("DEALDESK_API_KEY");
