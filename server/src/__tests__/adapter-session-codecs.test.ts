...
import { sessionCodec as claudeSessionCodec } from "@dealdesk/adapter-claude-local/server";
import { sessionCodec as codexSessionCodec, isCodexUnknownSessionError } from "@dealdesk/adapter-codex-local/server";
import {
...
import { sessionCodec as acpxSessionCodec } from "@dealdesk/adapter-acpx-local/server";
...
describe("adapter session codecs", () => {
  ...
  it("preserves acpx session params required for compatibility checks", () => {
    const parsed = acpxSessionCodec.deserialize({
      sessionKey: "dealdesk:company:agent:task:fingerprint",
