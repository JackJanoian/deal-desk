...
describeEmbeddedPostgres("plugin orchestration APIs", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("dealdesk-plugin-orchestration-");
  ...
  async function makeLocalRoot() {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "dealdesk-plugin-host-folder-"));
    tempRoots.push(root);
    return root;
  }
  ...
  it("creates plugin-origin issues with full orchestration fields and audit activity", async () => {
    ...
    const services = buildHostServices(db, "plugin-record-id", "dealdesk.missions", createEventBusStub());
    ...
    expect(stored?.originKind).toBe("plugin:dealdesk.missions");
      ...
      expect.arrayContaining([
        expect.objectContaining({
          ...
          details: expect.objectContaining({
            ...
            sourcePluginKey: "dealdesk.missions",
  ...
  it("enforces plugin origin namespaces", async () => {
    ...
    expect(featureIssue.originKind).toBe("plugin:dealdesk.missions:feature");
    ...
    ).rejects.toThrow("Plugin may only use originKind values under plugin:dealdesk.missions");
  ...
  it("creates plugin operation issues with the generic operation origin", async () => {
    ...
    expect(issue.originKind).toBe("plugin:dealdesk.missions:operation");
  ...
  it("lets bootstrap-style actions initialize required local folders from an empty root", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "dealdesk.plugin-llm-wiki",
      packageName: "@dealdesk/plugin-llm-wiki",
      version: "0.1.0",
      manifestJson: {
        id: "dealdesk.plugin-llm-wiki",
        apiVersion: 1,
        version: "0.1.0",
        ...
        localFolders: [
          {
            ...
            access: "readWrite",
            requiredDirectories: ["raw", "wiki", "wiki/concepts", ".dealdesk"],
  ...
  it("rejects worker local-folder access for undeclared manifest keys", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "dealdesk.local-folders",
      packageName: "@dealdesk/plugin-local-folders",
      version: "0.1.0",
      manifestJson: {
        id: "dealdesk.local-folders",
        apiVersion: 1,
        version: "0.1.0",
  ...
  it("resolves plugin-managed projects by stable key without overwriting user edits", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "dealdesk.missions",
      packageName: "@dealdesk/plugin-missions",
      version: "0.1.0",
      apiVersion: 1,
      categories: ["automation"],
      status: "ready",
      manifestJson: {
        id: "dealdesk.missions",
        apiVersion: 1,
        version: "0.1.0",
    ...
    const services = buildHostServices(db, pluginId, "dealdesk.missions", createEventBusStub());
