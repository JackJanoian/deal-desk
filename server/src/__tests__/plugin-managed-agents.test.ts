...
describeEmbeddedPostgres("plugin-managed agents", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("dealdesk-plugin-managed-agents-");
  ...
  async function seedCompanyAndPlugin(options: { requireApproval?: boolean; manifest?: DealDeskPluginManifestV1 } = {}) {
    ...
    await db.insert(plugins).values({
      ...
      packageName: "@dealdesk/plugin-managed-agents-test",
  ...
  it("materializes declared managed agent instructions with local folder paths", async () => {
    ...
    try {
      ...
      if (previousHome === undefined) delete process.env.DEALDESK_HOME;
      else process.env.DEALDESK_HOME = previousHome;
      if (previousInstance === undefined) delete process.env.DEALDESK_INSTANCE_ID;
      else process.env.DEALDESK_INSTANCE_ID = previousInstance;
