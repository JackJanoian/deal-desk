...
function registerModuleMocks() {
  vi.doMock("@dealdesk/adapter-opencode-local/server", async () => {
    const actual = await vi.importActual<typeof import("@dealdesk/adapter-opencode-local/server")>("@dealdesk/adapter-opencode-local/server");
    return {
      ...actual,
...
describe.sequential("agent permission routes", () => {
  beforeEach(() => {
    vi.resetModules();
    vi.doUnmock("@dealdesk/shared/telemetry");
    ...
    vi.doUnmock("@dealdesk/adapter-opencode-local/server");
    registerModuleMocks();
    vi.resetAllMocks();
