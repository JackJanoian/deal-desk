...
import { runChildProcess } from "@dealdesk/adapter-utils/server-utils";
import { execute } from "@dealdesk/adapter-codex-local/server";

async function writeFakeCodexCommand(commandPath: string): Promise<void> {
  const script = `#!/usr/bin/env node
const fs = require("node:fs");

const capturePath = process.env.DEALDESK_TEST_CAPTURE_PATH;
const payload = {
...
async function createRuntimeSkillDir(root: string, name: string) {
...
describe("codex execute", () => {
  it("uses a DealDesk-managed CODEX_HOME outside worktree mode while preserving shared auth and config", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "dealdesk-codex-execute-default-"));
    ...
    const previousDealDeskHome = process.env.DEALDESK_HOME;
    const previousDealDeskInstanceId = process.env.DEALDESK_INSTANCE_ID;
    const previousDealDeskInWorktree = process.env.DEALDESK_IN_WORKTREE;
    const previousCodexHome = process.env.CODEX_HOME;
    process.env.HOME = root;
    process.env.DEALDESK_HOME = dealDeskHome;
    delete process.env.DEALDESK_INSTANCE_ID;
    delete process.env.DEALDESK_IN_WORKTREE;
    ...
    try {
      ...
      const result = await execute({
        ...
        config: {
          ...
          cwd: workspace,
          env: {
            DEALDESK_TEST_CAPTURE_PATH: capturePath,
          },
          promptTemplate: "Follow the dealdesk heartbeat.",
          dealdeskSkillSync: { desiredSkills: ["dealdesk/dealdesk/dealdesk"] },
          dealDeskRuntimeSkills: [{
            key: "dealdesk/dealdesk/dealdesk",
            runtimeName: "dealdesk",
            source: dealdeskSkillDir,
          }],
        },
        context: {},
      ...
      expect(logs).toContainEqual(
        expect.objectContaining({
          stream: "stdout",
          chunk: expect.stringContaining("Using DealDesk-managed Codex home"),
        }),
      );
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
      if (previousDealDeskHome === undefined) delete process.env.DEALDESK_HOME;
      else process.env.DEALDESK_HOME = previousDealDeskHome;
      if (previousDealDeskInstanceId === undefined) delete process.env.DEALDESK_INSTANCE_ID;
      else process.env.DEALDESK_INSTANCE_ID = previousDealDeskInstanceId;
      if (previousDealDeskInWorktree === undefined) delete process.env.DEALDESK_IN_WORKTREE;
      else process.env.DEALDESK_IN_WORKTREE = previousDealDeskInWorktree;
  ...
  it("emits a command note that Codex auto-applies repo-scoped AGENTS.md files", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "dealdesk-codex-execute-notes-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("logs HOME and the resolved executable path in invocation metadata", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "dealdesk-codex-execute-meta-"));
    const workspace = path.join(root, "workspace");
    ...
    try {
      ...
      expect(loggedEnv.DEALDESK_RESOLVED_COMMAND).toBe(commandPath);
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
  ...
  it("injects bridge env into sandbox-managed remote runs", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "dealdesk-codex-execute-sandbox-"));
  ...
  it("injects structured DealDesk wake payloads into env and prompt", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "dealdesk-codex-execute-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      const capture = JSON.parse(await fs.readFile(capturePath, "utf8")) as CapturePayload;
      expect(capture.dealDeskEnvKeys).toContain("DEALDESK_WAKE_PAYLOAD_JSON");
      expect(capture.dealDeskWakePayloadJson).not.toBeNull();
      expect(JSON.parse(capture.dealDeskWakePayloadJson ?? "{}")).toMatchObject({
        reason: "issue_commented",
        latestCommentId: "comment-2",
        commentIds: ["comment-1", "comment-2"],
      });
      expect(capture.prompt).toContain("## DealDesk Wake Payload");
  ...
  it("classifies remote-compaction high-demand failures as retryable transient upstream errors", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "dealdesk-codex-execute-transient-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("persists retry-not-before metadata for codex usage-limit failures", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "dealdesk-codex-execute-usage-limit-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    await fs.mkdir(workspace, { recursive: true });
    await writeFailingCodexCommand(
  ...
  it("uses safer invocation settings and a fresh-session handoff for codex transient fallback retries", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "dealdesk-codex-execute-fallback-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      expect(capture.prompt).toContain("DealDesk session handoff:");
  ...
  it("renders execution-stage wake instructions for reviewer and executor roles", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "dealdesk-codex-execute-stage-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      const executorResult = await execute({
        ...
        config: {
          ...
          cwd: workspace,
          env: {
            DEALDESK_TEST_CAPTURE_PATH: executorCapturePath,
          },
          promptTemplate: "Follow the dealdesk heartbeat.",
        },
        context: {
  ...
  it("renders an issue-scoped wake prompt even when the wake has no comments yet", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "dealdesk-codex-execute-issue-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("uses a compact wake delta instead of the full heartbeat prompt when resuming a session", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "dealdesk-codex-execute-resume-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    ...
    try {
      ...
      expect(capture.prompt).toContain("## DealDesk Resume Delta");
      expect(capture.prompt).toContain("Do not switch to another issue until you have handled this wake.");
      expect(capture.prompt).toContain("Second comment");
      expect(capture.prompt).not.toContain("Follow the dealdesk heartbeat.");
      expect(capture.prompt).not.toContain("You are managed instructions.");
      expect(invocationPrompt).toContain("## DealDesk Resume Delta");
  ...
  it("uses a worktree-isolated CODEX_HOME while preserving shared auth and config", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "dealdesk-codex-execute-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    const sharedCodexHome = path.join(root, "shared-codex-home");
    ...
    const homeSkill = path.join(isolatedCodexHome, "skills", "dealdesk");
    await fs.mkdir(workspace, { recursive: true });
    const worktreeDealdeskSkillDir = await createRuntimeSkillDir(runtimeSkillsRoot, "dealdesk");
    await fs.mkdir(sharedCodexHome, { recursive: true });
    await fs.writeFile(path.join(sharedCodexHome, "auth.json"), '{"token":"shared"}\n', "utf8");
    await fs.writeFile(path.join(sharedCodexHome, "config.toml"), 'model = "codex-mini-latest"\n', "utf8");
    await writeFakeCodexCommand(commandPath);
    ...
    process.env.HOME = root;
    process.env.DEALDESK_HOME = dealDeskHome;
    process.env.DEALDESK_INSTANCE_ID = "worktree-1";
    process.env.DEALDESK_IN_WORKTREE = "true";
    process.env.CODEX_HOME = sharedCodexHome;
    ...
    try {
      ...
      const result = await execute({
        ...
        config: {
          ...
          dealDeskRuntimeSkills: [{
            key: "dealdesk/dealdesk/dealdesk",
            runtimeName: "dealdesk",
            source: worktreeDealdeskSkillDir,
          }],
        },
        context: {},
        authToken: "run-jwt-token",
      ...
      expect(capture.argv).toEqual(expect.arrayContaining(["exec", "--json", "-"]));
      expect(capture.prompt).toContain("Follow the dealdesk heartbeat.");
      expect(capture.dealDeskEnvKeys).toEqual(
      ...
      expect(logs).toContainEqual(
        expect.objectContaining({
          stream: "stdout",
          chunk: expect.stringContaining('Injected Codex skill "dealdesk"'),
        }),
      );
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
  ...
  it("respects an explicit CODEX_HOME config override even in worktree mode", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "dealdesk-codex-execute-explicit-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    const sharedCodexHome = path.join(root, "shared-codex-home");
    ...
    try {
      const result = await execute({
        ...
        config: {
          ...
          dealDeskRuntimeSkills: [{
            key: "dealdesk/dealdesk/dealdesk",
            runtimeName: "dealdesk",
            source: explicitDealdeskSkillDir,
          }],
        },
        context: {},
        authToken: "run-jwt-token",
      ...
      expect((await fs.lstat(path.join(explicitCodexHome, "skills", "dealdesk"))).isSymbolicLink()).toBe(true);
      await expect(fs.lstat(path.join(dealDeskHome, "instances", "worktree-1", "codex-home"))).rejects.toThrow();
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
      if (previousDealDeskHome === undefined) delete process.env.DEALDESK_HOME;
