...
import { buildSandboxNpmInstallCommand } from "@dealdesk/adapter-utils";
...
vi.mock("hermes-dealdesk-adapter/server", () => ({
...
describe("server adapter registry", () => {
  ...
  it("built-in local adapters declare cheap model profile defaults where supported", async () => {
    ...
    await expect(listAdapterModelProfiles("opencode_local")).resolves.toEqual([
      expect.objectContaining({
        key: "cheap",
        adapterConfig: expect.objectContaining({ model: "opencode/gpt-5.1-codex-mini" }),
        source: "adapter_default",
      }),
    ]);
  ...
  it("injects the local agent JWT and DealDesk API auth guidance into Hermes", async () => {
    ...
    expect(patchedCtx.agent.adapterConfig).toMatchObject({
      env: {
        OPENAI_API_KEY: "llm-token",
        DEALDESK_API_KEY: "agent-run-jwt",
        DEALDESK_RUN_ID: "run-123",
      },
    });
    expect(patchedCtx.agent.adapterConfig.promptTemplate).toContain(
      "Authorization: Bearer $DEALDESK_API_KEY",
    );
    expect(patchedCtx.agent.adapterConfig.promptTemplate).toContain(
      "X-DealDesk-Run-Id: $DEALDESK_RUN_ID",
    );
  ...
  it("preserves Hermes command normalization while injecting auth", async () => {
    ...
    expect(patchedCtx.agent.adapterConfig.env.DEALDESK_API_KEY).toBe("agent-run-jwt");
  });
  ...
  it("passes the original Hermes context through when authToken is absent", async () => {
    ...
    const ctx = {
      ...
      agent: {
        ...
        adapterConfig: {
          env: {
            DEALDESK_API_KEY: "server-level-key",
          },
          promptTemplate: "Existing prompt",
        },
      },
  ...
  it("preserves an explicit Hermes DealDesk API key and does not set promptTemplate when none was configured", async () => {
    const adapter = requireServerAdapter("hermes_local");
    ...
    await adapter.execute({
      ...
      agent: {
        ...
        adapterConfig: {
          env: {
            DEALDESK_API_KEY: "explicit-agent-key",
            DEALDESK_RUN_ID: "stale-run-id",
          },
        },
      },
      runtime: {},
    ...
    const [patchedCtx] = hermesExecuteMock.mock.calls[0];
    expect(patchedCtx.agent.adapterConfig.env.DEALDESK_API_KEY).toBe("explicit-agent-key");
    expect(patchedCtx.agent.adapterConfig.env.DEALDESK_RUN_ID).toBe("run-123");
