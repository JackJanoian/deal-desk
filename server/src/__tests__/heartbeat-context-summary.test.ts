...
import {
  buildDealDeskTaskMarkdown,
  mergeCoalescedContextSnapshot,
  summarizeHeartbeatRunContextSnapshot,
  summarizeHeartbeatRunListResultJson,
} from "../services/heartbeat.js";

describe("buildDealDeskTaskMarkdown", () => {
  it("adds planning directives for assignment and comment task context", () => {
    const assignment = buildDealDeskTaskMarkdown({
      issue: {
        id: "issue-1",
        identifier: "PAP-3404",
        title: "Plan first",
    ...
    const commentWake = buildDealDeskTaskMarkdown({
      issue: {
        id: "issue-1",
        identifier: "PAP-3404",
        title: "Plan first",
    ...
    const acceptedConfirmation = buildDealDeskTaskMarkdown({
      issue: {
        id: "issue-1",
        identifier: "PAP-3404",
        title: "Plan first",
...
describe("summarizeHeartbeatRunContextSnapshot", () => {
  ...
  it("returns null when no allowed fields are present", () => {
    expect(
      summarizeHeartbeatRunContextSnapshot({
        dealDeskWake: { comments: [{ body: "hello" }] },
      }),
    ).toBeNull();
  });
});
