...
const ORIGINAL_INSTANCE_ID = process.env.DEALDESK_INSTANCE_ID;

afterEach(() => {
  if (ORIGINAL_INSTANCE_ID === undefined) delete process.env.DEALDESK_INSTANCE_ID;
  else process.env.DEALDESK_INSTANCE_ID = ORIGINAL_INSTANCE_ID;
});

describe("Better Auth cookie scoping", () => {
  it("derives an instance-scoped cookie prefix", () => {
    expect(deriveAuthCookiePrefix("default")).toBe("dealdesk-default");
    expect(deriveAuthCookiePrefix("PAP-1601-worktree")).toBe("dealdesk-PAP-1601-worktree");
  });

  it("uses DEALDESK_INSTANCE_ID for the Better Auth cookie prefix", () => {
    process.env.DEALDESK_INSTANCE_ID = "sat-worktree";

    const advanced = buildBetterAuthAdvancedOptions({ disableSecureCookies: false });

    expect(advanced).toEqual({
      cookiePrefix: "dealdesk-sat-worktree",
    });
    expect(getCookies({ advanced } as BetterAuthOptions).sessionToken.name).toBe(
      "dealdesk-sat-worktree.session_token",
    );
  });

  it("keeps local http auth cookies non-secure while preserving the scoped prefix", () => {
    process.env.DEALDESK_INSTANCE_ID = "pap-worktree";

    expect(buildBetterAuthAdvancedOptions({ disableSecureCookies: true })).toEqual({
      cookiePrefix: "dealdesk-pap-worktree",
