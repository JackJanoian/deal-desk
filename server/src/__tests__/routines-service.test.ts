...
describeEmbeddedPostgres("routine service live-execution coalescing", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("dealdesk-routines-service-");
  ...
  it("interpolates routine variables into the execution issue and stores resolved values", async () => {
    ...
    expect(storedIssue?.title).toBe("repo triage for dealdesk");
    expect(storedIssue?.description).toBe("Review dealdesk for high bugs");
