...
import { execute, testEnvironment } from "@dealdesk/adapter-openclaw-gateway/server";
import {
  buildOpenClawGatewayConfig,
  parseOpenClawGatewayStdoutLine,
} from "@dealdesk/adapter-openclaw-gateway/ui";
...
describe("openclaw gateway adapter execute", () => {
  it("runs connect -> agent -> agent.wait and forwards wake payload", async () => {
    ...
    try {
      ...
      expect(payload?.sessionKey).toBe("dealdesk:issue:issue-123");
      expect(String(payload?.message ?? "")).toContain("wake now");
      expect(String(payload?.message ?? "")).toContain("DEALDESK_RUN_ID=run-123");
      expect(String(payload?.message ?? "")).toContain("DEALDESK_TASK_ID=task-123");
      expect(String(payload?.message ?? "")).toContain("## DealDesk Wake Payload");
      ...
      expect(payload?.dealDesk).toMatchObject({
        wake: {
          latestCommentId: "comment-2",
          commentIds: ["comment-1", "comment-2"],
        },
