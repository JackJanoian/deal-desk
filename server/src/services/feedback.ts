...
import { claudeConfigDir, parseClaudeStreamJson } from "@dealdesk/adapter-claude-local/server";
import { codexHomeDir, parseCodexJsonl } from "@dealdesk/adapter-codex-local/server";
import { parseOpenCodeJsonl } from "@dealdesk/adapter-opencode-local/server";
import {
...
import { resolveHomeAwarePath, resolveDealDeskInstanceRoot } from "../home-paths.js";
...
const FEEDBACK_SCHEMA_VERSION = "dealdesk-feedback-envelope-v2";
const FEEDBACK_BUNDLE_VERSION = "dealdesk-feedback-bundle-v2";
const FEEDBACK_PAYLOAD_VERSION = "dealdesk-feedback-v1";
const FEEDBACK_DESTINATION = "dealdesk_labs_feedback_v1";
...
function captureStatusFromFiles(files: FeedbackTraceBundleFile[]): FeedbackTraceBundleCaptureStatus {
  ...
  const hasAdapterFiles = files.some((file) =>
    file.source !== "dealdesk_run" &&
    file.source !== "dealdesk_run_events" &&
    file.source !== "dealdesk_run_log",
  );
...
async function buildOpenCodeTraceFiles(input: {
  ...
  const opencodeRoot = resolveHomeAwarePath(
    process.env.DEALDESK_OPENCODE_STORAGE_DIR ?? "~/.local/share/opencode",
  );
...
async function buildAgentContext(
  ...
  const desiredSkillRefs = uniqueNonEmpty(readDealDeskSkillSyncPreference(adapterConfig).desiredSkills).slice(0, MAX_SKILLS);
...
async function buildFeedbackTraceBundleFromRow(
  ...
  let dealDeskRun: Record<string, unknown> | null = null;
  ...
  if (!sourceRunId) {
    ...
    if (!run || run.companyId !== row.companyId) {
      ...
      dealDeskRun = sanitizeFeedbackValue(
        {
          id: run.id,
      ...
      files.push(makeBundleFile({
        path: "dealdesk/run.json",
        contentType: "application/json",
        source: "dealdesk_run",
        contents: `${JSON.stringify(dealDeskRun, null, 2)}\n`,
      }));

      const sanitizedEvents = sanitizeFeedbackValue(
        events,
        state,
        "bundle.dealDeskRun.events",
        MAX_TRACE_FILE_CHARS,
      );
      files.push(makeBundleFile({
        path: "dealdesk/run-events.json",
        contentType: "application/json",
        source: "dealdesk_run_events",
        contents: `${JSON.stringify(sanitizedEvents, null, 2)}\n`,
      }));

      if (logText) {
        files.push(makeBundleFile({
          path: "dealdesk/run-log.ndjson",
          contentType: "application/x-ndjson",
          source: "dealdesk_run_log",
          contents: `${sanitizeFeedbackText(logText, state, "bundle.dealDeskRun.log", MAX_TRACE_FILE_CHARS)}\n`,
        }));
      } else {
