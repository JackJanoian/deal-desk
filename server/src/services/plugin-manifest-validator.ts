 ...
 */
import { pluginManifestV1Schema } from "@dealdesk/shared";
import type { DealDeskPluginManifestV1 } from "@dealdesk/shared";
import { PLUGIN_API_VERSION } from "@dealdesk/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): DealDeskPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as DealDeskPluginManifestV1,
        };
      }
