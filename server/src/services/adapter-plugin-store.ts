 ...
 * ~/.dealdesk/adapter-plugins.json. This is the source of truth for which
...
import { resolveDealDeskHomeDir } from "../home-paths.js";
...
export interface AdapterPluginRecord {
  /** npm package name (e.g., "droid-dealdesk-adapter") */
  packageName: string;
...
function ensureDirs(): string {
  ...
  if (!fs.existsSync(pkgJsonPath)) {
    fs.writeFileSync(pkgJsonPath, JSON.stringify({
      name: "dealdesk-adapter-plugins",
      version: "0.0.0",
      private: true,
      description: "Managed directory for DealDesk external adapter plugins. Do not edit manually.",
    }, null, 2) + "\n");
  }
