...
function resolveMasterKeyFilePath() {
  const fromEnv = process.env.DEALDESK_SECRETS_MASTER_KEY_FILE;
...
function loadOrCreateMasterKey(): Buffer {
  const envKeyRaw = process.env.DEALDESK_SECRETS_MASTER_KEY;
  if (envKeyRaw && envKeyRaw.trim().length > 0) {
    const fromEnv = decodeMasterKey(envKeyRaw);
    if (!fromEnv) {
      throw badRequest(
        "Invalid DEALDESK_SECRETS_MASTER_KEY (expected 32-byte base64, 64-char hex, or raw 32-char string)",
      );
    }
    return fromEnv;
  }
...
async function inspectLocalEncryptedHealth(): Promise<SecretProviderHealthCheck> {
  ...
  if (envKeyRaw && envKeyRaw.trim().length > 0) {
    ...
    return {
      provider: "local_encrypted",
      status: "ok",
      message: "Local encrypted provider is using DEALDESK_SECRETS_MASTER_KEY",
      backupGuidance: [
...
export const localEncryptedProvider: SecretProviderModule = {
  ...
  async deleteOrArchive() {
    // Secret metadata deletion is handled in DealDesk DB; the local key is shared and must remain.
  },
  async healthCheck() {
