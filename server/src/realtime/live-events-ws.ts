...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@dealdesk/db";
...
interface IncomingMessageWithContext extends IncomingMessage {
  dealDeskUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).dealDeskUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.dealDeskUpgradeContext = context;
