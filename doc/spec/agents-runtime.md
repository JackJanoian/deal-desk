...
For `claude_local` and `codex_local`, DealDesk assumes the CLI is already installed and authenticated on the host machine.
...
DealDesk stores resumable session state per `(agent, taskKey, adapterType)`.
