...
This spec defines how DealDesk actually runs agents while staying runtime-agnostic.
...
1. DealDesk is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, DealDesk manages lifecycle, and on exit DealDesk parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `dealdesk.skill` (shared DealDesk skill text block)
...
2. Preferred transport is env vars (`DEALDESK_*`) injected at runtime.
