...
This document defines the next-stage plan for DealDesk company import/export.

The core shift is:

- move from a DealDesk-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside DealDesk.
...
DealDesk already has portability primitives in the repo:
...
11. adapter-aware skill sync surfaces so DealDesk can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point DealDesk at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a DealDesk sidecar.
...
DealDesk must not redefine `SKILL.md`.
...
### 5.4 Base Package Vs DealDesk Extension
...
DealDesk import/export should support these entity kinds:
...
- DealDesk can treat “this set of agents came from team package X” as the imported-team model
...
DealDesk should support source references in package metadata with:
...
DealDesk should define a new adapter surface area around skills:
...
Planned DealDesk behavior:

- if an adapter supports read, DealDesk should show current skills in the UI
- if an adapter supports write, DealDesk should let the user enable/disable imported skills
- if an adapter supports sync, DealDesk should compute desired vs actual state and offer reconcile actions
...
## 10. Storage Model Inside DealDesk
...
DealDesk should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `dealdesk company export <company-id> --out <path>`
- `dealdesk company import <path-or-url> --dry-run`
- `dealdesk company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in DealDesk storage, or only referenced at import time?
...
3. Should DealDesk support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps DealDesk aligned with:
