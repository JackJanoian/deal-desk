...
Yes, there is a likely root cause in DealDesk's local execution model.
...
If Codex, Claude, Cursor, or a skill launched through them starts Chrome or Chromium helpers, DealDesk can lose ownership of those descendants even when it still believes it handled the run correctly.
...
This is the clearest internal comparison point: DealDesk already has one local-process subsystem that treats process-group ownership as the right abstraction.
...
- DealDesk may think it cancelled the run because the parent process is gone
...
This work is successful when DealDesk can:
...
- require every skill author to add bespoke browser teardown logic before DealDesk can clean up correctly
...
- make the leak measurable from DealDesk's side before changing execution semantics
...
- add a control-plane cleanup action or CLI utility for stale local run processes owned by DealDesk
...
Treat this as a DealDesk executor ownership bug, not an `agent-browser` bug.
