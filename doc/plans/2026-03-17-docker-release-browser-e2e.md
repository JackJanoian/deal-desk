...
Today release smoke testing for published DealDesk packages is manual and shell-driven:
...
- runs `dealdesk@${DEALDESKAI_VERSION}` inside Docker
- waits for health
...
- the suite passes locally against both:
  - `DEALDESKAI_VERSION=canary`
  - `DEALDESKAI_VERSION=latest`
...
1. A published `dealdesk@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `dealdesk@latest` Docker install can be smoke-tested by Playwright in CI.
