...
Agents need API keys to authenticate with DealDesk. The current approach
...
**Trust model:** The adapter process runs on the same machine as the DealDesk
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** DealDesk generates a token and passes it directly to the agent
...
`DEALDESK_API_URL`, `DEALDESK_AGENT_ID`, `DEALDESK_COMPANY_ID`. We need to
add a `DEALDESK_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a DealDesk CLI
...
managed by the DealDesk server itself.
...
1. A company admin (user or agent) generates an **invite URL** from DealDesk.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The DealDesk SKILL.md (or a link to it)
   - What information DealDesk needs from the agent (e.g. webhook URL, adapter
...
5. DealDesk stores the pending registration.
...
7. On approval, DealDesk provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** DealDesk issues an API key (or JWT) upon approval, delivered
...
1. Generate a DealDesk invite link for the company.
...
4. A DealDesk company member approves the new agent.
5. DealDesk begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `DEALDESK_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `dealdesk auth login` for developer-managed remote agents.                                      |
