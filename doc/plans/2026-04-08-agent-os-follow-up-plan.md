...
Turn the `agent-os` research into a low-risk DealDesk execution plan that preserves DealDesk's control-plane model while testing the few runtime ideas that appear worth adopting.

## Decision summary

DealDesk should not absorb `agent-os` as a product model or orchestration layer.

DealDesk should evaluate `agent-os` in three narrow areas:
...
This work is successful when DealDesk has:
...
- a proposed runtime capability model that fits current DealDesk adapters
...
- replace DealDesk heartbeats, issues, comments, approvals, or budgets with `agent-os` primitives
- introduce Rust/sidecar requirements for all local execution paths
- migrate all adapters at once
- add runtime workflow/queue abstractions to DealDesk core

## Existing DealDesk integration points
...
- prove that DealDesk can drive one local agent through an `agent-os` runtime without breaking heartbeat semantics
...
- keep DealDesk's existing heartbeat, issue, workspace, and comment flow authoritative
...
- introduce a DealDesk-native capability vocabulary without coupling the product to `agent-os`
...
- determine whether a layered/snapshotted root model improves some DealDesk workloads
...
- identify where DealDesk should prefer explicit typed tools over ambient shell access
...
- one concrete proposal for typed-tool adoption in DealDesk
...
- runtime experimentation could blur the boundary between DealDesk as control plane and DealDesk as execution platform
