...
Add a single storage subsystem for DealDesk that supports:
...
- API serves attachment bytes through authenticated DealDesk endpoints.
...
- Default local path is under instance root: `~/.dealdesk/instances/<instanceId>/data/storage`.
...
- `dealdesk onboard` writes a valid `storage` config block by default.
- `dealdesk configure --section storage` can switch between local and s3 modes.
...
- `dealdesk doctor` reports actionable storage status.
