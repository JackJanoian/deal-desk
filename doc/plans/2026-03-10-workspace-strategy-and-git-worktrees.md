...
`PAP-447` asks how DealDesk should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same DealDesk instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into DealDesk:

- some operators use DealDesk to manage DealDesk and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as DealDesk
- Claude and Codex expose different built-in affordances, so DealDesk should not overfit to one tool

## Core Product Decision

DealDesk should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by DealDesk
...
- DealDesk should not assume that every adapter can see or use a host filesystem path directly
...
If DealDesk makes worktrees agent-first, it will blur:
...
- DealDesk core resolves and records execution workspace state
...
That gives DealDesk room to support:
...
- DealDesk handles workspace runtime services under the hood using project/workspace policy
...
Once DealDesk is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So DealDesk should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that DealDesk cannot infer
...
- when issue moves to done, should DealDesk auto-commit?
- should it auto-push?
...
DealDesk already has the right foundation for a project-first model.
...
For server-spawned local adapters, DealDesk already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that DealDesk should mirror directly

Implication:

- for `codex_local`, DealDesk should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but DealDesk should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, DealDesk should separate:
...
DealDesk should be able to express intentions such as:
...
These are not specific to DealDesk. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".dealdesk/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a DealDesk-specific `devServer` flag.
...
DealDesk should distinguish between:
...
DealDesk should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, DealDesk should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to DealDesk
...
For runtime services specifically, DealDesk should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as DealDesk-only dev-server flags
...
3. Inject resolved execution workspace details into `context.dealDeskWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / DealDesk instance can be reused through env/config injection when appropriate
...
- DealDesk can decide whether to reuse or start a fresh service deterministically
...
3. Test against DealDesk-on-DealDesk workflows first.
...
7. Workspace runtime services are modeled generically, not as DealDesk-specific dev-server toggles.
