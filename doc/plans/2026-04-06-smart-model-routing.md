...
This document defines a V1 plan for "smart model routing" in DealDesk.
...
- preserve DealDesk's existing task, session, and audit invariants
...
That separation is a better fit for DealDesk than copying Hermes' exact keyword heuristic.

## 3. Current DealDesk State

DealDesk already has the right execution shape for adapter-specific routing, but it currently assumes one model per heartbeat run.

Current implementation facts:

- `server/src/services/heartbeat.ts` builds rich run context, including `dealDeskWake`, workspace metadata, and session handoff context
...
DealDesk should implement smart model routing as an adapter-local, opt-in execution pattern.

V1 decision:

1. Do not add a global server-side router that tries to understand every adapter.
2. Do not copy Hermes' prompt-keyword classifier as DealDesk's default routing policy.
...
- DealDesk heartbeats are structured, issue-scoped, and already include wake metadata
...
- the adapter already injects DealDesk skills and workspace metadata cleanly
...
This matters because DealDesk's board UI is supposed to make cost and behavior legible.

## 9. Why Not Copy Hermes Exactly

Hermes' cheap-route heuristic is useful precedent, but DealDesk should not start there.

Reasons:

- Hermes is optimizing free-form conversational turns
- DealDesk agents run structured, issue-scoped heartbeats with explicit task and workspace context
- DealDesk already knows whether a run is fresh vs resumed, issue-scoped vs approval follow-up, and what workspace/session exists
- those execution facts are stronger routing signals than prompt keyword matching

If DealDesk later wants a cheap-only completion path for trivial runs, that can be a second-stage feature built on observed run data, not the first implementation.
...
DealDesk should ship smart model routing as:
