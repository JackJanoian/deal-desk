...
DealDesk already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model DealDesk should implement next.

## Product Goals

DealDesk should let operators:
...
For the next implementation pass, DealDesk should enforce these defaults:
...
DealDesk should separate subscription-included usage from billed spend:
...
DealDesk should have two threshold classes:
...
DealDesk must know that a project is budget-paused and enforce it.
...
DealDesk must also block execution before new work starts.
