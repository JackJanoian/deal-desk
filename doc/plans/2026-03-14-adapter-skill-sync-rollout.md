...
This document defines the rollout plan for adapter-wide skill support in DealDesk.
...
- DealDesk stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

DealDesk currently has these adapters:
...
1. Desired skills live in DealDesk for every adapter.
...
These adapters have a stable local skills directory that DealDesk can read and manage.
...
These adapters do not have a meaningful DealDesk-owned persistent install state.
...
- desired skills stored in DealDesk
...
- runtime already injects DealDesk skills into `~/.cursor/skills`
...
- runtime already injects DealDesk skills into `~/.gemini/skills`
...
- runtime already injects DealDesk skills into `~/.pi/agent/skills`
...
- DealDesk can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects DealDesk skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- DealDesk may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed DealDesk skills that are clearly marked as DealDesk-managed
...
- DealDesk stores desired skills only
...
- avoid destructive wording unless DealDesk can prove a skill is DealDesk-managed
...
That gets DealDesk from “skills work for Codex and Claude” to “skills work for the whole local-adapter family,” which is the meaningful V1 milestone.
