...
This document defines the product and UI plan for skill management in DealDesk.
...
- DealDesk company import/export can include skills as package content
...
3. The UI must always tell the truth about what DealDesk knows:
   - desired state in DealDesk
...
DealDesk should treat skills at two scopes:
...
- created locally inside DealDesk later
...
7. “Understand what DealDesk knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a DealDesk-specific package layout
...
This matters because Codex and similar adapters may already have local skills that DealDesk did not install.

These should be clearly marked:

- external
- not managed by DealDesk
...
- `desired_only`: DealDesk wants it, adapter does not show it yet
- `external`: adapter has it but DealDesk does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, DealDesk only tracks desired state
...
- DealDesk can still track desired skills
...
- DealDesk can see adapter skills
...
- `.paperclip.yaml` may add DealDesk-specific fidelity, but should not replace the base shortname association model
...
- `.paperclip.yaml` may add DealDesk-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
DealDesk should have a company-scoped skill model or managed package model representing:
...
DealDesk should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too DealDesk-specific.
...
That gives DealDesk one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
