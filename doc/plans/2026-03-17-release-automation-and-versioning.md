...
DealDesk's current release flow is documented in `doc/RELEASING.md` and implemented through:
...
Move DealDesk to semver-compatible calendar versioning, auto-publish canaries from `master`, promote stable from a chosen tested commit, and use npm trusted publishing plus GitHub environments so no long-lived npm or LLM token needs to live in Actions.

## Core Decisions

### 1. Use calendar versions, but keep semver syntax

The repo and npm tooling still assume semver-shaped version strings in many places. That does not mean DealDesk must keep semver as a product policy. It does mean the version format should remain semver-valid.
...
That tradeoff is probably acceptable for DealDesk, but it should be explicit:
...
This is especially relevant for public library packages like `@dealdesk/shared`, `@dealdesk/db`, and the adapter packages.
...
- you can move `latest` to `dealdesk@1.2.3`
- you cannot turn `dealdesk@2026.317.0-canary.8` into `dealdesk@2026.317.0`
...
DealDesk's release problem is now "publish the whole fixed package set at one explicit version", not "derive the next semantic bump from human intent".
...
DealDesk should adopt this model:
