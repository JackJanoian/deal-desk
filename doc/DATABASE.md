...
DealDesk uses PostgreSQL via [Drizzle ORM](https://orm.drizzle.team/). There are three ways to run the database, from simplest to most production-ready.
...
Data persists across restarts in `~/.dealdesk/instances/default/db/`. To reset local dev data, delete that directory.
...
When `DATABASE_URL` is unset, this command targets the current embedded PostgreSQL instance for your active DealDesk config/instance.
...
If you later run the app with a pooled runtime URL, set `DATABASE_MIGRATION_URL` to the direct connection URL. DealDesk uses it for startup schema checks/migrations and plugin namespace migrations, while the app continues to use `DATABASE_URL` for runtime queries:
...
If your hosted database requires transaction-pooling-only connections, use a direct or session-pooled connection for DealDesk until runtime pooling support is documented in this guide. Do not edit database client source files as part of deployment setup.
...
| `DATABASE_URL` | Mode |
|---|---|
| Not set | Embedded PostgreSQL (`~/.dealdesk/instances/default/db/`) |
...
DealDesk supports automatic and manual logical database backups. These dumps include
...
`dealdesk db:backup` / `pnpm db:backup` commands and backup retention
configuration.
...
DealDesk stores secret metadata and versions in:
...
- Default key file: `~/.dealdesk/instances/default/secrets/master.key` (auto-created if missing).
- CLI config location: `~/.dealdesk/instances/default/config.json` under `secrets.localEncrypted.keyFilePath`.
...
- `DEALDESK_SECRETS_MASTER_KEY` (32-byte key as base64, hex, or raw 32-char string)
