...
Low-level reference for how DealDesk packages are prepared and published to npm.
...
DealDesk no longer uses release branches or Changesets for publishing.

## Why the CLI needs special packaging

The CLI package, `dealdesk`, imports code from workspace packages such as:
...
### Manual first publish for `@dealdesk/ui`
...
npm view @dealdesk/ui version

# make sure the dist payload is fresh
pnpm --filter @dealdesk/ui build
...
- If `npm view @dealdesk/ui version` already returns the same version that is in [`ui/package.json`](../ui/package.json), do not republish. Bump the version or use the normal repo-wide release flow in [`scripts/release.sh`](../scripts/release.sh).
...
- For an organization-scoped package like `@dealdesk/ui`, the `dealdesk` npm organization must exist and the publisher must be a member with permission to publish to that scope.
...
DealDesk uses calendar versions:
...
- every published internal `@dealdesk/*` dependency referenced by that manifest exists on npm
...
DealDesk does not auto-publish every non-private workspace package anymore.
...
Example for `@dealdesk/adapter-acpx-local` from the repo root:

```bash
# safe preview
pnpm run release:bootstrap-package -- @dealdesk/adapter-acpx-local

# one-time first publish from an authenticated maintainer machine
pnpm run release:bootstrap-package -- @dealdesk/adapter-acpx-local --publish --otp 123456
```
