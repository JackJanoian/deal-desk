...
Operational contract for the hosted `aws_secrets_manager` secret provider used by DealDesk Cloud.

## Scope

- Hosted provider for DealDesk-managed secrets when DealDesk Cloud runs on AWS.
- Source of truth for secret values is AWS Secrets Manager, not Postgres.
- DealDesk stores only metadata needed for ownership, bindings, version selection, audit, and runtime resolution.
- AWS provider bootstrap credentials are deployment/runtime credentials, not DealDesk-managed company secrets.
- Remote import for existing AWS secrets is metadata-only. Preview/import uses
  AWS inventory metadata and creates DealDesk external references; it does not
  copy plaintext into DealDesk.
...
The AWS provider has a chicken-and-egg boundary: DealDesk cannot use
`company_secrets` to unlock the AWS provider that stores those secrets. The
initial AWS trust must exist before the DealDesk server starts.

Allowed bootstrap locations:

- Infrastructure IAM or workload identity attached to the DealDesk server
  runtime.
- Process environment or orchestrator secret store used to start the DealDesk
  server.
...
keys into the DealDesk board UI. Do not store those bootstrap keys in
`company_secrets`.

## DealDesk Cloud Bootstrap

DealDesk Cloud must provision the AWS backing resources before any board user
can create AWS-backed company secrets:

1. Create or select the deployment KMS key.
2. Create the DealDesk server runtime role for the deployment.
...
4. Configure the server runtime with the non-secret provider environment
   variables below.
5. Run `dealdesk doctor` or the provider health endpoint from the deployed
...
Once this is in place, the board UI can create DealDesk-managed AWS secrets and
DealDesk will write them under the deployment/company namespace.
...
DEALDESK_SECRETS_AWS_KMS_KEY_ID=arn:aws:kms:us-east-1:123456789012:key/abcd-... \
pnpm dev
```
...
should not be written to DealDesk config, committed to `.env` files, stored in
`company_secrets`, or used as the default DealDesk Cloud bootstrap path.
...
DEALDESK_SECRETS_PROVIDER=aws_secrets_manager
DEALDESK_SECRETS_AWS_REGION=us-east-1
DEALDESK_SECRETS_AWS_DEPLOYMENT_ID=prod-us-1
DEALDESK_SECRETS_AWS_KMS_KEY_ID=arn:aws:kms:us-east-1:123456789012:key/abcd-...
```
...
- One DealDesk app role per deployment.
- One deployment-scoped KMS key per deployment at launch.
- Future per-company KMS keys remain compatible because DealDesk stores provider refs and version metadata separately from values.
...
- Prefer workload identity / role-based auth. Do not store AWS credentials inline in DealDesk config.
...
- DealDesk-managed secrets may be deleted only by DealDesk or an operator with equivalent break-glass access.
- External references may resolve through DealDesk runtime, but DealDesk should not delete the external secret resource.
...
```json
{
  "Sid": "DealDeskRemoteSecretInventory",
  "Effect": "Allow",
...
operator-approved external prefixes that DealDesk is allowed to consume:

```json
{
  "Sid": "DealDeskResolveImportedExternalReferences",
  "Effect": "Allow",
...
- Prefer one DealDesk runtime role per environment/account.
...
DealDesk-managed resources.

Use the DealDesk-managed flow when DealDesk should create and rotate the value. The AWS
...
```text
/dealdesk-bench/anthropic_api_key
```

In that mode DealDesk stores only the path or ARN, resolves it at runtime, and records
redacted access events. Operators rotate the actual value in AWS. Update the DealDesk
reference only when the AWS path, ARN, or pinned provider version changes.

DealDesk does not currently offer an "adopt existing AWS secret" flow that takes over future
`PutSecretValue` writes for an arbitrary existing secret. Adding that later requires explicit
confirmation UX, scope validation, expected DealDesk tags, and security/cloud-ops review.

## Data Custody

- DealDesk stores `externalRef`, `providerVersionRef`, provider id, fingerprint hash, status, and binding metadata.
- DealDesk does not store AWS secret plaintext in `company_secret_versions.material`.
- Runtime resolution fetches the value from AWS only when a bound consumer needs it.

## Rotation Runbook

Manual DealDesk-managed rotation:

1. Write the new value through the DealDesk secret rotate flow.
2. DealDesk creates a new AWS secret version with `PutSecretValue`.
3. DealDesk records the new `providerVersionRef` in `company_secret_versions`.
4. Re-run or restart affected workloads that consume `latest`, or pin consumers to a specific DealDesk version before rollout when you need staged release safety.

Guidance:

- Prefer pinned DealDesk secret versions for risky rollouts.
...
- DealDesk database metadata for secret ownership, bindings, status, and provider version refs.
- AWS Secrets Manager namespace under the configured deployment prefix.
- The configured KMS key and its decrypt permissions.

Restore checklist:

1. Restore DealDesk database metadata.
2. Confirm the same AWS Secrets Manager namespace still exists.
3. Confirm the DealDesk runtime role can call `GetSecretValue` on the restored prefix.
4. Confirm the role still has decrypt access to the CMK referenced by `DEALDESK_SECRETS_AWS_KMS_KEY_ID`.
...
1. Stop or pause affected DealDesk runs.
2. Audit recent DealDesk secret access events for impacted secret ids and consumers.
...
4. Rotate impacted secrets in AWS through DealDesk-managed versioning.
...
- `DEALDESK_SECRETS_PROVIDER=aws_secrets_manager`
- The required `DEALDESK_SECRETS_AWS_*` environment variables set.
