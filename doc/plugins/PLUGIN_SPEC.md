# DealDesk Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for DealDesk's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main DealDesk app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary DealDesk HTTP APIs directly.
...
- The current runtime ships a small host-provided plugin UI component kit through `@dealdesk/plugin-sdk/ui`, but does not support plugin asset uploads/reads yet. Treat plugin asset APIs as future-scope ideas, not current implementation promises.
...
DealDesk plugin design is based on the following assumptions:

1. DealDesk is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core DealDesk business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by DealDesk core.
...
2. Let plugins add major capabilities without editing DealDesk core.
...
4. Depend on project-local plugin folders such as `.dealdesk/plugins`.
...
The single DealDesk deployment an operator installs and controls.

### 5.2 Company

A first-class DealDesk business object inside the instance.
...
A trusted in-process extension loaded directly by DealDesk core.
...
An installable instance-wide extension package loaded through the DealDesk plugin runtime.
...
DealDesk has two extension classes.
...
DealDesk already has a concrete workspace model:
...
Plugins live under the DealDesk instance directory.

Suggested layout:

- `~/.dealdesk/instances/default/plugins/package.json`
- `~/.dealdesk/instances/default/plugins/node_modules/`
- `~/.dealdesk/instances/default/plugins/.cache/`
- `~/.dealdesk/instances/default/data/plugins/<plugin-id>/`
...
DealDesk should add CLI commands:

- `pnpm dealdesk plugin list`
- `pnpm dealdesk plugin install <package[@version]>`
- `pnpm dealdesk plugin uninstall <plugin-id>`
- `pnpm dealdesk plugin upgrade <plugin-id> [version]`
- `pnpm dealdesk plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@dealdesk/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface DealDeskPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
Plugins may contribute tools that DealDesk agents can use during runs.
...
- DealDesk server starts one worker process per installed plugin
...
Receives one typed DealDesk domain event.
...
export function definePlugin(definition: PluginDefinition): DealDeskPlugin;
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@dealdesk/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@dealdesk/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@dealdesk/plugin-sdk/ui`)
...
DealDesk-internal pages. It exposes `resolveHref(to)`, `navigate(to,
...
DealDesk navigation because those can force a full document reload.
...
- Plugin bundles must not import from host internals. They may only import from `@dealdesk/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@dealdesk/plugin-sdk/ui`
...
The `@dealdesk/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the DealDesk secret provider system rather than a plain text input
...
1. Core DealDesk data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing DealDesk objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual DealDesk product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if DealDesk later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing DealDesk secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm dealdesk plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the DealDesk server. This is a normative requirement, not optional.
...
### 27.1 `@dealdesk/plugin-test-harness`
...
```ts
import { createTestHarness } from "@dealdesk/plugin-test-harness";
...
For developing a plugin against a running DealDesk instance:

- The operator installs the plugin from a local path: `pnpm dealdesk plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-dealdesk-plugin`) that scaffolds:

- `package.json` with correct `dealDeskPlugin` keys
...
- `@dealdesk/plugin-workspace-files`
- `@dealdesk/plugin-terminal`
- `@dealdesk/plugin-git`
- `@dealdesk/plugin-linear`
- `@dealdesk/plugin-github-issues`
- `@dealdesk/plugin-grafana`
- `@dealdesk/plugin-runtime-processes`
- `@dealdesk/plugin-stripe`
...
- `@dealdesk/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@dealdesk/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@dealdesk/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@dealdesk/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@dealdesk/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@dealdesk/plugin-sdk/ui`
...
- `@dealdesk/plugin-test-harness`
- `create-dealdesk-plugin` starter template
...
DealDesk should not implement a generic in-process hook bag modeled directly after local coding tools.

DealDesk should implement:
...
That is the complete target design for the DealDesk plugin system.
