...
DealDesk V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a DealDesk-shaped design.

Assumption for this document: DealDesk is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class DealDesk objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into DealDesk.

The main conclusion is:

- DealDesk should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- DealDesk should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- DealDesk should use multiple extension classes instead of one generic plugin bag:
...
If DealDesk does this well, the examples you listed become straightforward:
...
Relevant DealDesk files reviewed for current extension seams:
...
It is too dangerous for DealDesk core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for DealDesk — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
DealDesk should absolutely do this.
...
DealDesk should copy this discipline.
...
DealDesk should adopt this style for plugin actions, automations, and UI schemas.
...
DealDesk can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for DealDesk too.

## What DealDesk Should Not Copy Directly
...
DealDesk is a control plane for an operator-managed instance with company objects.
...
DealDesk is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

DealDesk should not auto-load arbitrary code from a workspace repo like `.dealdesk/plugins` or project directories.
...
For DealDesk, equivalent hooks into:
...
DealDesk should not allow plugins to silently replace:
...
For DealDesk it would be risky because it combines:
...
DealDesk should require an explicit operator install step.

## Why DealDesk Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | DealDesk |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted DealDesk instance |
...
That means DealDesk should borrow the good ideas from `opencode` but use a stricter architecture.

## DealDesk Already Has Useful Pre-Plugin Seams

DealDesk has several extension-like seams already:
...
This is good news.
DealDesk does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended DealDesk Plugin Model
...
DealDesk already has this pattern implicitly:
...
- `plugin` means integration code that talks to DealDesk through a typed plugin protocol and capability model
...
- invoke explicit DealDesk actions through the API
...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@dealdesk/plugin-sdk/ui`) with design tokens and common components.
...
DealDesk should treat plugin installation as a global instance-level action.

Examples:

- install `@dealdesk/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over DealDesk objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

DealDesk already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for DealDesk too.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@dealdesk/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `@dealdesk/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running DealDesk instance.

A `create-dealdesk-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how DealDesk achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the DealDesk server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. DealDesk's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.

Recommended approach:

- **Single SDK package**: `@dealdesk/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@dealdesk/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
## A Concrete SDK Shape For DealDesk

An intentionally narrow first pass could look like this:

```ts
import { definePlugin, z } from "@dealdesk/plugin-sdk";

export default definePlugin({
  id: "@dealdesk/plugin-linear",
  version: "0.1.0",
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit DealDesk entities
    });
    ...
    ctx.events.on("plugin.@dealdesk/plugin-git.push-detected", async (event) => {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@dealdesk/plugin-sdk/ui";
...
Any DealDesk plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific DealDesk objects, those are plugin data, not enable/disable boundaries.
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@dealdesk/plugin-linear`)
...
- reuse core tables when the data is clearly part of DealDesk itself
...
If a concept is becoming part of DealDesk's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of DealDesk itself
...
### 3. Scope plugin data to DealDesk objects before adding custom schemas

A lot of plugin data naturally hangs off existing DealDesk objects:
...
If DealDesk eventually needs extension-owned tables, I would only allow that for:
...
Package idea: `@dealdesk/plugin-workspace-files`
...
Package idea: `@dealdesk/plugin-terminal`
...
Package idea: `@dealdesk/plugin-git`
...
- Board opens a project page and reviews the diff for that project's workspace without leaving DealDesk.
- Board reviews the diff after a run without leaving DealDesk.
...
- optional `events.emit` (e.g. `plugin.@dealdesk/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@dealdesk/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@dealdesk/plugin-linear`

This plugin syncs DealDesk work with Linear. It is useful for:

- importing backlog from Linear
- linking DealDesk issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between DealDesk and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into DealDesk.
- DealDesk issue status changes push to Linear and Linear comments arrive back through webhooks.
...
Package idea: `@dealdesk/plugin-github-issues`

This plugin syncs DealDesk issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to DealDesk issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with DealDesk task governance
...
- Plugin settings:
  ...
  - whether new DealDesk issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active DealDesk issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into DealDesk.
- GitHub webhooks update status/comment state in DealDesk.
- A PR is linked back to the DealDesk issue so the board can follow delivery status.
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@dealdesk/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@dealdesk/plugin-grafana`

This plugin surfaces external metrics and dashboards inside DealDesk. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating DealDesk issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full DealDesk plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the DealDesk dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a DealDesk issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of DealDesk core; keep it as additive signal and issue creation

## Child Process / Server Tracking

Package idea: `@dealdesk/plugin-runtime-processes`
...
Package idea: `@dealdesk/plugin-stripe`

This plugin pulls Stripe revenue and subscription data into DealDesk. It is useful for:
...
- Failed payment spikes or churn events can generate DealDesk issues for follow-up.
...
- Stripe data should stay additive to DealDesk core
...
- plugin UI bundle loading, host bridge, `@dealdesk/plugin-sdk/ui`
...
- `@dealdesk/plugin-test-harness` and `create-dealdesk-plugin` starter template
...
DealDesk should not implement "an OpenCode-style generic in-process hook system."
DealDesk should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In DealDesk
...
8. Ship `@dealdesk/plugin-test-harness` and `create-dealdesk-plugin` starter template.
