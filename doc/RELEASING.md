...
Maintainer runbook for shipping DealDesk across npm, GitHub, and the website-facing changelog surface.
...
DealDesk uses calendar versions that still fit semver syntax:
...
2. **npm** — `dealdesk` and public workspace packages are published
...
claude --print --output-format stream-json --verbose --dangerously-skip-permissions --model claude-opus-4-6 "Use the release-changelog skill to draft or update releases/v${VERSION}.md for DealDesk. Read doc/RELEASING.md and .agents/skills/release-changelog/SKILL.md, then generate the stable changelog for v${VERSION} from commits since the last stable tag. Do not create a canary changelog."
```
...
```bash
DEALDESKAI_VERSION=canary ./scripts/docker-onboard-smoke.sh
```

For the current stable:

```bash
DEALDESKAI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```

Useful isolated variants:

```bash
HOST_PORT=3232 DATA_DIR=./data/release-smoke-canary DEALDESKAI_VERSION=canary ./scripts/docker-onboard-smoke.sh
HOST_PORT=3233 DATA_DIR=./data/release-smoke-stable DEALDESKAI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```

Automated browser smoke is also available:

```bash
gh workflow run release-smoke.yml -f dealdesk_version=canary
gh workflow run release-smoke.yml -f dealdesk_version=latest
```

Minimum checks:

- `npx dealdesk@canary onboard` installs
