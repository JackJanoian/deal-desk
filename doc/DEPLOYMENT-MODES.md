...
DealDesk supports two runtime modes:
...
DealDesk now treats **bind** as a separate concern from auth:
...
pnpm dealdesk onboard --yes --bind lan
pnpm dealdesk run --bind tailnet
```
...
When running `authenticated` mode, if the only instance admin is `local-board`, DealDesk emits a startup warning with a one-time high-entropy claim URL.
