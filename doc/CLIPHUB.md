...
ClipHub is the public registry where people share, discover, and download DealDesk company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to DealDesk what a package registry is to a programming language. DealDesk already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in DealDesk — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own DealDesk instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the DealDesk spec. It contains:
...
ClipHub is a **separate service** from DealDesk itself. DealDesk is self-hosted; ClipHub is a hosted registry that DealDesk instances talk to.
...
| **DealDesk CLI** | `dealdesk install`, `dealdesk publish`, `dealdesk cliphub sync` — built into DealDesk |
| **DealDesk UI** | "Browse ClipHub" panel in the DealDesk web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with DealDesk) |
| Backend | TypeScript + Hono (consistent with DealDesk) |
| Database | PostgreSQL |
...
4. Run `dealdesk install cliphub:acme/lean-saas-shop`
5. DealDesk creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in DealDesk until it works well
2. Export: `dealdesk export --template my-agency`
3. Publish: `dealdesk publish cliphub my-agency`
...
3. Install just that agent: `dealdesk install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to DealDesk

ClipHub is **not required** to use DealDesk. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to DealDesk what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `dealdesk install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in DealDesk web UI
- [ ] `dealdesk cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with DealDesk's own philosophy)
