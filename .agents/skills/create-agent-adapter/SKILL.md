...
description: >
  Technical guide for creating a new DealDesk agent adapter. Use when building
...
# Creating a DealDesk Agent Adapter

An adapter bridges DealDesk's orchestration layer to a specific AI agent runtime (Claude Code, Codex CLI, a custom process, an HTTP endpoint, etc.). Each adapter is a self-contained package that provides implementations for **three consumers**: the server, the UI, and the CLI.
...
packages/adapters/<name>/
  src/
    ...
    cli/
      ...
      format-event.ts   # Colored terminal output for `dealdesk run --watch`
  package.json
  tsconfig.json
```
...
## 2. Shared Types (`@dealdesk/adapter-utils`)

All adapter interfaces live in `packages/adapter-utils/src/types.ts`. Import from `@dealdesk/adapter-utils` (types) or `@dealdesk/adapter-utils/server-utils` (runtime helpers).
...
interface AdapterExecutionResult {
  ...
  clearSession?: boolean;         // true = tell DealDesk to forget the stored session
}
...
```json
{
  ...
  "dependencies": {
    "@dealdesk/adapter-utils": "workspace:*",
    "picocolors": "^1.1.1"
  },
  "devDependencies": {
    "typescript": "^5.7.3"
...
The `agentConfigurationDoc` is read by LLM agents (including DealDesk agents that create other agents). Write it as **routing logic**, not marketing copy. Include concrete "use when" and "don't use when" guidance so an LLM can decide whether this adapter is appropriate for a given task.
...
1. **Read config** — extract typed values from `ctx.config` using helpers (`asString`, `asNumber`, `asBoolean`, `asStringArray`, `parseObject` from `@dealdesk/adapter-utils/server-utils`)
2. **Build environment** — call `buildDealDeskEnv(agent)` then layer in `DEALDESK_RUN_ID`, context vars (`DEALDESK_TASK_ID`, `DEALDESK_WAKE_REASON`, `DEALDESK_WAKE_COMMENT_ID`, `DEALDESK_APPROVAL_ID`, `DEALDESK_APPROVAL_STATUS`, `DEALDESK_LINKED_ISSUE_IDS`), user env overrides, and auth token
...
| `DEALDESK_API_URL` | Server's own URL |
| `DEALDESK_RUN_ID` | Current run id |
| `DEALDESK_TASK_ID` | `context.taskId` or `context.issueId` |
| `DEALDESK_WAKE_REASON` | `context.wakeReason` |
| `DEALDESK_WAKE_COMMENT_ID` | `context.wakeCommentId` or `context.commentId` |
| `DEALDESK_APPROVAL_ID` | `context.approvalId` |
| `DEALDESK_APPROVAL_STATUS` | `context.approvalStatus` |
| `DEALDESK_LINKED_ISSUE_IDS` | `context.issueIds` (comma-separated) |
| `DEALDESK_API_KEY` | `authToken` (if no explicit key in config) |
...
Pretty-prints stdout lines for `dealdesk run --watch`. Use `picocolors` for coloring.
...
```ts
import { execute as myExecute, sessionCodec as mySessionCodec } from "@dealdesk/adapter-my-agent/server";
import { agentConfigurationDoc as myDoc, models as myModels } from "@dealdesk/adapter-my-agent";
...
const myAgentAdapter: ServerAdapterModule = {
  ...
  models: myModels,
  supportsLocalAgentJwt: true,  // true if agent can use DealDesk API
...
import { parseMyAgentStdoutLine } from "@dealdesk/adapter-my-agent/ui";
import { MyAgentConfigFields } from "./config-fields";
import { buildMyAgentConfig } from "@dealdesk/adapter-my-agent/ui";
...
```ts
import { printMyAgentStreamEvent } from "@dealdesk/adapter-my-agent/cli";
...
- **Unknown session retry**: if resume fails with a "session not found" error, retry with a fresh session and return `clearSession: true` so DealDesk wipes the stale session
...
Import from `@dealdesk/adapter-utils/server-utils`:
...
| `buildDealDeskEnv(agent)` | Standard `DEALDESK_*` env vars |
...
- Package name: `@dealdesk/adapter-<kebab-name>`
...
- Default prompt should use `DEFAULT_DEALDESK_AGENT_PROMPT_TEMPLATE` from `@dealdesk/adapter-utils/server-utils` so local adapters share DealDesk's execution contract: act in the same heartbeat, avoid planning-only exits unless requested, leave durable progress and a next action, use child issues instead of polling, mark blockers with owner/action, and respect governance boundaries.
...
### DealDesk Skills Injection

DealDesk ships shared skills (in the repo's top-level `skills/` directory) that agents need at runtime — things like the `dealdesk` API skill and the `dealdesk-create-agent` workflow skill. Each adapter is responsible for making these skills discoverable by its agent runtime **without polluting the agent's working directory**.

**The constraint:** never copy or symlink skills into the agent's `cwd`. The cwd is the user's project checkout — writing `.claude/skills/` or any other files into it would contaminate the repo with DealDesk internals, break git status, and potentially leak into commits.

**The pattern:** create a clean, isolated location for skills and tell the agent runtime to look there.

**How claude-local does it:**

1. At execution time, create a fresh tmpdir: `mkdtemp("dealdesk-skills-")`
...
async function buildSkillsDir(): Promise<string> {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "dealdesk-skills-"));
  const target = path.join(tmp, ".claude", "skills");
  await fs.mkdir(target, { recursive: true });
  const entries = await fs.readdir(DEALDESK_SKILLS_DIR, { withFileTypes: true });
  for (const entry of entries) {
    if (entry.isDirectory()) {
      await fs.symlink(
        path.join(DEALDESK_SKILLS_DIR, entry.name),
...
Codex has a global personal skills directory (`$CODEX_HOME/skills` or `~/.codex/skills`). The adapter symlinks DealDesk skills there if they don't already exist. This is acceptable because it's the agent tool's own config directory, not the user's project.
...
**Skills as loaded procedures, not prompt bloat.** The DealDesk skills (like `dealdesk` and `dealdesk-create-agent`) are designed as on-demand procedures: the agent sees skill metadata (name + description) in its context, but only loads the full SKILL.md content when it decides to invoke a skill. This keeps the base prompt small. When writing `agentConfigurationDoc` or prompt templates for your adapter, do not inline skill content — let the agent runtime's skill discovery do the work. The descriptions in each SKILL.md frontmatter act as routing logic: they tell the agent when to load the full skill, not what the skill contains.

**Explicit vs. fuzzy skill invocation.** For production workflows where reliability matters (e.g. an agent that must always call the DealDesk API to report status), use explicit instructions in the prompt template: "Use the dealdesk skill to report your progress." Fuzzy routing (letting the model decide based on description matching) is fine for exploratory tasks but unreliable for mandatory procedures.

---

## 8. Security Considerations

Adapters sit at the boundary between DealDesk's orchestration layer and arbitrary agent execution. This is a high-risk surface.
...
- `DEALDESK_API_KEY` is injected by the server into the process environment, not the prompt
...
- Prefer minimal allowlists over open internet access. An agent that only needs to call the DealDesk API and GitHub should not have access to arbitrary hosts.
