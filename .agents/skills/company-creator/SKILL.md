...
description: >
  ...
  is already running in DealDesk.
---
...
**Do not specify an adapter unless the repo or user context warrants it.** If you don't know what adapter the user wants, omit the adapter block entirely — DealDesk will use its default. Specifying an unknown adapter type causes an import error.

DealDesk's supported adapter types (these are the ONLY valid values):
