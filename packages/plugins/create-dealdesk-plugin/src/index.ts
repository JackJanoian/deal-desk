...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".dealdesk-sdk");
...
/**
 * Generate a complete DealDesk plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A DealDesk plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "dealdesk-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["dealdesk", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@dealdesk/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@dealdesk/plugin-sdk": sdkDependency,
...
`,
    ...
    writeFile(
      ...
      `import { usePluginData, type PluginWidgetProps } from "@dealdesk/plugin-sdk/ui";
...
} from "@dealdesk/plugin-sdk/testing";
...
`,
    ...
    writeFile(
      path.join(outputDir, "src", "ui", "index.tsx"),
      `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@dealdesk/plugin-sdk/ui";
...
When this package is installed from a local path, DealDesk watches that rebuilt
output and reloads the plugin worker. Local installs run trusted code from this
folder on your machine.

${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@dealdesk/plugin-sdk\` and \`@dealdesk/shared\` from a local DealDesk checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.dealdesk-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}

## Install Into DealDesk

\`\`\`bash
dealdesk plugin install ${shellQuote(toPosixPath(outputDir))}
\`\`\`
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-dealdesk-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <paperclip-sdk-path>]");
    process.exit(1);
  }
