...
Example DealDesk plugin that demonstrates:
...
```bash
pnpm --filter @dealdesk/plugin-file-browser-example build
pnpm dealdesk plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm dealdesk plugin uninstall dealdesk-file-browser-example --force
```
...
- Optional: use `dealdesk-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
