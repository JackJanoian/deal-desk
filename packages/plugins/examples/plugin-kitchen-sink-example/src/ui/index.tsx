...
function KitchenSinkIssueCrudDemo({ context }: { context: PluginPageProps["context"] }) {
  ...
  return (
    <Section title="Issue CRUD">
      <div style={mutedTextStyle}>
        This is a regular embedded React page inside DealDesk calling the board API directly. It creates, updates, and deletes issues for the current company.
      </div>
      {!context.companyId ? (
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "grid", gap: "14px" }}>
        ...
        <Section title="DealDesk Animation">
          <div style={mutedTextStyle}>
            This is the same DealDesk ASCII treatment used in onboarding, copied into the example plugin so the package stays self-contained.
          </div>
          <AsciiArtAnimation />
        </Section>
      </div>
...
function KitchenSinkHostIntegrationDemo({ context }: { context: PluginPageProps["context"] }) {
  ...
  return (
    <Section title="Host Integrations">
      <div style={mutedTextStyle}>
        Plugin pages can feel like native DealDesk pages. This section demonstrates host toasts, company-scoped routing, and reading live heartbeat data from the embedded page.
      </div>
      <div style={subtleCardStyle}>
        <div style={rowStyle}>
...
function KitchenSinkSharedPickerDemo({ context }: { context: PluginPageProps["context"] }) {
  ...
  return (
    <Section title="Shared Host Pickers">
      <div style={mutedTextStyle}>
        These controls are imported from `@dealdesk/plugin-sdk/ui` and reuse the host's assignee and project pickers from the new issue pane.
      </div>
      {!context.companyId ? (
...
function KitchenSinkConsole({ context }: { context: { companyId: string | null; companyPrefix?: string | null; projectId?: string | null; entityId?: string | null; entityType?: string | null } }) {
  ...
  return (
    <div style={{ display: "grid", gap: "14px" }}>
      ...
      <Section title="DealDesk Domain APIs">
    ...
    <form onSubmit={onSubmit} style={{ display: "grid", gap: "18px" }}>
      <div style={{ display: "grid", gap: "12px", gridTemplateColumns: "minmax(0, 1.8fr) minmax(220px, 1fr)" }}>
        <div style={{ display: "grid", gap: "8px" }}>
          <strong>About</strong>
          <div style={{ fontSize: "13px", lineHeight: 1.5 }}>
            Kitchen Sink demonstrates the current DealDesk plugin API surface in one local, trusted example. It intentionally includes domain mutations, event handling, streams, tools, jobs, webhooks, and local workspace/process demos.
          </div>
