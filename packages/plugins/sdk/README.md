...
Official TypeScript SDK for DealDesk plugin authors.

- **Worker SDK:** `@dealdesk/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@dealdesk/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@dealdesk/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@dealdesk/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@dealdesk/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@dealdesk/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@dealdesk/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, `useHostNavigation`, slot prop types |
| `@dealdesk/plugin-sdk/ui/hooks` | Hooks only |
| `@dealdesk/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@dealdesk/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@dealdesk/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@dealdesk/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@dealdesk/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@dealdesk/plugin-sdk/types` | Worker context and API types (advanced) |
...
- Plugin UI bundles run as same-origin JavaScript inside the main DealDesk app. They can call ordinary DealDesk HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the DealDesk instance at runtime.
...
- The host ships a small shared React component kit through `@dealdesk/plugin-sdk/ui`. Use it for native DealDesk controls; custom React and CSS are still supported.
...
```ts
import { definePlugin, runWorker } from "@dealdesk/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@dealdesk/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core DealDesk information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@dealdesk/plugin-sdk`.
...
const child = await ctx.issues.create({
  ...
  originKind: "plugin:dealdesk.missions:feature",
...
```tsx
import { usePluginData, usePluginAction } from "@dealdesk/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@dealdesk/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@dealdesk/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@dealdesk/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@dealdesk/plugin-sdk/ui";
...
Routes DealDesk-internal plugin links through the host router without a full document reload. Use `linkProps()` for anchors so the browser still gets a real `href` for copy-link, modifier-click, middle-click, and open-in-new-tab behavior.

```tsx
import { useHostNavigation } from "@dealdesk/plugin-sdk/ui";
...
Avoid raw same-origin `href`s or `window.location.assign()` for DealDesk-internal navigation from plugin UI. Those bypass the host router and can reload the whole app. External links should keep normal anchors with `target="_blank"` and `rel="noopener noreferrer"` as appropriate.
...
```tsx
import { usePluginStream } from "@dealdesk/plugin-sdk/ui";
...
The host provides selected shared UI components through `@dealdesk/plugin-sdk/ui`.
...
used by DealDesk issue comments and documents:

```tsx
import { MarkdownBlock, MarkdownEditor } from "@dealdesk/plugin-sdk/ui";
...
```tsx
import { FileTree, type FileTreeNode } from "@dealdesk/plugin-sdk/ui";
...
or configure work against DealDesk entities. Both are controlled components and
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@dealdesk/plugin-sdk/ui`.
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@dealdesk/plugin-sdk/ui";
...
```ts
import { createPluginBundlerPresets } from "@dealdesk/plugin-sdk/bundlers";
...
```bash
dealdesk-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@dealdesk/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__dealdesk__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__dealdesk__/events` streams `reload` SSE events on UI build changes
