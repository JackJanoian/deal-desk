/**
 * Core types for the DealDesk plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@dealdesk/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @dealdesk/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of DealDesk object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the DealDesk UI. */
  title?: string;
...
export interface PluginLocalFoldersClient {
  ...
  declarations(): import("@dealdesk/shared").PluginLocalFolderDeclaration[];
...
/**
 * `ctx.events` — subscribe to and emit DealDesk domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core DealDesk domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the DealDesk secret provider
...
export interface PluginSecretsClient {
   ...
   * in the DealDesk secret provider (e.g. `"MY_API_KEY"`).
   *
...
/**
 * `ctx.routines` — resolve and reconcile plugin-managed DealDesk routines.
 *
 ...
 * ```
 *
 * @see usePluginStream in `@dealdesk/plugin-sdk/ui`
 */
