...
function requireCapability(manifest: DealDeskPluginManifestV1, allowed: Set<PluginCapability>, capability: PluginCapability) {
 ...
 * can validate plugin behavior without spinning up the DealDesk server runtime.
 */
export function createTestHarness(options: TestHarnessOptions): TestHarness {
  ...
  function isManagedAgent(agent: Agent, agentKey: string) {
    const marker = agent.metadata?.dealdeskManagedResource;
    return Boolean(
      marker
