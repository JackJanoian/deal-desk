 ...
 * channel.
 *
 * The DealDesk plugin runtime uses JSON-RPC 2.0 over stdio to communicate
...
export type { PluginLauncherRenderContextSnapshot } from "@dealdesk/shared";
 ...
 * we use strings (UUIDs or monotonic counters) for all DealDesk messages.
 */
...
/**
 * DealDesk plugin-specific error codes.
 *
...
export interface InitializeParams {
  ...
  instanceInfo: {
    /** UUID of this DealDesk instance. */
    instanceId: string;
    /** Semver version of the running DealDesk host. */
    hostVersion: string;
  };
  /** Host API version. */
  apiVersion: number;
