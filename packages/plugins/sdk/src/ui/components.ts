 ...
 *
 * These components are exported from `@dealdesk/plugin-sdk/ui` and are
 ...
 *
 * @see PLUGIN_SPEC.md §19.6 — Shared Components In `@dealdesk/plugin-sdk/ui`
 */
...
function createSdkUiComponent<TProps>(name: string): React.ComponentType<TProps> {
  return function DealDeskSdkUiComponent(props: TProps) {
...
/**
 * Renders DealDesk's shared Markdown editor.
 *
 * @see PLUGIN_SPEC.md §19.6 — Shared Components
 */
...
/**
 * Renders DealDesk's native issue list component for company-scoped plugin
...
/**
 * Renders DealDesk's native managed routines list for plugin settings pages.
 */
