...
import type { Agent, AgentSessionEvent, Issue, IssueComment, PluginContext, PluginEvent, PluginLocalFolderEntry, Project, ToolResult } from "@dealdesk/plugin-sdk";
import type { IssueDocument, PluginIssueOriginKind, PluginManagedRoutineResolution, PluginManagedSkillResolution } from "@dealdesk/plugin-sdk/types";
import {
  DEFAULT_MAX_SOURCE_BYTES,
  DEFAULT_MAX_DEALDESK_CURSOR_WINDOW_CHARS,
  DEFAULT_MAX_DEALDESK_ISSUE_SOURCE_CHARS,
  DEFAULT_MAX_DEALDESK_ROUTINE_RUN_CHARS,
  DEFAULT_DEALDESK_COST_CENTS_PER_1K_CHARS,
  PLUGIN_ID,
...
const MAX_DEALDESK_INGESTION_PROFILE_SOURCE_COUNT = 3;
const MAX_DEALDESK_DISTILLATION_FAN_OUT = 25;
const MAX_DEALDESK_PROFILE_SELECTED_PROJECTS = 25;
const MAX_DEALDESK_PROFILE_ROOT_ISSUES = 25;
...
export type DealDeskDistillationScope = "company" | "project" | "root_issue";
export type DealDeskDistillationWorkItemKind = "manual" | "retry" | "backfill" | "priority_override" | "review_patch";
...
export type DealDeskIngestionSourceScope =
...
export type DealDeskIngestionProfileV1 = {
  version: 1;
  enabled: boolean;
  sourceScopes: DealDeskIngestionSourceScope[];
...
export type DealDeskIngestionProfileEffectiveState =
  | "enabled"
  | "disabled"
  | "policy_blocked"
  | "pending_approval"
...
export type DealDeskIngestionProfileRead = {
  wikiId: string;
  space: Pick<WikiSpace, "id" | "slug" | "displayName" | "accessScope" | "status">;
  profile: DealDeskIngestionProfileV1;
  effectiveState: DealDeskIngestionProfileEffectiveState;
  policyBlocks: string[];
...
type DealDeskIngestionPolicyPurpose =
  | "profile_read"
  | "profile_update"
  | "candidate_search"
  | "queue"
...
type DealDeskIngestionPolicyDecision =
...
type DealDeskIngestionCandidatesInput = SpaceInput & {
  query?: string | null;
};
...
type DealDeskSourceBundleInput = {
  companyId: string;
  wikiId?: string | null;
  spaceSlug?: string | null;
  projectId?: string | null;
...
type DealDeskSourceBundle = {
  markdown: string;
  sourceRefs: DealDeskSourceRef[];
  sourceHash: string;
...
type DealDeskDistillationRunInput = DealDeskSourceBundleInput;

type DealDeskDistillationOutcomeInput = {
  companyId: string;
  wikiId?: string | null;
  spaceSlug?: string | null;
  runId: string;
...
type DealDeskDistillationWorkItemInput = {
  companyId: string;
  wikiId?: string | null;
  spaceSlug?: string | null;
  kind: DealDeskDistillationWorkItemKind;
  projectId?: string | null;
  rootIssueId?: string | null;
...
type DealDeskProjectPageDistillationInput = DealDeskSourceBundleInput & {
  autoApply?: boolean;
...
type DealDeskDistillationPatchOperation =
  | "standup_update"
  ...
  | "index_refresh"
  | "log_append";
type DealDeskDistillationPatch = {
  pagePath: string;
  operationType: DealDeskDistillationPatchOperation;
...
type DealDeskEventIngestResult =
...
async function requireDealDeskIngestionPolicy(
  ctx: PluginContext,
  input: { companyId: string; wikiId: string; spaceSlug?: string | null },
  purpose: DealDeskIngestionPolicyPurpose,
  ...
  const decision = evaluateDealDeskProfilePolicy({
    space,
    profile,
    purpose,
...
function assertDealDeskSourceScopePayload(input: { projectId?: string | null; rootIssueId?: string | null }) {
  if (input.projectId && input.rootIssueId) {
    throw new Error("DealDesk source scope must specify either projectId or rootIssueId, not both.");
  }
}
...
function assertRequestedCharacterLimit(name: string, value: unknown, max: number) {
  ...
  if (Math.floor(value) > max) {
    throw new Error(`${name} exceeds the hard DealDesk ingestion cap of ${max} characters.`);
  }
}
...
function normalizeCostRate(value: unknown): number {
  if (typeof value !== "number" || !Number.isFinite(value)) return DEFAULT_DEALDESK_COST_CENTS_PER_1K_CHARS;
  return Math.max(0, value);
}

type DealDeskDistillationLimits = {
  maxCharacters: number;
  maxCharactersPerSource: number;
...
type DistillationSourceProtectionResult = {
  body: string;
  warning: string | null;
  refPatch: Pick<DealDeskSourceRef, "redactionStatus" | "redactionReasons">;
};
...
function protectDistillationSourceBody(input: {
  ...
  return {
    body: [
      ...
      "- Review the original DealDesk source directly if a human needs the unredacted material.",
    ].join("\n"),
...
async function resolveDealDeskDistillationLimits(
  ctx: PluginContext,
  input: Pick<DealDeskSourceBundleInput, "maxCharacters" | "maxCharactersPerSource" | "routineRun">,
): Promise<DealDeskDistillationLimits> {
  assertRequestedCharacterLimit("maxCharacters", input.maxCharacters, DEFAULT_MAX_DEALDESK_CURSOR_WINDOW_CHARS);
  assertRequestedCharacterLimit("maxCharactersPerSource", input.maxCharactersPerSource, DEFAULT_MAX_DEALDESK_ISSUE_SOURCE_CHARS);
  const config = await ctx.config.get() as Record<string, unknown>;
  const maxCharactersPerSource = Math.min(
    normalizeBundleLimit(input.maxCharactersPerSource, DEFAULT_MAX_DEALDESK_ISSUE_SOURCE_CHARS),
    normalizeBundleLimit(config.maxDealDeskIssueSourceCharacters, DEFAULT_MAX_DEALDESK_ISSUE_SOURCE_CHARS),
  );
  const cursorWindowCap = normalizeBundleLimit(
    config.maxDealDeskCursorWindowCharacters,
    DEFAULT_MAX_DEALDESK_CURSOR_WINDOW_CHARS,
  );
  const routineRunCap = normalizeBundleLimit(
    config.maxDealDeskRoutineRunCharacters,
    DEFAULT_MAX_DEALDESK_ROUTINE_RUN_CHARS,
  );
...
async function resolveDealDeskDistillationLimitsForSpace(
  ctx: PluginContext,
  input: Pick<DealDeskSourceBundleInput, "companyId" | "maxCharacters" | "maxCharactersPerSource" | "routineRun"> & { space: WikiSpace },
): Promise<DealDeskDistillationLimits> {
  const [base, profile] = await Promise.all([
    resolveDealDeskDistillationLimits(ctx, input),
...
function defaultDealDeskIngestionProfile(input: {
  space: Pick<WikiSpace, "slug">;
  legacySettings?: WikiEventIngestionSettings | null;
}): DealDeskIngestionProfileV1 {
  ...
  return {
    ...
    cursor: {
      maxWindowCharacters: DEFAULT_MAX_DEALDESK_CURSOR_WINDOW_CHARS,
      maxCharactersPerSource: DEFAULT_MAX_DEALDESK_ISSUE_SOURCE_CHARS,
...
function normalizeDealDeskIngestionSourceScope(value: unknown): DealDeskIngestionSourceScope | null {
  ...
  if (kind === "active_projects") {
    ...
    return {
      kind,
      limit: normalizeLimit(record.limit, 3, MAX_DEALDESK_PROFILE_SELECTED_PROJECTS),
      ...(statuses && statuses.length > 0 ? { statuses: [...new Set(statuses)] } : {}),
    };
  }
  if (kind === "selected_projects") {
    return { kind, projectIds: stringArray(record.projectIds).slice(0, MAX_DEALDESK_PROFILE_SELECTED_PROJECTS) };
  }
  if (kind === "root_issues") {
    return { kind, issueIds: stringArray(record.issueIds).slice(0, MAX_DEALDESK_PROFILE_ROOT_ISSUES) };
  }
  if (kind === "company_all") {
...
function normalizeDealDeskIngestionProfile(
  value: unknown,
  input: { space: Pick<WikiSpace, "slug">; legacySettings?: WikiEventIngestionSettings | null },
): DealDeskIngestionProfileV1 {
  const fallback = defaultDealDeskIngestionProfile(input);
  ...
  return {
    ...
    sourceScopes: Array.isArray(record.sourceScopes)
      ? record.sourceScopes.map(normalizeDealDeskIngestionSourceScope).filter((scope): scope is DealDeskIngestionSourceScope => Boolean(scope))
    ...
    cursor: {
      maxWindowCharacters: normalizeLimit(cursor.maxWindowCharacters, fallback.cursor.maxWindowCharacters, DEFAULT_MAX_DEALDESK_CURSOR_WINDOW_CHARS),
      maxCharactersPerSource: normalizeLimit(cursor.maxCharactersPerSource, fallback.cursor.maxCharactersPerSource, DEFAULT_MAX_DEALDESK_ISSUE_SOURCE_CHARS),
...
function evaluateDealDeskProfilePolicy(input: {
  space: WikiSpace;
  profile?: DealDeskIngestionProfileV1 | null;
  purpose: DealDeskIngestionPolicyPurpose;
  requireEnabledProfile?: boolean;
}): DealDeskIngestionPolicyDecision {
  ...
  if (space.status !== "active") {
    return {
      ...
      message: `DealDesk ingestion policy denied ${purpose}: space "${space.slug}" is ${space.status}.`,
    };
  }
  if (space.accessScope !== "shared") {
    return {
      ...
      message: `DealDesk ingestion policy denied ${purpose}: ${space.accessScope} spaces cannot ingest DealDesk sources until host permissions are enforced.`,
    };
  }
  if (input.requireEnabledProfile && space.slug !== DEFAULT_SPACE_SLUG && !profile?.enabled) {
    return {
      ...
      message: `DealDesk ingestion policy denied ${purpose}: DealDesk ingestion is not enabled for space "${space.slug}".`,
    };
  }
  if (input.requireEnabledProfile && space.slug !== DEFAULT_SPACE_SLUG && profile?.enabled && profile.sourceScopes.length === 0) {
    return {
      ...
      message: `DealDesk ingestion policy denied ${purpose}: space "${space.slug}" has no source scopes configured.`,
    };
  }
  return { allowed: true, space };
}

export async function getDealDeskIngestionProfile(
  ctx: PluginContext,
  input: { companyId: string; wikiId?: string | null; spaceSlug?: string | null },
): Promise<DealDeskIngestionProfileRead> {
  ...
  const profile = await profileForSpace(ctx, input.companyId, space);
  const policy = evaluateDealDeskProfilePolicy({ space, profile, purpose: "profile_read" });
  const historicalPageCount = await countDealDeskHistoricalPages(ctx, {
    companyId: input.companyId,
    wikiId,
    spaceId: space.id,
  });
  const overlapCount = await countDealDeskProfileOverlaps(ctx, {
    companyId: input.companyId,
    wikiId,
    space,
    profile,
  });
  const effectiveState: DealDeskIngestionProfileEffectiveState = !policy.allowed
    ? "policy_blocked"
...
async function countDealDeskHistoricalPages(ctx: PluginContext, input: { companyId: string; wikiId: string; spaceId: string }): Promise<number> {
...
function scopeIdentity(scope: DealDeskIngestionSourceScope): string[] {
...
async function countDealDeskProfileOverlaps(ctx: PluginContext, input: {
  companyId: string;
  wikiId: string;
  space: WikiSpace;
  profile: DealDeskIngestionProfileV1;
...
async function validateDealDeskIngestionProfile(ctx: PluginContext, input: {
  companyId: string;
  space: WikiSpace;
  profile: DealDeskIngestionProfileV1;
}) {
  const policy = evaluateDealDeskProfilePolicy({
    space: input.space,
  ...
  if (input.profile.enabled && input.profile.sourceScopes.length === 0) {
    throw new Error("DealDesk ingestion profile must include at least one source scope before it can be enabled.");
  }
  if (input.profile.sourceScopes.length > MAX_DEALDESK_INGESTION_PROFILE_SOURCE_COUNT) {
    throw new Error(`DealDesk ingestion profile sources exceed the hard cap of ${MAX_DEALDESK_INGESTION_PROFILE_SOURCE_COUNT}.`);
  }
  for (const scope of input.profile.sourceScopes) {
    ...
    if (scope.kind === "selected_projects") {
      if (scope.projectIds.length > MAX_DEALDESK_PROFILE_SELECTED_PROJECTS) {
        throw new Error(`selected_projects exceeds the hard cap of ${MAX_DEALDESK_PROFILE_SELECTED_PROJECTS}.`);
      }
    ...
    if (scope.kind === "root_issues") {
      if (scope.issueIds.length > MAX_DEALDESK_PROFILE_ROOT_ISSUES) {
        throw new Error(`root_issues exceeds the hard cap of ${MAX_DEALDESK_PROFILE_ROOT_ISSUES}.`);
      }
...
export async function updateDealDeskIngestionProfile(ctx: PluginContext, input: {
  companyId: string;
  wikiId?: string | null;
  spaceSlug?: string | null;
  profile: unknown;
  ...
  const profile = normalizeDealDeskIngestionProfile(input.profile, { space, legacySettings: space.slug === DEFAULT_SPACE_SLUG ? await getEventIngestionSettings(ctx, input.companyId) : null });
  await validateDealDeskIngestionProfile(ctx, { companyId: input.companyId, space, profile });
  ...
  await ctx.activity.log({
    companyId: input.companyId,
    message: `Updated DealDesk ingestion profile for ${space.displayName}`,
    entityType: "llm_wiki_space",
    entityId: space.id,
    metadata: {
      type: "plugin.llm_wiki.dealdesk_ingestion_profile_updated",
      wikiId,
      spaceSlug: space.slug,
  ...
  return getDealDeskIngestionProfile(ctx, { companyId: input.companyId, wikiId, spaceSlug: space.slug });
}

export async function listDealDeskIngestionCandidates(ctx: PluginContext, input: DealDeskIngestionCandidatesInput): Promise<{
  ...
  const wikiId = normalizeWikiId(input.wikiId);
  await requireDealDeskIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "candidate_search");
...
export async function updateEventIngestionSettings(
  ...
  await requireDealDeskIngestionPolicy(ctx, {
    companyId: input.companyId,
  ...
  if (sourceKeys.length > MAX_DEALDESK_INGESTION_PROFILE_SOURCE_COUNT) {
    throw new Error(`DealDesk ingestion profile sources exceed the hard cap of ${MAX_DEALDESK_INGESTION_PROFILE_SOURCE_COUNT}.`);
  }
  ...
  const profile = normalizeDealDeskIngestionProfile(
    {
      ...defaultDealDeskIngestionProfile({ space: defaultSpace, legacySettings: next }),
      ...
      cursor: {
        ...defaultDealDeskIngestionProfile({ space: defaultSpace, legacySettings: next }).cursor,
...
function assertWikiPath(path: string, options: { allowMetadata?: boolean } = {}): string {
  ...
  if (
    ...
    !(options.allowMetadata && trimmed.startsWith(".dealdesk/"))
  ) {
...
function distillationCursorTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "dealdesk_distillation_cursors");
}

function distillationRunTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "dealdesk_distillation_runs");
}

function sourceSnapshotTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "dealdesk_source_snapshots");
}

function distillationWorkItemTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "dealdesk_distillation_work_items");
}

function pageBindingTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "dealdesk_page_bindings");
}
...
export async function createOperationIssue(ctx: PluginContext, input: OperationInput) {
  ...
  const space = input.operationType === "distill" || input.operationType === "backfill"
    ? await requireDealDeskIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "queue", { requireEnabledProfile: true })
...
async function upsertDealDeskDistillationCursor(ctx: PluginContext, input: {
  companyId: string;
  wikiId: string;
  spaceId: string;
  projectId?: string | null;
  ...
  await ctx.db.execute(
    `INSERT INTO ${distillationCursorTable(ctx)} AS dealdesk_distillation_cursors
     ...
     VALUES ($1, $2, $3, $11, $4, $5, $6, $7, 'dealdesk_issue_history', $8::timestamptz, $9, $10::jsonb)
     ON CONFLICT (company_id, wiki_id, space_id, source_scope, scope_key, source_kind)
     DO UPDATE SET last_observed_at = GREATEST(
                       COALESCE(dealdesk_distillation_cursors.last_observed_at, EXCLUDED.last_observed_at),
                       COALESCE(EXCLUDED.last_observed_at, dealdesk_distillation_cursors.last_observed_at)
                     ),
                   pending_event_count = dealdesk_distillation_cursors.pending_event_count + EXCLUDED.pending_event_count,
                   metadata = dealdesk_distillation_cursors.metadata || EXCLUDED.metadata,
                   updated_at = now()`,
    [
      cursorId,
      input.companyId,
  ...
  const rows = await ctx.db.query<{ id: string }>(
    `SELECT id
      ...
      WHERE company_id = $1
        ...
        AND source_scope = $4
        AND scope_key = $5
        AND source_kind = 'dealdesk_issue_history'
      LIMIT 1`,
...
export async function enableActiveProjectDistillation(ctx: PluginContext, input: {
  ...
  const wikiId = normalizeWikiId(input.wikiId);
  const space = await requireDealDeskIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "candidate_search", { requireEnabledProfile: true });
  if (typeof input.limit === "number" && Number.isFinite(input.limit) && Math.floor(input.limit) > MAX_DEALDESK_DISTILLATION_FAN_OUT) {
...
function appendBoundedSection(input: {
  ...
  title: string;
  body: string;
  refs: DealDeskSourceRef[];
  ref: DealDeskSourceRef;
...
function sourceRefUpdatedAt(ref: DealDeskSourceRef): string | null {
  return ref.updatedAt ?? ref.createdAt ?? null;
}

function issueInBackfillWindow(issue: Issue, input: Pick<DealDeskSourceBundleInput, "backfillStartAt" | "backfillEndAt">): boolean {
...
async function listDealDeskBundleIssues(ctx: PluginContext, input: DealDeskSourceBundleInput): Promise<Issue[]> {
...
export async function assembleDealDeskSourceBundle(ctx: PluginContext, input: DealDeskSourceBundleInput): Promise<DealDeskSourceBundle> {
  const wikiId = normalizeWikiId(input.wikiId);
...
export async function createDealDeskDistillationRun(ctx: PluginContext, input: DealDeskDistillationRunInput) {
  const wikiId = normalizeWikiId(input.wikiId);
  assertDealDeskSourceScopePayload(input);
  const space = await requireDealDeskIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "execute", { requireEnabledProfile: true });
  const scope = paperclipCursorScopeMetadata(input);
  ...
  const bundle = await assembleDealDeskSourceBundle(ctx, input);
...
export async function recordDealDeskDistillationOutcome(ctx: PluginContext, input: DealDeskDistillationOutcomeInput) {
  const wikiId = normalizeWikiId(input.wikiId);
  const space = await requireDealDeskIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "execute", { requireEnabledProfile: true });
...
export async function createDealDeskDistillationWorkItem(ctx: PluginContext, input: DealDeskDistillationWorkItemInput) {
  const wikiId = normalizeWikiId(input.wikiId);
  assertDealDeskSourceScopePayload(input);
  ...
  await ctx.db.execute(
    `INSERT INTO ${distillationWorkItemTable(ctx)} AS dealdesk_distillation_work_items
     ...
     DO UPDATE SET priority = EXCLUDED.priority,
                   metadata = dealdesk_distillation_work_items.metadata || EXCLUDED.metadata,
                   updated_at = now()`,
    [
      itemId,
      input.companyId,
...
function sourceRefLabel(ref: DealDeskSourceRef): string {
  const issue = ref.issueIdentifier ? issueReference(ref.issueIdentifier) : (ref.title ?? "DealDesk source");
  if (ref.kind === "document") return `${issue} document:${ref.documentKey ?? "unknown"}`;
  if (ref.kind === "comment") return `${issue} comment`;
  return issue;
}

function sourceRefMarkdown(ref: DealDeskSourceRef): string {
  const metadata = [
...
function issueSourceRef(issue: Issue): DealDeskSourceRef {
  return {
    kind: "issue",
    issueId: issue.id,
    issueIdentifier: issue.identifier ?? null,
...
function hasDurableSignal(bundle: DealDeskSourceBundle, issues: Issue[]): boolean {
...
function standupPageContents(input: {
  ...
  issues: Issue[];
  bundle: DealDeskSourceBundle;
  pagePath: string;
  durablePagePath: string;
}): string {
  const currentAsOf = input.bundle.sourceWindowEnd ?? new Date().toISOString();
  const title = input.project?.name ?? input.rootIssue?.title ?? "DealDesk Project";
  ...
  return [
    ...
    "",
    lead
      ? `The current center of gravity is **${issueConcept(lead)}** (${issueReferenceFor(lead)}). ${input.bundle.clipped ? "The source window was clipped, so treat this as a bounded readout rather than the full live state." : "This is a high-level readout of the meaningful DealDesk work in the current source window."}`
...
function projectPageContents(input: {
  ...
  return [
    ...
    "## Overview",
    "",
    description ? excerpt(description, 700) : `This page synthesizes DealDesk issue history into a stable project brief for ${title}.`,
    "",
    "## Current Direction",
    "",
    activeIssues.length
    ...
    input.bundle.clipped ? "\nThe source window was clipped, so verify DealDesk before treating this as complete state." : null,
    "",
    "## Workstreams",
    "",
    ...(recentIssues.length
...
function decisionsPageContents(input: { project: Project | null; rootIssue: Issue | null; issues: Issue[]; bundle: DealDeskSourceBundle }): string {
  const title = input.project?.name ?? input.rootIssue?.title ?? "DealDesk Project";
...
function historyPageContents(input: { project: Project | null; rootIssue: Issue | null; issues: Issue[]; bundle: DealDeskSourceBundle }): string {
  const title = input.project?.name ?? input.rootIssue?.title ?? "DealDesk Project";
...
function appendProjectLogContents(current: string | null, input: { standupPath: string; pagePath: string; runId: string; sourceHash: string; status: string; warnings: string[] }): string {
  ...
  const warningLines = input.warnings.length
    ...
    : ["- warnings: none"];
  const entry = [
    `## [${new Date().toISOString().slice(0, 10)}] dealdesk-distill | ${input.status}`,
...
async function upsertPageBinding(ctx: PluginContext, input: {
  ...
  await ctx.db.execute(
    `INSERT INTO ${pageBindingTable(ctx)} AS dealdesk_page_bindings
     ...
     DO UPDATE SET last_applied_source_hash = EXCLUDED.last_applied_source_hash,
                   ...
                   metadata = dealdesk_page_bindings.metadata || EXCLUDED.metadata,
                   updated_at = now()`,
    [
      randomUUID(),
      input.companyId,
...
export function getDistillationAutoApplyRestriction(): DistillationAutoApplyRestriction {
  const rawMode = process.env.DEALDESK_DEPLOYMENT_MODE;
  const rawExposure = process.env.DEALDESK_DEPLOYMENT_EXPOSURE;
...
export async function distillDealDeskProjectPage(ctx: PluginContext, input: DealDeskProjectPageDistillationInput) {
  if (!input.projectId && !input.rootIssueId) {
  ...
  const run = await createDealDeskDistillationRun(ctx, input);
  ...
  if (!hasDurableSignal(bundle, issues)) {
    await recordDealDeskDistillationOutcome(ctx, {
      companyId: input.companyId,
      wikiId,
      spaceSlug: space.slug,
      runId: run.runId,
      ...
      warning: "Skipped low-signal DealDesk source window.",
    });
    return {
      status: "skipped",
      reason: "low_signal",
      ...
      cursorId: run.cursorId,
      sourceHash: bundle.sourceHash,
      warnings: ["Skipped low-signal DealDesk source window."],
      patches: [] as DealDeskDistillationPatch[],
    };
  }
  ...
  if (existingBinding?.last_applied_source_hash === bundle.sourceHash) {
    await recordDealDeskDistillationOutcome(ctx, {
      ...
      sourceHash: bundle.sourceHash,
      sourceWindowEnd: bundle.sourceWindowEnd,
      warning: "Skipped unchanged DealDesk source hash.",
    });
    return {
      status: "skipped",
      ...
      cursorId: run.cursorId,
      sourceHash: bundle.sourceHash,
      warnings: ["Skipped unchanged DealDesk source hash."],
      patches: [] as DealDeskDistillationPatch[],
    };
  }
  ...
  const title = project?.name ?? rootIssue?.title ?? "DealDesk Project";
  ...
  const patches: DealDeskDistillationPatch[] = [
  ...
  for (const patch of patches) {
    await writeWikiPage(ctx, {
      ...
      summary: `DealDesk distillation ${patch.operationType} from ${bundle.sourceHash}`,
...
function rawPathForDealDeskEvent(input: {
...
function formatIssueEventSource(issue: Issue, event: PluginEvent, maxCharacters: number): string {
  return truncateEventSource([
    `# DealDesk issue: ${sourceTitleForIssue(issue)}`,
    "",
    "## Provenance",
    "",
...
function formatCommentEventSource(issue: Issue, comment: IssueComment, event: PluginEvent, maxCharacters: number): string {
  return truncateEventSource([
    `# DealDesk comment on ${sourceTitleForIssue(issue)}`,
    "",
    "## Provenance",
    "",
    `- Company ID: ${issue.companyId}`,
...
function formatDocumentEventSource(issue: Issue, document: IssueDocument, event: PluginEvent, maxCharacters: number): string {
  return truncateEventSource([
    `# DealDesk document: ${document.title ?? document.key}`,
    "",
    "## Provenance",
    "",
    `- Company ID: ${issue.companyId}`,
...
async function recordDealDeskCursorObservation(ctx: PluginContext, input: {
  companyId: string;
  wikiId: string;
  space: WikiSpace;
  sourceKind: WikiEventIngestionSource;
  ...
  issue: Issue;
  event: PluginEvent;
}): Promise<Extract<DealDeskEventIngestResult, { status: "recorded" }>> {
  const cursorId = await upsertDealDeskDistillationCursor(ctx, {
    companyId: input.companyId,
    wikiId: input.wikiId,
    spaceId: input.space.id,
...
async function routeDealDeskCursorObservation(ctx: PluginContext, input: {
  companyId: string;
  sourceKind: WikiEventIngestionSource;
  sourceId: string;
  issue: Issue;
  event: PluginEvent;
}): Promise<DealDeskEventIngestResult> {
  const { spaces } = await listSpaces(ctx, { companyId: input.companyId, wikiId: DEFAULT_WIKI_ID });
  const recorded: Array<Extract<DealDeskEventIngestResult, { status: "recorded" }>> = [];
  ...
  for (const space of spaces) {
    ...
    recorded.push(await recordDealDeskCursorObservation(ctx, {
      ...input,
      wikiId: space.wikiId,
      space,
    }));
...
export async function handleDealDeskEventIngestion(ctx: PluginContext, event: PluginEvent): Promise<DealDeskEventIngestResult> {
  ...
  if (event.eventType === "issue.created" || event.eventType === "issue.updated") {
    ...
    return routeDealDeskCursorObservation(ctx, {
      companyId,
      sourceKind: "issues",
      sourceId,
      issue,
