...
export const PLUGIN_ID = "dealdesk.plugin-llm-wiki";
...
export const DEALDESK_DISTILL_SKILL_KEY = "dealdesk-distill";
...
export const DEFAULT_MAX_DEALDESK_ISSUE_SOURCE_CHARS = 12000;
export const DEFAULT_MAX_DEALDESK_CURSOR_WINDOW_CHARS = 60000;
export const DEFAULT_MAX_DEALDESK_ROUTINE_RUN_CHARS = 120000;
export const DEFAULT_DEALDESK_COST_CENTS_PER_1K_CHARS = 1;
...
function canonicalSkillKey(skillKey: string) {
  return `plugin/dealdesk-plugin-llm-wiki/${skillKey}`;
}
...
const CURSOR_WINDOW_ROUTINE_DESCRIPTION = `Process bounded DealDesk issue-history windows into the LLM Wiki.

Run procedure:
Target space: default (slug: default). DealDesk-derived indexing currently writes only into the default space, so this routine never sweeps other spaces. Per-space DealDesk ingestion profiles are a later phase; until they ship, treat any prompt to operate on a non-default space here as a bug and stop.
...
5. For ambiguous contradictions or major rewrites, leave the pages unchanged and create or comment a follow-up DealDesk issue with the exact files and evidence.
...
const manifest: DealDeskPluginManifestV1 = {
  ...
  localFolders: [
    {
      ...
      description: "Company-scoped local folder that stores raw sources, wiki pages, DealDesk project standups under wiki/projects/, AGENTS.md, IDEA.md, wiki/index.md, and wiki/log.md.",
      access: "readWrite",
      requiredDirectories: [
        "raw",
        "wiki",
  ...
  skills: [
    ...
    {
      skillKey: DEALDESK_DISTILL_SKILL_KEY,
      displayName: "DealDesk Distill",
      slug: DEALDESK_DISTILL_SKILL_KEY,
      description: "Turn DealDesk cursor-window, distill, or backfill source bundles into wiki-insightful project knowledge.",
      markdown: skillMarkdown(DEALDESK_DISTILL_SKILL_KEY)
    },
    {
