...
const PLUGIN_ID = "dealdesk.plugin-llm-wiki";
const WIKI_SIDEBAR_NAV_STATE_KEY = "dealDeskWikiSidebarTreePath";
...
type DealDeskIngestionSourceScope =
...
type DealDeskIngestionProfileData = {
  wikiId: string;
  space: Pick<WikiSpace, "id" | "slug" | "displayName" | "accessScope" | "status">;
  profile: DealDeskIngestionProfile;
...
function useDealDeskIngestionProfile(companyId: string | null, spaceSlug: string | null) {
  const params = useMemo(() => {
    if (!companyId || !spaceSlug) return undefined;
    return { companyId, spaceSlug };
  }, [companyId, spaceSlug]);
  return usePluginData<DealDeskIngestionProfileData>("dealdesk-ingestion-profile", params);
}
        ...
        <div style={{ padding: "16px 18px", borderBottom: `1px solid ${tokens.border}`, display: "flex", gap: 12, alignItems: "flex-start" }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <Tiny style={{ marginTop: 4 }}>
              ...
              Wiki ingest operations. This is manual file ingest - DealDesk-derived distillation always
    ...
    >
      ...
      >
        <div style={{ padding: "16px 20px", borderBottom: `1px solid ${tokens.border}`, display: "flex", alignItems: "flex-start", gap: 12 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <Tiny style={{ marginTop: 4 }}>
              ...
              under the wiki root. DealDesk distillation and event capture always write into the
              default space and skip new spaces created here - per-space DealDesk routing is a later
              phase.
            </Tiny>
          </div>
        ...
        <div style={{ padding: 20, display: "grid", gap: 16 }}>
          ...
          <FormField label="Access scope" help="Access scope is metadata only. It does not currently enforce who can read or write the space, and it does not change which DealDesk sources reach the space.">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto border-t border-border px-3 py-3">
        ...
        <div className="mb-1 flex items-center gap-1 px-2 text-[11px] font-semibold uppercase tracking-normal text-muted-foreground" style={{ height: 24 }}>
          <span
            ...
            title="Destination spaces. Browsing and manual ingest happen in the active space; DealDesk distillation always writes into the default space in Phase 1."
          >
            Shared Spaces
          </span>
          <button
          ...
          <div style={{ display: "grid", gap: isMobile ? 12 : 14, minWidth: 0 }}>
            <div ref={markdownBodyRef} style={{ minWidth: 0, fontSize: 13, lineHeight: 1.65 }}>
              ...
              {displaySourceRefs.length > 0 ? (
                <section aria-label="DealDesk source refs" style={{ marginTop: 16, display: "grid", gap: 6 }}>
                  <Tiny style={{ fontWeight: 650 }}>DealDesk source refs</Tiny>
...
// ---------------------------------------------------------------------------
// History tab: native DealDesk issue table for recent LLM Wiki operation
  ...
  return (
    <div style={{ flex: 1, minHeight: isMobile ? "auto" : 0, overflow: isMobile ? "visible" : "auto", padding: isMobile ? 12 : 0, minWidth: 0 }}>
      <PluginIssuesList
        ...
        viewStateKey="dealdesk:llm-wiki-history-issues-view"
...
const MANAGED_SKILL_LABELS: Record<string, string> = {
  ...
  "wiki-lint": "Wiki Lint",
  "dealdesk-distill": "DealDesk Distill",
...
const SETTINGS_SECTIONS: ReadonlyArray<{
  ...
  { key: "spaces", label: "Spaces", description: "Destination spaces - folders, slugs, and folder health. Per-space DealDesk indexing is not configurable yet." },
  { key: "distillation", label: "Distillation", description: "DealDesk -> default space. Cursors, caps, and routines for the company-wide distillation pipeline." },
  ...
  { key: "events", label: "Ingestion Settings", description: "DealDesk event capture into the default space (issues, comments, documents)." },
];
...
function DistillationSettingsPanel({ context, settings }: { context: { companyId: string | null }; settings: SettingsData }) {
  ...
  const distillNow = usePluginAction("distill-dealdesk-now");
  const enableActiveProjects = usePluginAction("enable-dealdesk-distillation-active-projects");
  const queueBackfill = usePluginAction("backfill-dealdesk-distillation");
  const toast = usePluginToast();
  const isMobile = useIsMobileLayout();
            ...
            <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", gap: 12 }}>
              ...
              <div>
                ...
                <Tiny style={{ marginTop: 6, fontSize: 13, color: tokens.fg, lineHeight: 1.55, maxWidth: 540 }}>
                  When enabled, the Wiki Maintainer reads DealDesk issues, comments, and documents for this
                ...
                <Tiny style={{ marginTop: 6, fontSize: 13, color: tokens.fg, lineHeight: 1.55, maxWidth: 540 }}>
                  Other spaces do not receive DealDesk-derived pages yet. They stay on manual and raw-file
                  ingest until per-space DealDesk ingestion profiles ship.
                </Tiny>
              </div>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        ...
        <CardBody padding={14}>
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 16 }}>
            <fieldset style={{ border: 0, padding: 0, margin: 0, display: "grid", gap: 6 }}>
              ...
              <Tiny>
                These filters narrow the DealDesk source scope. The destination is always the default
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {activeSettingsSection === "root" ? (
              ...
              <div style={{ display: "grid", gap: 14, maxWidth: isMobile ? "none" : 620, minWidth: 0 }}>
                ...
                <div style={{ display: "grid", gap: 8 }}>
                  ...
                  {showMaintainerWarning ? (
                    <Callout tone="warn">
                      This is not the DealDesk-provided Wiki Maintainer. Plugin operations and routines may miss the recommended wiki role, tools, and default instructions.
                    </Callout>
                  ) : null}
          ...
          <SettingsPanel
            ...
            description="Read DealDesk issues, comments, and documents for this company and write project pages into the default wiki space. Assets/attachments and work products stay metadata-only in Phase 5 and are excluded from source-text extraction. Other spaces cannot be selected as a destination yet - that lands with per-space DealDesk ingestion profiles."
          >
          ...
          <SettingsPanel
            title="DealDesk event ingestion"
          ...
          <Tiny style={{ marginBottom: 10 }}>
            Company-scoped DealDesk events can advance default-space cursors. Enable only the first-party text sources this wiki should observe for default-space distillation.
          </Tiny>
          <div style={{ display: "grid", gap: 10 }}>
            ...
            <Callout tone="warn">
              Event ingestion records selected DealDesk issue, comment, and document activity for the default wiki space. Assets/attachments and work products are excluded here: Phase 5 allows metadata-only references later, not blob reads or linked-content fetches. It never reads across companies or creates non-default space cursors.
            </Callout>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
...
function SpaceEditCard({
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      ...
      <DealDeskIngestionSpaceCard companyId={companyId} space={space} refresh={refresh} />
        ...
        <CardBody>
          <div aria-disabled="true" style={{ display: "grid", gap: 10 }}>
            ...
            <Tiny>
              ...
              read/write permissions, and they do not change which DealDesk sources reach this space.
            </Tiny>
...
function dealDeskIngestionStateBadge(data: DealDeskIngestionProfileData | null): { tone: Tone; label: string } {
...
function DealDeskIngestionSpaceCard({ companyId, space, refresh }: { companyId: string | null; space: WikiSpace; refresh: () => void }) {
  const profileQuery = useDealDeskIngestionProfile(companyId, space.slug);
  const updateProfile = usePluginAction("update-dealdesk-ingestion-profile");
  const toast = usePluginToast();
  const data = profileQuery.data ?? null;
  const [draft, setDraft] = useState<DealDeskIngestionProfile | null>(null);
  const [busy, setBusy] = useState(false);
  ...
  const badge = dealDeskIngestionStateBadge(data);
  ...
  function patchDraft(patch: Partial<DealDeskIngestionProfile>) {
  ...
  async function save() {
    ...
    try {
      ...
      toast({ tone: "success", title: "DealDesk ingestion profile saved", body: `${space.displayName} will use the selected DealDesk sources.` });
  ...
  return (
    <Card>
      <CardHeader
        title={<span>DealDesk → {space.displayName}</span>}
        right={<Badge tone={badge.tone} style={{ fontSize: 10 }}>{badge.label}</Badge>}
      />
      <CardBody>
        <div style={{ display: "grid", gap: 12 }}>
          {profileQuery.loading && !data ? <Tiny>Loading DealDesk ingestion profile…</Tiny> : null}
          {profileQuery.error ? <Callout tone="danger">{profileQuery.error.message}</Callout> : null}
          {locked ? (
            <Callout tone="warn">
              Locked — host permissions pending. DealDesk ingestion stays disabled on team and personal spaces until LLM Wiki enforces read/write permissions for non-shared spaces.
            </Callout>
          ) : null}
          {data && data.historicalPageCount > 0 && data.effectiveState === "disabled" ? (
            <Callout>
              Off · {data.historicalPageCount} historical DealDesk page{data.historicalPageCount === 1 ? "" : "s"} still in this space. Disabling stops new observations but does not delete prior wiki pages.
            </Callout>
          ) : null}
          ...
          {draft ? (
            <>
              <label style={{ display: "flex", gap: 8, alignItems: "flex-start", fontSize: 13 }}>
                ...
                />
                <span>
                  Enable DealDesk ingestion for this destination space
                  <Tiny style={{ display: "block" }}>Future DealDesk issue, comment, and document events can advance cursors in {space.displayName}. Existing pages are preserved when this is turned off.</Tiny>
                </span>
              </label>
              <div style={{ display: "grid", gap: 8 }}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                <Button size="sm" variant="primary" onClick={save} loading={busy} disabled={!canSave || emptyScopes}>Save DealDesk profile</Button>
