...
import type { Agent, Issue, PluginManagedRoutineResolution, Project } from "@dealdesk/plugin-sdk";
import manifest, {
  CURSOR_WINDOW_ROUTINE_KEY,
  INDEX_REFRESH_ROUTINE_KEY,
  NIGHTLY_LINT_ROUTINE_KEY,
  DEALDESK_DISTILL_SKILL_KEY,
...
const ORIGINAL_DEPLOYMENT_MODE = process.env.DEALDESK_DEPLOYMENT_MODE;
const ORIGINAL_DEPLOYMENT_EXPOSURE = process.env.DEALDESK_DEPLOYMENT_EXPOSURE;
type TestBridgeGlobal = typeof globalThis & {
  __dealDeskPluginBridge__?: {
...
beforeEach(() => {
  if (ORIGINAL_DEPLOYMENT_MODE == null) {
    delete process.env.DEALDESK_DEPLOYMENT_MODE;
  } else {
    process.env.DEALDESK_DEPLOYMENT_MODE = ORIGINAL_DEPLOYMENT_MODE;
  }
  if (ORIGINAL_DEPLOYMENT_EXPOSURE == null) {
    delete process.env.DEALDESK_DEPLOYMENT_EXPOSURE;
  } else {
    process.env.DEALDESK_DEPLOYMENT_EXPOSURE = ORIGINAL_DEPLOYMENT_EXPOSURE;
  }
  ...
  (globalThis as TestBridgeGlobal).__dealDeskPluginBridge__ = {
    sdkUi: {
...
afterEach(() => {
  ...
  delete (globalThis as TestBridgeGlobal).__dealDeskPluginBridge__;
});
...
function dealDeskIssue(overrides: Partial<Issue> = {}): Issue {
  const now = new Date();
  return {
    ...
    description: "Decide which DealDesk issues, comments, and documents can be ingested into the wiki.",
    status: "todo",
    workMode: "standard",
    priority: "medium",
...
describe("LLM Wiki plugin scaffold", () => {
  it("declares standalone plugin surfaces without core wiki coupling", () => {
    expect(manifest.id).toBe("dealdesk.plugin-llm-wiki");
  ...
  it("ships Karpathy-pattern schema and workflow prompts by default", () => {
    ...
    expect(manifest.agents?.[0]?.adapterConfig?.dealdeskSkillSync).toEqual({
  ...
  it("routes legacy Wiki operations URLs to the History issue table", () => {
    ...
    expect(markup).toContain("Issues table · project-1 · plugin:dealdesk.plugin-llm-wiki:operation");
...
`;
    ...
    expect(markup).toContain("dealdesk");
...
`;
  ...
  it("does not ingest DealDesk events until operator controls enable them", async () => {
    const harness = createTestHarness({ manifest });
    const issue = dealDeskIssue();
  ...
  it("records enabled DealDesk issue events as cursor observations without creating ingest operations", async () => {
    const harness = createTestHarness({ manifest });
    const issue = dealDeskIssue();
    harness.seed({ issues: [issue] });
    const writes: Array<{ path: string; contents: string }> = [];
    ...
    expect(harness.dbExecutes.some((execute) => execute.sql.includes("wiki_sources"))).toBe(false);
    expect(harness.dbExecutes.some((execute) => execute.sql.includes("wiki_operations"))).toBe(false);
    const cursorUpsert = harness.dbExecutes.find((execute) => execute.sql.includes("dealdesk_distillation_cursors"));
  ...
  it("preserves DealDesk event ingestion sources when only enabled changes", async () => {
    const harness = createTestHarness({ manifest });
  ...
  it("keeps DealDesk event cursor observations company scoped and ignores plugin-operation issues", async () => {
    const harness = createTestHarness({ manifest });
    const visibleIssue = dealDeskIssue({ projectId: "77777777-7777-4777-8777-777777777777" });
    const otherCompanyIssue = dealDeskIssue({
      id: "77777777-7777-4777-8777-777777777778",
      companyId: OTHER_COMPANY_ID,
      projectId: "77777777-7777-4777-8777-777777777779",
      identifier: "PAP-9999",
    });
    const operationIssue = dealDeskIssue({
    ...
    const cursorWrites = harness.dbExecutes.filter((execute) => execute.sql.includes("dealdesk_distillation_cursors"));
  ...
  it("routes DealDesk issue, comment, and document event cursors only to the default space", async () => {
    const harness = createTestHarness({ manifest });
    const issue = dealDeskIssue({ projectId: "77777777-7777-4777-8777-777777777777" });
    harness.seed({ issues: [issue] });
  ...
  it("routes DealDesk events into an explicitly enabled shared non-default space", async () => {
    const harness = createTestHarness({ manifest });
    const project = existingProject();
    const issue = dealDeskIssue({ projectId: project.id });
    ...
    await harness.performAction("update-dealdesk-ingestion-profile", {
      companyId: COMPANY_ID,
  ...
  it("stops new non-default observations after the per-space profile is disabled", async () => {
    ...
    const nonDefaultCursorWrites = harness.dbExecutes.filter((execute) =>
      execute.sql.includes("dealdesk_distillation_cursors") && execute.params?.[10] === created.space.id);
    expect(nonDefaultCursorWrites).toHaveLength(1);
  });

  it("assembles deterministic DealDesk source bundles with issue, document, and comment provenance", async () => {
    const harness = createTestHarness({ manifest });
    const root = dealDeskIssue({
    ...
    }>("assemble-dealdesk-source-bundle", {
      companyId: COMPANY_ID,
      rootIssueId: root.id,
      maxCharacters: 20000,
    });
    const second = await harness.performAction<typeof first>("assemble-dealdesk-source-bundle", {
      companyId: COMPANY_ID,
      rootIssueId: root.id,
      maxCharacters: 20000,
    });
  ...
  it("suppresses secret-like comment and document bodies before storing distillation snapshots", async () => {
    ...
    }>("create-dealdesk-distillation-run", {
      companyId: COMPANY_ID,
    ...
    const snapshotInsert = harness.dbExecutes.find((execute) => execute.sql.includes("dealdesk_source_snapshots"));
  ...
  it("creates source snapshots and only advances cursors after successful distillation outcomes", async () => {
    ...
    expect(harness.dbExecutes.some((execute) => execute.sql.includes("dealdesk_source_snapshots"))).toBe(true);

    const failed = await harness.performAction<{ cursorAdvanced: boolean }>("record-dealdesk-distillation-outcome", {
      companyId: COMPANY_ID,
      runId: run.runId,
    ...
    const cursorUpdatesAfterFailure = harness.dbExecutes.filter((execute) =>
      execute.sql.trim().startsWith("UPDATE") && execute.sql.includes("dealdesk_distillation_cursors"));
    expect(cursorUpdatesAfterFailure).toHaveLength(0);

    const succeeded = await harness.performAction<{ cursorAdvanced: boolean }>("record-dealdesk-distillation-outcome", {
      companyId: COMPANY_ID,
      runId: run.runId,
      cursorId: run.cursorId,
      status: "succeeded",
  ...
  it("uses the existing distillation cursor id after an upsert conflict", async () => {
    ...
    const originalQuery = harness.ctx.db.query.bind(harness.ctx.db);
    harness.ctx.db.query = async <T,>(sql: string, params?: unknown[]) => {
      if (sql.includes("SELECT id") && sql.includes("dealdesk_distillation_cursors")) {
    ...
    await plugin.definition.setup(harness.ctx);
    const result = await harness.performAction<{ cursorId: string }>("create-dealdesk-distillation-run", {
      companyId: COMPANY_ID,
      projectId: project.id,
    });
    ...
    const runInsert = harness.dbExecutes.find((execute) =>
      execute.sql.includes("dealdesk_distillation_runs") && execute.sql.includes("'source_ready'"));
  ...
  it("creates explicit distillation work items for manual, retry, and backfill lanes", async () => {
    ...
    await harness.performAction("create-dealdesk-distillation-work-item", {
      companyId: COMPANY_ID,
      kind: "manual",
      projectId: "77777777-7777-4777-8777-777777777777",
    ...
    const workItemWrites = harness.dbExecutes.filter((execute) => execute.sql.includes("dealdesk_distillation_work_items"));
    ...
    await expect(harness.performAction("create-dealdesk-distillation-work-item", {
      companyId: COMPANY_ID,
      kind: "backfill",
  ...
  it("records estimated DealDesk distillation cost without refusing on legacy cost config", async () => {
  ...
  it("queues visible manual distill operation issues for a company-wide stale cursor scan", async () => {
    ...
    expect(result.operation.issue.description).toContain("Prompt source: LLM Wiki plugin action `distill-dealdesk-now`");
    expect(result.operation.issue.description).toContain(`Required skill: use the installed \`${DEALDESK_DISTILL_SKILL_KEY}\` skill`);
    ...
    const workItemInsert = harness.dbExecutes.find((execute) =>
      execute.sql.includes("dealdesk_distillation_work_items") && execute.params?.[3] === "manual");
    ...
    const runInsert = harness.dbExecutes.find((execute) =>
      execute.sql.includes("dealdesk_distillation_runs") && execute.sql.includes("operation_issue_id"));
  ...
  it("enables distillation cursors for the most recently active non-plugin projects", async () => {
    ...
    }>("enable-dealdesk-distillation-active-projects", {
      companyId: COMPANY_ID,
      limit: 2,
    });
  ...
  it("backfills only the selected DealDesk project and date window", async () => {
    const harness = createTestHarness({ manifest });
    const project = existingProject();
    const inWindow = dealDeskIssue({
    ...
    const outOfWindow = dealDeskIssue({
    ...
    const otherProject = dealDeskIssue({
    ...
    }>("backfill-dealdesk-distillation", {
      companyId: COMPANY_ID,
      projectId: project.id,
    ...
    const workItemInsert = harness.dbExecutes.find((execute) =>
      execute.sql.includes("dealdesk_distillation_work_items") && execute.params?.[3] === "backfill");
    expect(String(workItemInsert?.params?.[9])).toContain('"backfillStartAt":"2026-04-01T00:00:00Z"');
  });

  it("generates review-required DealDesk project page patches with provenance, index, and log updates", async () => {
    const harness = createTestHarness({ manifest });
    const project = existingProject();
    const issue = dealDeskIssue({
    ...
    expect(result.patches[3].proposedContents).toContain("dealdesk-distill | proposed");
  ...
  it("auto-applies DealDesk project page patches by default when policy allows and records page bindings", async () => {
    const harness = createTestHarness({ manifest });
    const project = existingProject();
    const issue = dealDeskIssue({
    ...
    expect(files.get("wiki/log.md")).toContain("dealdesk-distill | proposed");
    const bindingWrites = harness.dbExecutes.filter((execute) => execute.sql.includes("dealdesk_page_bindings"));
  ...
  it("refuses auto-apply DealDesk project page patches in authenticated/public deployments", async () => {
    process.env.DEALDESK_DEPLOYMENT_MODE = "authenticated";
    process.env.DEALDESK_DEPLOYMENT_EXPOSURE = "public";
  ...
  it("refuses stale project page hashes before writing generated DealDesk pages", async () => {
    const harness = createTestHarness({ manifest, config: { autoApplyIngestPatches: true } });
    const project = existingProject();
    const issue = dealDeskIssue({
    ...
    await plugin.definition.setup(harness.ctx);
    await expect(harness.performAction("distill-dealdesk-project-page", {
      companyId: COMPANY_ID,
      projectId: project.id,
      autoApply: true,
  ...
  it("skips low-signal DealDesk source windows without proposing wiki writes", async () => {
    const harness = createTestHarness({ manifest });
    const project = existingProject();
    const issue = dealDeskIssue({
    ...
    await plugin.definition.setup(harness.ctx);
    const result = await harness.performAction<{ status: string; reason: string; patches: unknown[] }>("distill-dealdesk-project-page", {
      companyId: COMPANY_ID,
      projectId: project.id,
      maxCharacters: 20000,
    });
  ...
  it("preserves manual source ingest for non-default spaces while refusing DealDesk distillation there", async () => {
    const harness = createTestHarness({ manifest });
    const writes: Array<{ path: string; contents: string }> = [];
    harness.ctx.localFolders.writeTextAtomic = async (_companyId, _folderKey, relativePath, contents) => {
      writes.push({ path: relativePath, contents });
    ...
    await expect(harness.performAction("distill-dealdesk-now", {
      companyId: COMPANY_ID,
      spaceSlug: created.space.slug,
      projectId: existingProject().id,
    })).rejects.toThrow("DealDesk ingestion policy denied queue");
  });

  it("fails closed for direct DealDesk ingestion actions against restricted spaces", async () => {
    const harness = createTestHarness({ manifest });
    harness.seed({
      agents: [wikiMaintainerAgent()],
      projects: [existingProject()],
    ...
    expect(operations).toHaveLength(0);
    expect(harness.dbExecutes.some((execute) => execute.sql.includes("dealdesk_distillation_work_items"))).toBe(false);
  });

  it("re-checks DealDesk ingestion policy at execution time for queued work", async () => {
    const harness = createTestHarness({ manifest });
    const project = existingProject();
    const issue = dealDeskIssue({
    ...
    await plugin.definition.setup(harness.ctx);
    const queued = await harness.performAction<{ status: string }>("distill-dealdesk-now", {
      companyId: COMPANY_ID,
      projectId: project.id,
    });
    ...
    await expect(harness.performAction("create-dealdesk-distillation-run", {
      companyId: COMPANY_ID,
      projectId: project.id,
    })).rejects.toThrow("personal spaces cannot ingest DealDesk sources");
    expect(harness.dbExecutes.some((execute) =>
      execute.sql.includes("dealdesk_distillation_runs") && execute.sql.includes("'source_ready'"))).toBe(false);
  });

  it("queues DealDesk ingestion backfills for every selected project scope", async () => {
    const harness = createTestHarness({ manifest });
    await plugin.definition.setup(harness.ctx);
    ...
    }>("queue-dealdesk-ingestion-backfill", {
      companyId: COMPANY_ID,
      sourceScope: {
    ...
    expect(harness.dbExecutes.filter((execute) => execute.sql.includes("dealdesk_distillation_work_items"))).toHaveLength(2);
  });

  it("rejects oversized DealDesk ingestion profile and source-scope payloads", async () => {
    const harness = createTestHarness({ manifest });
    ...
    await expect(harness.performAction("update-event-ingestion-settings", {
      ...
      maxCharacters: 20001,
      sources: { issues: true },
    })).rejects.toThrow("maxCharacters exceeds the hard DealDesk ingestion cap");

    await expect(harness.performAction("enable-dealdesk-distillation-active-projects", {
      companyId: COMPANY_ID,
      limit: 26,
    })).rejects.toThrow("fan-out exceeds the hard cap");

    await expect(harness.performAction("assemble-dealdesk-source-bundle", {
      companyId: COMPANY_ID,
      projectId: "77777777-7777-4777-8777-777777777777",
