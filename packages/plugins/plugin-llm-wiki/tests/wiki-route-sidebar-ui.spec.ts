...
const EXPANDED_STORAGE_KEY = `dealdesk.plugin-llm-wiki:route-sidebar-expanded:v2:${COMPANY_ID}`;
...
describe("WikiRouteSidebar", () => {
  ...
  beforeEach(() => {
    ...
    (globalThis as BridgeGlobal).__dealDeskPluginBridge__ = {
      sdkUi: {
  ...
  afterEach(() => {
    ...
    container.remove();
    window.localStorage.clear();
    delete (globalThis as BridgeGlobal).__dealDeskPluginBridge__;
  });
  ...
  it("keeps sidebar tree selection scoped to sidebar navigation", () => {
    ...
    expect(navigatedTo).toEqual({
      ...
      options: { state: { dealDeskWikiSidebarTreePath: "wiki/concepts/sidebar-navigation.md" } },
    });
...
describe("WikiPage", () => {
  ...
  it("renders structured DealDesk source refs as text", () => {
    act(() => {
