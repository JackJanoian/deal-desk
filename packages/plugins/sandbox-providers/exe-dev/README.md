# `@dealdesk/plugin-exe-dev`

Published exe.dev sandbox provider plugin for DealDesk.

This package lives in the DealDesk monorepo, but it is intentionally excluded from the root `pnpm` workspace and shaped to publish and install like a standalone npm package. That lets operators install it from the Plugins page by package name without introducing root lockfile churn.

## Install

From a DealDesk instance, install:
...
- When you save an environment, DealDesk stores pasted API keys and pasted SSH private keys as company secrets.
...
- SSH access from the DealDesk host to the resulting `*.exe.xyz` VMs.
...
These commands assume the repo root has already been installed once so the local `@dealdesk/plugin-sdk` workspace package is available to the compiler during development.

## Package layout

- `src/manifest.ts` declares the sandbox-provider driver metadata
- `src/plugin.ts` implements the environment lifecycle hooks
- `dealDeskPlugin.manifest` and `dealDeskPlugin.worker` point the host at the built plugin entrypoints in `dist/`
