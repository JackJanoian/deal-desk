...
// has passwordless sudo. The DealDesk sandbox callback bridge is a Node script, so
// every DealDesk workload on this provider needs node on PATH before the bridge can
// start. When the operator hasn't supplied their own setup script, install Node 20 via
// nodesource so the VM comes up ready for DealDesk out of the box.
...
function normalizeNamePrefix(value: string | null): string {
  const normalized = (value ?? "dealdesk")
    .toLowerCase()
    .replace(/[^a-z0-9-]+/g, "-")
    .replace(/^-+|-+$/g, "")
    .replace(/-{2,}/g, "-");
  return normalized.length > 0 ? normalized.slice(0, 24) : "dealdesk";
}
...
async function prepareSshIdentity(config: ExeDevDriverConfig): Promise<{
  ...
  const tempDir = await mkdtemp(path.join(tmpdir(), "dealdesk-exe-dev-ssh-"));
...
function formatSshFailure(
  ...
  if (
    ...
    return [
      `Failed to ${action} exe.dev VM ${vmName}: the DealDesk host SSH key is not registered with exe.dev.`,
...
async function buildLease(
  ...
  const remoteCwd = requestedCwd?.trim() || path.posix.join(remote.homeDir, "dealdesk-workspace");
