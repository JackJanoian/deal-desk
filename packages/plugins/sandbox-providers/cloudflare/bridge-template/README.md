...
This Worker is the operator-facing bridge used by `@dealdesk/plugin-cloudflare-sandbox`.

It exposes a small authenticated JSON API under `/api/dealdesk-sandbox/v1` and translates DealDesk lease and command requests into Cloudflare Sandbox SDK calls.
...
- clean up timed-out sessions so DealDesk does not inherit wedged background processes
...
After deploy, configure DealDesk with:
