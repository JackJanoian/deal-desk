...
function buildHeaders(config: CloudflareDriverConfig, extra: BridgeClientHeaders = {}): Headers {
  ...
  if (extra.environmentId) headers.set("X-DealDesk-Environment-Id", extra.environmentId);
  if (extra.runId) headers.set("X-DealDesk-Run-Id", extra.runId);
  if (extra.issueId) headers.set("X-DealDesk-Issue-Id", extra.issueId);
  return headers;
}
...
export function createCloudflareBridgeClient(options: BridgeClientOptions) {
  ...
  const apiPrefix = "/api/dealdesk-sandbox/v1";
