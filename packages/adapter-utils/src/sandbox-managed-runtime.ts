...
export async function prepareSandboxManagedRuntime(input: {
  ...
  const runtimeRootDir = path.posix.join(workspaceRemoteDir, ".dealdesk-runtime", input.adapterKey);
  const baselineSnapshot = await captureDirectorySnapshot(input.workspaceLocalDir, {
    exclude: [...new Set([".dealdesk-runtime", ...(input.preserveAbsentOnRestore ?? []), ...(input.workspaceExclude ?? [])])],
  });

  await withTempDir("dealdesk-sandbox-sync-", async (tempDir) => {
    ...
    const preservedNames = new Set([".dealdesk-runtime", ...(input.preserveAbsentOnRestore ?? [])]);
  ...
  return {
    ...
    restoreWorkspace: async () => {
      await withTempDir("dealdesk-sandbox-restore-", async (tempDir) => {
