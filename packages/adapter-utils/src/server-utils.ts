...
const DEFAULT_DEALDESK_INSTANCE_ID = "default";
const PATH_SEGMENT_RE = /^[a-zA-Z0-9_-]+$/;
const SENSITIVE_ENV_KEY = /(key|token|secret|password|passwd|authorization|cookie)/i;
const REDACTED_LOG_VALUE = "***REDACTED***";
const DEALDESK_SKILL_ROOT_RELATIVE_CANDIDATES = [
  "../../skills",
  "../../../../../skills",
];
const MATERIALIZED_SKILL_SENTINEL = ".dealdesk-materialized-skill.json";
...
export function resolveDealDeskInstanceRootForAdapter(input: {
  homeDir?: string;
  instanceId?: string;
  env?: NodeJS.ProcessEnv;
} = {}): string {
  const env = input.env ?? process.env;
  const homeRaw = input.homeDir?.trim() || env.DEALDESK_HOME?.trim();
  const homeDir = path.resolve(homeRaw ? expandHomePrefix(homeRaw) : path.resolve(os.homedir(), ".dealdesk"));
  const instanceId = input.instanceId?.trim() || env.DEALDESK_INSTANCE_ID?.trim() || DEFAULT_DEALDESK_INSTANCE_ID;
  if (!PATH_SEGMENT_RE.test(instanceId)) throw new Error(`Invalid DEALDESK_INSTANCE_ID '${instanceId}'.`);
  return path.resolve(homeDir, "instances", instanceId);
}

export const DEFAULT_DEALDESK_AGENT_PROMPT_TEMPLATE = [
  "You are agent {{agent.id}} ({{agent.name}}). Continue your DealDesk work.",
  "",
  "Execution contract:",
...
export interface DealDeskSkillEntry {
  key: string;
  runtimeName: string;
  source: string;
  required?: boolean;
...
export interface MaterializedDealDeskSkillCopyResult {
  copiedFiles: number;
...
interface PersistentSkillSnapshotOptions {
  adapterType: string;
  availableEntries: DealDeskSkillEntry[];
...
> {
  if (entry.required) {
    return {
      origin: "dealdesk_required",
      originLabel: "Required by DealDesk",
      readOnly: false,
    };
  }
  return {
    origin: "company_managed",
    originLabel: "Managed by DealDesk",
    readOnly: false,
  };
}
...
type DealDeskWakeExecutionPrincipal = {
  type: "agent" | "user" | null;
  agentId: string | null;
  userId: string | null;
};

type DealDeskWakeExecutionStage = {
  wakeRole: "reviewer" | "approver" | "executor" | null;
  stageId: string | null;
  stageType: string | null;
  currentParticipant: DealDeskWakeExecutionPrincipal | null;
  returnAssignee: DealDeskWakeExecutionPrincipal | null;
  reviewRequest: {
    instructions: string;
  } | null;
...
type DealDeskWakeContinuationSummary = {
  key: string | null;
  title: string | null;
  body: string;
  bodyTruncated: boolean;
...
type DealDeskWakeLivenessContinuation = {
  attempt: number | null;
...
type DealDeskWakeChildIssueSummary = {
  id: string | null;
  identifier: string | null;
  title: string | null;
  status: string | null;
...
type DealDeskWakeBlockerSummary = {
  id: string | null;
  identifier: string | null;
  title: string | null;
  status: string | null;
...
type DealDeskWakeTreeHoldSummary = {
  holdId: string | null;
...
type DealDeskWakePayload = {
  reason: string | null;
  issue: DealDeskWakeIssue | null;
  checkedOutByHarness: boolean;
  dependencyBlockedInteraction: boolean;
  treeHoldInteraction: boolean;
  activeTreeHold: DealDeskWakeTreeHoldSummary | null;
  unresolvedBlockerIssueIds: string[];
  unresolvedBlockerSummaries: DealDeskWakeBlockerSummary[];
  executionStage: DealDeskWakeExecutionStage | null;
  continuationSummary: DealDeskWakeContinuationSummary | null;
  livenessContinuation: DealDeskWakeLivenessContinuation | null;
  interactionKind: string | null;
  interactionStatus: string | null;
  childIssueSummaries: DealDeskWakeChildIssueSummary[];
  childIssueSummaryTruncated: boolean;
  commentIds: string[];
  latestCommentId: string | null;
  comments: DealDeskWakeComment[];
...
function normalizeDealDeskWakeIssue(value: unknown): DealDeskWakeIssue | null {
...
function normalizeDealDeskWakeComment(value: unknown): DealDeskWakeComment | null {
...
function normalizeDealDeskWakeContinuationSummary(value: unknown): DealDeskWakeContinuationSummary | null {
...
function normalizeDealDeskWakeLivenessContinuation(value: unknown): DealDeskWakeLivenessContinuation | null {
...
function normalizeDealDeskWakeChildIssueSummary(value: unknown): DealDeskWakeChildIssueSummary | null {
...
function normalizeDealDeskWakeBlockerSummary(value: unknown): DealDeskWakeBlockerSummary | null {
...
function normalizeDealDeskWakeTreeHoldSummary(value: unknown): DealDeskWakeTreeHoldSummary | null {
...
function normalizeDealDeskWakeExecutionPrincipal(value: unknown): DealDeskWakeExecutionPrincipal | null {
...
function normalizeDealDeskWakeExecutionStage(value: unknown): DealDeskWakeExecutionStage | null {
  ...
  const allowedActions = Array.isArray(stage.allowedActions)
    ...
    : [];
  const currentParticipant = normalizeDealDeskWakeExecutionPrincipal(stage.currentParticipant);
  const returnAssignee = normalizeDealDeskWakeExecutionPrincipal(stage.returnAssignee);
...
export function normalizeDealDeskWakePayload(value: unknown): DealDeskWakePayload | null {
  const payload = parseObject(value);
  const comments = Array.isArray(payload.comments)
    ? payload.comments
        .map((entry) => normalizeDealDeskWakeComment(entry))
        .filter((entry): entry is DealDeskWakeComment => Boolean(entry))
    : [];
  ...
  const commentIds = Array.isArray(payload.commentIds)
    ? payload.commentIds
        ...
        .map((entry) => entry.trim())
    : [];
  const executionStage = normalizeDealDeskWakeExecutionStage(payload.executionStage);
  const continuationSummary = normalizeDealDeskWakeContinuationSummary(payload.continuationSummary);
  const livenessContinuation = normalizeDealDeskWakeLivenessContinuation(payload.livenessContinuation);
  const childIssueSummaries = Array.isArray(payload.childIssueSummaries)
    ? payload.childIssueSummaries
        .map((entry) => normalizeDealDeskWakeChildIssueSummary(entry))
        .filter((entry): entry is DealDeskWakeChildIssueSummary => Boolean(entry))
    : [];
  ...
  const unresolvedBlockerSummaries = Array.isArray(payload.unresolvedBlockerSummaries)
    ? payload.unresolvedBlockerSummaries
        .map((entry) => normalizeDealDeskWakeBlockerSummary(entry))
        .filter((entry): entry is DealDeskWakeBlockerSummary => Boolean(entry))
    : [];

  const activeTreeHold = normalizeDealDeskWakeTreeHoldSummary(payload.activeTreeHold);
  if (comments.length === 0 && commentIds.length === 0 && childIssueSummaries.length === 0 && unresolvedBlockerIssueIds.length === 0 && unresolvedBlockerSummaries.length === 0 && !activeTreeHold && !executionStage && !continuationSummary && !livenessContinuation && !normalizeDealDeskWakeIssue(payload.issue)) {
    return null;
  }

  return {
    reason: asString(payload.reason, "").trim() || null,
    issue: normalizeDealDeskWakeIssue(payload.issue),
...
export function stringifyDealDeskWakePayload(value: unknown): string | null {
  const normalized = normalizeDealDeskWakePayload(value);
  if (!normalized) return null;
  return JSON.stringify(normalized);
}

export function readDealDeskIssueWorkModeFromContext(value: unknown): string | null {
  const context = parseObject(value);
  const issue = parseObject(context.dealDeskIssue);
  const direct = asString(issue.workMode, "").trim();
  if (direct) return direct;
  const wake = normalizeDealDeskWakePayload(context.dealDeskWake);
  return wake?.issue?.workMode ?? null;
}

export function renderDealDeskWakePrompt(
  value: unknown,
  ...
  const principalLabel = (principal: DealDeskWakeExecutionPrincipal | null) => {
  ...
  const lines = resumedSession
      ? [
        "## DealDesk Resume Delta",
        "",
        "You are resuming an existing DealDesk session.",
...
export function buildInvocationEnvForLogs(
  ...
  if (resolvedCommand) {
    merged[options.resolvedCommandEnvKey ?? "DEALDESK_RESOLVED_COMMAND"] = redactCommandTextForLogs(resolvedCommand);
  }

  return redactEnvForLogs(merged);
}

export function buildDealDeskEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const vars: Record<string, string> = {
    DEALDESK_AGENT_ID: agent.id,
    DEALDESK_COMPANY_ID: agent.companyId,
  };
  const runtimeHost = resolveHostForUrl(
    process.env.DEALDESK_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  const runtimePort = process.env.DEALDESK_LISTEN_PORT ?? process.env.PORT ?? "3100";
  const apiUrl =
    process.env.DEALDESK_RUNTIME_API_URL ??
    process.env.DEALDESK_API_URL ??
    `http://${runtimeHost}:${runtimePort}`;
  vars.DEALDESK_API_URL = apiUrl;
  return vars;
}

export function applyDealDeskWorkspaceEnv(
  env: Record<string, string>,
  input: {
  ...
  const mappings = [
    ["DEALDESK_WORKSPACE_CWD", input.workspaceCwd],
    ["DEALDESK_WORKSPACE_SOURCE", input.workspaceSource],
    ["DEALDESK_WORKSPACE_STRATEGY", input.workspaceStrategy],
    ["DEALDESK_WORKSPACE_ID", input.workspaceId],
    ["DEALDESK_WORKSPACE_REPO_URL", input.workspaceRepoUrl],
    ["DEALDESK_WORKSPACE_REPO_REF", input.workspaceRepoRef],
    ["DEALDESK_WORKSPACE_BRANCH", input.workspaceBranch],
    ["DEALDESK_WORKSPACE_WORKTREE_PATH", input.workspaceWorktreePath],
...
export function shapeDealDeskWorkspaceEnvForExecution(input: {
  workspaceCwd?: string | null;
  workspaceWorktreePath?: string | null;
  ...
  if (executionCwd === null) {
    ...
    console.warn(
      "[dealdesk] shapeDealDeskWorkspaceEnvForExecution called with executionCwd=null on a remote target; " +
...
export function refreshDealDeskWorkspaceEnvForExecution(input: {
  env: Record<string, string>;
  ...
  const shapedWorkspaceEnv = shapeDealDeskWorkspaceEnvForExecution({
  ...
  delete input.env.DEALDESK_WORKSPACE_CWD;
  delete input.env.DEALDESK_WORKSPACE_WORKTREE_PATH;
  delete input.env.DEALDESK_WORKSPACES_JSON;

  applyDealDeskWorkspaceEnv(input.env, {
  ...
  if (shapedWorkspaceEnv.workspaceHints.length > 0) {
    input.env.DEALDESK_WORKSPACES_JSON = JSON.stringify(shapedWorkspaceEnv.workspaceHints);
  }
...
export function sanitizeInheritedDealDeskEnv(baseEnv: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  const env: NodeJS.ProcessEnv = { ...baseEnv };
  for (const key of Object.keys(env)) {
    if (!key.startsWith("DEALDESK_")) continue;
    if (key === "DEALDESK_RUNTIME_API_URL") continue;
    if (key === "DEALDESK_LISTEN_HOST") continue;
    if (key === "DEALDESK_LISTEN_PORT") continue;
    delete env[key];
  }
  return env;
}
...
export async function resolveDealDeskSkillsDir(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<string | null> {
  const candidates = [
    ...DEALDESK_SKILL_ROOT_RELATIVE_CANDIDATES.map((relativePath) => path.resolve(moduleDir, relativePath)),
...
export async function listDealDeskSkillEntries(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<DealDeskSkillEntry[]> {
  const root = await resolveDealDeskSkillsDir(moduleDir, additionalCandidates);
  if (!root) return [];
  ...
  try {
    ...
    return Promise.all(dirs.map(async (entry) => {
      ...
      return {
        key: `dealdesk/dealdesk/${entry.name}`,
        runtimeName: entry.name,
        source: skillDir,
        required,
        requiredReason: required
          ? "Bundled DealDesk skills are always available for local adapters."
          : null,
      };
    }));
  } catch {
...
export function buildPersistentSkillSnapshot(
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    warnings.push(`Desired skill "${desiredSkill}" is not available from the DealDesk skills directory.`);
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      desired: true,
      ...
      sourcePath: null,
      targetPath: null,
      detail: "DealDesk cannot find this skill in the local runtime skills directory.",
...
function normalizeConfiguredDealDeskRuntimeSkills(value: unknown): DealDeskSkillEntry[] {
  ...
  for (const rawEntry of value) {
    ...
    if (isDealDeskBundledRuntimeSkillEntry(entry)) continue;
    out.push({
      key,
      runtimeName,
      source,
...
export function isDealDeskBundledRuntimeSkillEntry(entry: { sourceKind?: unknown }): boolean {
  return typeof entry.sourceKind === "string" && entry.sourceKind.trim() === "dealdesk_bundled";
}

export async function readDealDeskRuntimeSkillEntries(
  config: Record<string, unknown>,
  _moduleDir: string,
  _additionalCandidates: string[] = [],
): Promise<DealDeskSkillEntry[]> {
  return normalizeConfiguredDealDeskRuntimeSkills(config.dealDeskRuntimeSkills);
}

export async function readDealDeskSkillMarkdown(
  moduleDir: string,
  skillKey: string,
): Promise<string | null> {
...
export function readDealDeskSkillSyncPreference(config: Record<string, unknown>): {
  explicit: boolean;
  desiredSkills: string[];
} {
  const raw =
...
function canonicalizeDesiredDealDeskSkillReference(
  reference: string,
...
export function resolveDealDeskDesiredSkillNames(
  config: Record<string, unknown>,
  ...
  const desiredSkills = preference.desiredSkills
    .map((reference) => canonicalizeDesiredDealDeskSkillReference(reference, availableEntries))
    .filter(Boolean);
  return Array.from(new Set([...requiredSkills, ...desiredSkills]));
}

export function writeDealDeskSkillSyncPreference(
  config: Record<string, unknown>,
  desiredSkills: string[],
  ...
  next.dealdeskSkillSync = current;
  return next;
}

export async function ensureDealDeskSkillSymlink(
  source: string,
  target: string,
  ...
  while (true) {
    try {
      ...
      if (Date.now() >= deadline) {
        throw new Error(`Timed out waiting for DealDesk skill materialization lock at ${lockDir}`);
      }
...
export async function materializeDealDeskSkillCopy(
  source: string,
  target: string,
): Promise<MaterializedDealDeskSkillCopyResult> {
  ...
  if (!rootStat.isDirectory()) {
    throw new Error("DealDesk skills must be directories.");
  }

  const result: MaterializedDealDeskSkillCopyResult = {
    copiedFiles: 0,
    skippedSymlinks: [],
  };
...
export async function runChildProcess(
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    const rawMerged: NodeJS.ProcessEnv = {
      ...sanitizeInheritedDealDeskEnv(process.env),
      ...opts.env,
    };

    // Strip Claude Code nesting-guard env vars so spawned `claude` processes
    // don't refuse to start with "cannot be launched inside another session".
    // These vars leak in when the DealDesk server itself is started from
    // within a Claude Code session (e.g. `npx dealdesk run` in a terminal
