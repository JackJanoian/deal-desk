...
describe("sandbox adapter execution targets", () => {
  ...
  it("treats SSH targets as bridge-only", () => {
    ...
    expect(adapterExecutionTargetUsesDealDeskBridge(target)).toBe(true);
    expect(adapterExecutionTargetSessionIdentity(target)).toEqual({
      transport: "ssh",
      host: "ssh.example.test",
      port: 22,
  ...
  it("starts a localhost DealDesk bridge for sandbox targets in bridge mode", async () => {
    ...
    const apiServer = createServer((req, res) => {
      requests.push({
        ...
        runId: typeof req.headers["x-dealdesk-run-id"] === "string" ? req.headers["x-dealdesk-run-id"] : null,
      });
    ...
    const bridge = await startAdapterExecutionTargetDealDeskBridge({
      runId: "run-bridge",
      target,
      runtimeRootDir,
      adapterKey: "codex",
    ...
    try {
      ...
      expect(bridge?.env.DEALDESK_API_URL).toMatch(/^http:\/\/127\.0\.0\.1:\d+$/);
      expect(bridge?.env.DEALDESK_API_KEY).not.toBe("real-run-jwt");
      expect(bridge?.env.DEALDESK_API_BRIDGE_MODE).toBe("queue_v1");

      const response = await fetch(`${bridge!.env.DEALDESK_API_URL}/api/agents/me`, {
        headers: {
          authorization: `Bearer ${bridge!.env.DEALDESK_API_KEY}`,
