...
export interface AdapterExecutionTargetDealDeskBridgeHandle {
  env: Record<string, string>;
  stop(): Promise<void>;
}
...
function resolveDefaultDealDeskApiUrl(): string {
  const runtimeHost = resolveHostForUrl(
    process.env.DEALDESK_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  // 3100 matches the default DealDesk dev server port when the runtime does not provide one.
...
function isBridgeDebugEnabled(env: NodeJS.ProcessEnv): boolean {
  const value = env.DEALDESK_BRIDGE_DEBUG?.trim().toLowerCase();
...
export function adapterExecutionTargetUsesDealDeskBridge(
  target: AdapterExecutionTarget | null | undefined,
): boolean {
  return target?.kind === "remote";
}
  ...
  if (detectCommand) {
    ...
    if (!recheck.timedOut && recheck.exitCode === 0) {
      if (input.onLog) {
        ...
        await input.onLog(
          "stderr",
          `[dealdesk] Install command ${reason} (${installCommand}) but ${detectCommand} is on PATH; continuing.\n`,
        );
      }
      return;
    }
 ...
 * the directory state inside the environment, not on the DealDesk host.
 *
...
export function runtimeAssetDir(
  ...
  return prepared.assetDirs[key] ?? path.posix.join(fallbackRemoteCwd, ".dealdesk-runtime", key);
}
...
export async function startAdapterExecutionTargetDealDeskBridge(input: {
  runId: string;
  target: AdapterExecutionTarget | null | undefined;
...
}): Promise<AdapterExecutionTargetDealDeskBridgeHandle | null> {
  if (!adapterExecutionTargetUsesDealDeskBridge(input.target)) {
    return null;
  }
  ...
  if (hostApiToken.length === 0) {
    throw new Error("Sandbox bridge mode requires a host-side DealDesk API token.");
  }

  const runtimeRootDir =
    input.runtimeRootDir?.trim().length
      ? input.runtimeRootDir.trim()
      : path.posix.join(target.remoteCwd, ".dealdesk-runtime", input.adapterKey);
  const bridgeRuntimeDir = path.posix.join(runtimeRootDir, "dealdesk-bridge");
  ...
  await onLog(
    "stdout",
    `[dealdesk] Starting sandbox callback bridge for ${input.adapterKey} in ${bridgeRuntimeDir}.\n`,
  );
  ...
  try {
    ...
    // DEALDESK_BRIDGE_DEBUG opts into verbose stdout logs of every bridge
    ...
    worker = await startSandboxCallbackBridgeWorker({
      ...
      handleRequest: async (request) => {
        ...
        if (bridgeDebugEnabled) {
          await onLog(
            "stdout",
            `[dealdesk] Bridge proxy ${method} ${request.path}${request.query ? `?${request.query}` : ""}\n`,
          );
        }
        ...
        headers.set("x-dealdesk-run-id", input.runId);
        ...
        if (bridgeDebugEnabled) {
          await onLog(
            "stdout",
            `[dealdesk] Bridge proxy response ${response.status} for ${method} ${request.path}${request.query ? `?${request.query}` : ""}\n`,
          );
        }
        return {
  ...
  return {
    env: {
      DEALDESK_API_URL: server.baseUrl,
      DEALDESK_API_KEY: bridgeToken,
