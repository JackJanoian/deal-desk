...
async function createSshAuthArgs(
  ...
  if (config.strictHostKeyChecking) {
    if (config.knownHosts) {
      const knownHosts = await withTempFile("dealdesk-ssh-known-hosts-", config.knownHosts, 0o600);
  ...
  if (config.privateKey) {
    const privateKey = await withTempFile("dealdesk-ssh-key-", config.privateKey, 0o600);
...
async function importGitWorkspaceToSsh(input: {
  ...
  const bundleDir = await fs.mkdtemp(path.join(os.tmpdir(), "dealdesk-ssh-bundle-"));
  ...
  const tempRef = `refs/dealdesk/ssh-sync/import/${randomUUID()}`;
  ...
  try {
    ...
    const remoteSetupScript = [
      "set -e",
      `mkdir -p ${shellQuote(path.posix.join(input.remoteDir, ".dealdesk-runtime"))}`,
      `tmp_bundle=$(mktemp ${shellQuote(path.posix.join(input.remoteDir, ".dealdesk-runtime", "import-XXXXXX.bundle"))})`,
      ...
      `git -C ${shellQuote(input.remoteDir)} clean -fdx -e .dealdesk-runtime >/dev/null`,
...
async function exportGitWorkspaceFromSsh(input: {
  ...
  const bundleDir = await fs.mkdtemp(path.join(os.tmpdir(), "dealdesk-ssh-bundle-"));
  const bundlePath = path.join(bundleDir, "workspace.bundle");
  const importedRef = input.importedRef ?? `refs/dealdesk/ssh-sync/imported/${randomUUID()}`;

  try {
    const exportScript = [
      "set -e",
      `git -C ${shellQuote(input.remoteDir)} update-ref refs/dealdesk/ssh-sync/export HEAD`,
      `mkdir -p ${shellQuote(path.posix.join(input.remoteDir, ".dealdesk-runtime"))}`,
      `tmp_bundle=$(mktemp ${shellQuote(path.posix.join(input.remoteDir, ".dealdesk-runtime", "export-XXXXXX.bundle"))})`,
      'cleanup() { rm -f "$tmp_bundle"; git -C ' + shellQuote(input.remoteDir) + ' update-ref -d refs/dealdesk/ssh-sync/export >/dev/null 2>&1 || true; }',
      'trap cleanup EXIT',
      `git -C ${shellQuote(input.remoteDir)} bundle create "$tmp_bundle" refs/dealdesk/ssh-sync/export >/dev/null`,
      'cat "$tmp_bundle"',
    ].join("\n");
    ...
    await runLocalGit(input.localDir, ["fetch", "--force", bundlePath, `refs/dealdesk/ssh-sync/export:${importedRef}`], {
      timeout: 60_000,
      maxBuffer: 1024 * 1024,
    });
...
async function integrateImportedGitHead(input: {
  ...
  const mergeCommit = await runLocalGit(
    ...
    [
      ...
      input.importedHead,
      "-m",
      `DealDesk SSH sync merge ${input.importedHead.slice(0, 12)}`,
    ],
    {
      timeout: 60_000,
      maxBuffer: 64 * 1024,
...
export async function getSshEnvLabSupport(): Promise<SshEnvLabSupport> {
  if (process.platform === "darwin" && process.env.DEALDESK_ENABLE_DARWIN_SSH_ENV_LAB !== "1") {
    return {
      supported: false,
      reason: "SSH env-lab fixture is disabled on macOS; set DEALDESK_ENABLE_DARWIN_SSH_ENV_LAB=1 to opt in.",
    };
  }
...
export async function syncDirectoryFromSsh(input: {
  ...
  const auth = await createSshAuthArgs(input.spec);
  const stagingDir = await fs.mkdtemp(path.join(os.tmpdir(), "dealdesk-ssh-sync-back-"));
...
export async function prepareWorkspaceForSshExecution(input: {
  ...
  await clearRemoteDirectory({
    spec: input.spec,
    remoteDir,
    preserveEntries: [".dealdesk-runtime"],
  });
  await syncDirectoryToSsh({
    spec: input.spec,
    localDir: input.localDir,
    remoteDir,
    exclude: [".dealdesk-runtime"],
  });
...
export async function restoreWorkspaceFromSshExecution(input: {
  ...
  if (input.baselineSnapshot) {
    ...
    const importedRef = input.restoreGitHistory
      ? `refs/dealdesk/ssh-sync/imported/${randomUUID()}`
      : null;
    try {
