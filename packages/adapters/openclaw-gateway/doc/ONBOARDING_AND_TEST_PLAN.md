...
## Scope
This plan is now **gateway-only**. DealDesk supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses DealDesk skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a DealDesk task.
...
```bash
pnpm --filter @dealdesk/server test:run -- openclaw-gateway
pnpm --filter @dealdesk/server typecheck
pnpm --filter @dealdesk/ui typecheck
pnpm --filter dealdesk typecheck
```
