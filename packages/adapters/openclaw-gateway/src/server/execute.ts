...
const DEFAULT_CLIENT_VERSION = "dealdesk";
...
export function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "dealdesk";
  if (input.strategy === "run") {
    return prefixSessionKeyForAgent(`dealdesk:run:${input.runId}`, input.agentId);
  }
  if (input.strategy === "issue" && input.issueId) {
    return prefixSessionKeyForAgent(`dealdesk:issue:${input.issueId}`, input.agentId);
  }
...
function resolveDealDeskApiUrlOverride(value: unknown): string | null {
...
const DEFAULT_CLAIMED_API_KEY_PATH = "~/.openclaw/workspace/dealdesk-claimed-api-key.json";

function resolveClaimedApiKeyPath(value: unknown): string {
  return nonEmpty(value) ?? DEFAULT_CLAIMED_API_KEY_PATH;
}

function buildDealDeskEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const dealDeskApiUrlOverride = resolveDealDeskApiUrlOverride(ctx.config.dealDeskApiUrl);
  const dealDeskEnv: Record<string, string> = {
    ...buildDealDeskEnv(ctx.agent),
    DEALDESK_RUN_ID: ctx.runId,
  };

  if (dealDeskApiUrlOverride) {
    dealDeskEnv.DEALDESK_API_URL = dealDeskApiUrlOverride;
  }
  if (wakePayload.taskId) dealDeskEnv.DEALDESK_TASK_ID = wakePayload.taskId;
  const issueWorkMode = readDealDeskIssueWorkModeFromContext(ctx.context);
  if (issueWorkMode) dealDeskEnv.DEALDESK_ISSUE_WORK_MODE = issueWorkMode;
  if (wakePayload.wakeReason) dealDeskEnv.DEALDESK_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) dealDeskEnv.DEALDESK_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) dealDeskEnv.DEALDESK_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) dealDeskEnv.DEALDESK_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    dealDeskEnv.DEALDESK_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }
...
function buildWakeText(
  payload: WakePayload,
  dealDeskEnv: Record<string, string>,
  structuredWakePrompt: string,
): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/dealdesk-claimed-api-key.json";
  const orderedKeys = [
    "DEALDESK_RUN_ID",
    "DEALDESK_AGENT_ID",
    "DEALDESK_COMPANY_ID",
    "DEALDESK_API_URL",
    "DEALDESK_TASK_ID",
    "DEALDESK_WAKE_REASON",
    "DEALDESK_WAKE_COMMENT_ID",
    "DEALDESK_APPROVAL_ID",
    "DEALDESK_APPROVAL_STATUS",
    "DEALDESK_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = dealDeskEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = dealDeskEnv.DEALDESK_API_URL ?? "<set DEALDESK_API_URL>";

  const lines = [
    "DealDesk wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `DEALDESK_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load DEALDESK_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $DEALDESK_API_KEY on every API call.",
    "- Use X-DealDesk-Run-Id: $DEALDESK_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: DEALDESK_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$DEALDESK_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\",\"in_review\"]}",
    ...
    "   - GET /api/companies/$DEALDESK_COMPANY_ID/issues?assigneeAgentId=$DEALDESK_AGENT_ID&status=todo,in_progress,in_review,blocked",
...
function buildStandardDealDeskPayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  dealDeskEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const templateDealDesk = parseObject(payloadTemplate.dealDesk);
  const workspace = asRecord(ctx.context.dealDeskWorkspace);
  const workspaces = Array.isArray(ctx.context.dealDeskWorkspaces)
    ? ctx.context.dealDeskWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.dealDeskRuntimeServiceIntents)
    ? ctx.context.dealDeskRuntimeServiceIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];

  const standardDealDesk: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: dealDeskEnv.DEALDESK_API_URL ?? null,
  };
  const structuredWake = parseObject(ctx.context.dealDeskWake);
  if (Object.keys(structuredWake).length > 0) {
    standardDealDesk.wake = structuredWake;
  }

  if (workspace) {
    standardDealDesk.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standardDealDesk.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standardDealDesk.workspaceRuntime = {
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "dealdesk-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const dealDeskEnv = buildDealDeskEnvForWake(ctx, wakePayload);
  const structuredWakePrompt = renderDealDeskWakePrompt(ctx.context.dealDeskWake);
  const structuredWakeJson = stringifyDealDeskWakePayload(ctx.context.dealDeskWake);
  ...
  const dealDeskPayload = buildStandardDealDeskPayload(ctx, wakePayload, dealDeskEnv, payloadTemplate);
  ...
  agentParams.dealDesk = dealDeskPayload;
