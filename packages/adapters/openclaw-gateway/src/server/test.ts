...
import { asString, parseObject } from "@dealdesk/adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the DealDesk server host.",
        });
      }
    } catch (err) {
      checks.push({
