...
Use when:
- You want DealDesk to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the DealDesk server.
...
- dealDeskApiUrl (string, optional): absolute DealDesk base URL advertised in wake text
- claimedApiKeyPath (string, optional): path to the claimed API key JSON file read by the agent at wake time (default ~/.openclaw/workspace/dealdesk-claimed-api-key.json)

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default dealdesk)

Standard outbound payload additions:
- dealdesk (object): standardized DealDesk context added to every gateway agent request
- dealdesk.workspace (object, optional): resolved execution workspace for this run
- dealdesk.workspaces (array, optional): additional workspace hints DealDesk exposed to the run
- dealdesk.workspaceRuntime (object, optional): reserved workspace runtime metadata when explicitly supplied outside normal heartbeat execution
