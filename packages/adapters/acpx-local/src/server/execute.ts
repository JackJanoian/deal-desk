...
import type { AdapterExecutionContext, AdapterExecutionResult } from "@dealdesk/adapter-utils";
import { readAdapterExecutionTarget, adapterExecutionTargetSessionIdentity } from "@dealdesk/adapter-utils/execution-target";
import {
...
function defaultDealDeskInstanceDir(): string {
  const home = process.env.DEALDESK_HOME?.trim() || path.join(os.homedir(), ".dealdesk");
  const instanceId = process.env.DEALDESK_INSTANCE_ID?.trim() || "default";
  return resolveDealDeskInstanceRootForAdapter({
    homeDir: home,
    instanceId,
  });
}
...
function defaultStateDir(companyId: string, agentId: string): string {
  return path.join(defaultDealDeskInstanceDir(), "companies", companyId, "acpx-local", "agents", agentId);
}

function resolveManagedCodexHomeDir(companyId: string): string {
  return path.join(defaultDealDeskInstanceDir(), "companies", companyId, "codex-home");
}
...
async function prepareManagedCodexHome(input: {
  ...
  await onLog(
    "stdout",
    `[dealdesk] Using DealDesk-managed ACPX Codex home "${targetHome}" (seeded from "${sourceHome}").\n`,
  );
  return targetHome;
}
...
async function buildSkillSetKey(input: {
  skills: DealDeskSkillEntry[];
  label: string;
}): Promise<string> {
  const hash = createHash("sha256");
  hash.update(`dealdesk-acpx-${input.label}-skills:v1\n`);
...
): Promise<{ allSkills: DealDeskSkillEntry[]; selectedSkills: DealDeskSkillEntry[]; desiredSkillNames: string[] }> {
  const allSkills = await readDealDeskRuntimeSkillEntries(config, __moduleDir);
  const desiredSkillNames = resolveDealDeskDesiredSkillNames(config, allSkills);
...
async function prepareClaudeSkillRuntime(input: {
  ...
  for (const entry of selectedSkills) {
    ...
    try {
      const result = await materializeDealDeskSkillCopy(entry.source, target);
      if (result.skippedSymlinks.length > 0) {
        await input.onLog(
          "stdout",
          `[dealdesk] Materialized ACPX Claude skill "${entry.runtimeName}" into ${skillsHome} and skipped ${result.skippedSymlinks.length} symlink(s).\n`,
        );
      }
    } catch (err) {
      await input.onLog(
        "stderr",
        `[dealdesk] Failed to materialize ACPX Claude skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
  ...
  const promptInstructions = selectedSkills.length > 0
    ? [
        "DealDesk has materialized selected runtime skills for this ACPX Claude session.",
  ...
  return {
    ...
    commandNotes: selectedSkills.length > 0
      ? [`Materialized ${selectedSkills.length} DealDesk skill(s) for ACPX Claude at ${skillsHome}.`]
      : [],
  };
}

async function readManagedCodexSkillsManifest(skillsHome: string): Promise<Set<string>> {
  const manifestPath = path.join(skillsHome, DEALDESK_MANAGED_CODEX_SKILLS_MANIFEST);
  try {
...
async function writeManagedCodexSkillsManifest(skillsHome: string, skillNames: Iterable<string>): Promise<void> {
  ...
  await fs.writeFile(
    path.join(skillsHome, DEALDESK_MANAGED_CODEX_SKILLS_MANIFEST),
...
async function reconcileManagedCodexSkills(input: {
  skillsHome: string;
  allSkills: DealDeskSkillEntry[];
  selectedSkills: DealDeskSkillEntry[];
  onLog: AdapterExecutionContext["onLog"];
}): Promise<void> {
  ...
  for (const name of managed) {
    ...
    if (await removeSkillTarget(path.join(input.skillsHome, name))) {
      await input.onLog("stdout", `[dealdesk] Revoked ACPX Codex skill "${name}" from ${input.skillsHome}\n`);
    }
  }
  ...
  for (const entry of input.allSkills) {
    ...
    if (await removeSkillTarget(target)) {
      await input.onLog("stdout", `[dealdesk] Revoked legacy ACPX Codex skill "${entry.runtimeName}" from ${input.skillsHome}\n`);
    }
  }
  ...
  for (const name of managed) {
    ...
    if (await removeSkillTarget(path.join(input.skillsHome, name))) {
      await input.onLog("stdout", `[dealdesk] Revoked unavailable ACPX Codex skill "${name}" from ${input.skillsHome}\n`);
    }
  }
}
  ...
  for (const entry of selectedSkills) {
    ...
    try {
      ...
      if (result.skippedSymlinks.length > 0) {
        await input.onLog(
          "stdout",
          `[dealdesk] Materialized ACPX Codex skill "${entry.runtimeName}" into ${skillsHome} and skipped ${result.skippedSymlinks.length} symlink(s).\n`,
        );
      }
    } catch (err) {
      await input.onLog(
        "stderr",
        `[dealdesk] Failed to inject ACPX Codex skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
...
async function buildRuntime(input: {
  ...
  const env: Record<string, string> = { ...buildDealDeskEnv(agent), DEALDESK_RUN_ID: runId };
  const wakeTaskId =
  ...
  if (!hasExplicitApiKey && authToken) env.DEALDESK_API_KEY = authToken;
  ...
  if (acpxAgent === "claude") {
    ...
    const desired = resolveDealDeskDesiredSkillNames(config, await readDealDeskRuntimeSkillEntries(config, __moduleDir));
    skillsIdentity = { mode: "custom_unsupported", desiredSkillNames: desired };
    if (desired.length > 0) {
      skillCommandNotes.push("Selected DealDesk skills are tracked only; ACPX custom commands do not expose a runtime skill contract yet.");
    }
  }
...
async function applySessionConfigOptions(input: {
  ...
  if (!input.runtime.setConfigOption) {
    ...
    await input.onLog("stderr", `[dealdesk] ${message}\n`);
  ...
  for (const option of options) {
    ...
    await input.onLog(
      "stdout",
      `[dealdesk] Applied ACPX ${input.prepared.acpxAgent} config ${option.key}=${option.value}\n`,
    );
  }
}
...
async function buildPrompt(ctx: AdapterExecutionContext, resumedSession: boolean): Promise<{
  ...
  const renderedBootstrapPrompt =
    !resumedSession && bootstrapPromptTemplate.trim().length > 0
      ...
      : "";
  const wakePrompt = renderDealDeskWakePrompt(context.dealDeskWake, { resumedSession });
  ...
  const taskContextNote = asString(context.dealDeskTaskMarkdown, "").trim();
...
export function createAcpxLocalExecutor(deps: ExecuteDeps = {}) {
  ...
  return async function executeAcpxLocal(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
    ...
    if (!canResume && asString(previousParams.runtimeSessionName, "")) {
      await ctx.onLog(
        "stdout",
        `[dealdesk] ACPX session "${asString(previousParams.runtimeSessionName, "")}" does not match the current agent/cwd/mode/runtime identity; starting fresh in "${prepared.cwd}".\n`,
      );
    }
    ...
    try {
      if (!handle) {
        try {
          ...
          await ctx.onLog(
            "stdout",
            `[dealdesk] ACPX resume session "${resumeSessionId}" is unavailable; retrying with a fresh session.\n`,
          );
          handle = await runtime.ensureSession({
            sessionKey: prepared.sessionKey,
            agent: prepared.acpxAgent,
    ...
    if (ctx.onMeta) {
      await ctx.onMeta({
        ...
        cwd: prepared.cwd,
        commandNotes: [
          `ACPX runtime embedded in DealDesk with ${prepared.mode} session mode.`,
