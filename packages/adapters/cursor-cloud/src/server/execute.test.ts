...
describe("cursor_cloud execute", () => {
  ...
  it("creates a fresh Cursor agent and injects DealDesk env without CURSOR_API_KEY", async () => {
    ...
    expect(createMock.mock.calls[0]?.[0]).toMatchObject({
      ...
      name: "DealDesk Cursor Cloud Agent",
      model: { id: "gpt-5.4" },
      cloud: {
        env: { type: "cloud" },
  ...
  it("maps non-finished Cursor results to failing DealDesk runs", async () => {
