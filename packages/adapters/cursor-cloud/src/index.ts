...
Use when:
- You want DealDesk to run Cursor Cloud Agents through the official Cursor SDK
- You want durable remote Cursor agent sessions across DealDesk heartbeats
- You want DealDesk to keep task state while Cursor handles remote code execution
...
Notes:
- DealDesk reuses the durable Cursor agent across heartbeats when the repo/runtime identity still matches.
- Each DealDesk heartbeat maps to a Cursor run on that durable agent.
- DealDesk injects DEALDESK_* runtime env vars into the cloud agent shell through Cursor SDK cloud envVars.
- DealDesk remains the source of truth for issue/task state; Cursor provides the remote execution surface.
`;
