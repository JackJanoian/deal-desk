...
Use when:
- You want DealDesk to run Pi (the AI coding agent) locally as the agent runtime
...
- DealDesk requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/dealdesks/ and resumed with --session.
