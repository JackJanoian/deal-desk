...
import { resolveDealDeskInstanceRootForAdapter } from "@dealdesk/adapter-utils/server-utils";
...
function isWorktreeMode(env: NodeJS.ProcessEnv): boolean {
  return TRUTHY_ENV_RE.test(env.DEALDESK_IN_WORKTREE ?? "");
}
...
export function resolveManagedCodexHomeDir(
  ...
  const instanceRoot = resolveDealDeskInstanceRootForAdapter({
    homeDir: nonEmpty(env.DEALDESK_HOME) ?? undefined,
    instanceId: nonEmpty(env.DEALDESK_INSTANCE_ID) ?? undefined,
    env,
  });
  return companyId
...
export async function prepareManagedCodexHome(
  ...
  if (seedFromShared) {
    ...
    await onLog(
      "stdout",
      `[dealdesk] Using ${isWorktreeMode(env) ? "worktree-isolated" : "DealDesk-managed"} Codex home "${targetHome}" (seeded from "${sourceHome}").\n`,
    );
  }
  ...
  if (apiKey) {
    ...
    await onLog(
      "stdout",
      `[dealdesk] Wrote API-key auth.json into Codex home "${targetHome}" from configured OPENAI_API_KEY.\n`,
    );
  }
