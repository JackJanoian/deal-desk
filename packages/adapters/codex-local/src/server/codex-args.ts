import { asBoolean, asString, asStringArray } from "@dealdesk/adapter-utils/server-utils";
import {
  ...
  return {
    ...
    fastModeIgnoredReason:
      fastModeRequested && !fastModeApplied
        ? `Configured fast mode is currently only supported on ${formatFastModeSupportedModels()}; DealDesk will ignore it for model ${model || "(default)"}.`
        : null,
  };
}
