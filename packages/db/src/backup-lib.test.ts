...
async function createTempDatabase(): Promise<string> {
  const db = await startEmbeddedPostgresTestDatabase("dealdesk-db-backup-");
...
describe("createBufferedTextFileWriter", () => {
  it("preserves line boundaries across buffered flushes", async () => {
    const tempDir = createTempDir("dealdesk-buffered-writer-");
...
describeEmbeddedPostgres("runDatabaseBackup", () => {
  it(
    ...
    async () => {
      ...
      const restoreConnectionString = await createSiblingDatabase(
        ...
        "dealdesk_restore_target",
      );
      const backupDir = createTempDir("dealdesk-db-backup-output-");
      ...
      try {
        ...
        const result = await runDatabaseBackup({
          ...
          filenamePrefix: "dealdesk-test",
          backupEngine: "javascript",
        });

        expect(result.backupFile).toMatch(/dealdesk-test-.*\.sql\.gz$/);
  ...
  it(
    ...
    async () => {
      ...
      const restoreConnectionString = await createSiblingDatabase(
        sourceConnectionString,
        "dealdesk_full_logical_restore_target",
      );
      const backupDir = createTempDir("dealdesk-db-full-logical-backup-");
      const sourceSql = postgres(sourceConnectionString, { max: 1, onnotice: () => {} });
      const restoreSql = postgres(restoreConnectionString, { max: 1, onnotice: () => {} });
      ...
      try {
        await sourceSql.unsafe(`
          ...
          VALUES ('dealdesk-migration-history', 1770000000000);
        `);
        await sourceSql.unsafe(`
        ...
        const result = await runDatabaseBackup({
          ...
          backupDir,
          retention: { dailyDays: 7, weeklyWeeks: 4, monthlyMonths: 1 },
          filenamePrefix: "dealdesk-full-logical-test",
          backupEngine: "javascript",
        ...
        const migrationRows = await restoreSql.unsafe<{ hash: string }[]>(`
          ...
          WHERE "hash" = 'dealdesk-migration-history'
        `);
        expect(migrationRows).toEqual([{ hash: "dealdesk-migration-history" }]);
  ...
  it(
    ...
    async () => {
      ...
      const restoreSql = postgres(restoreConnectionString, { max: 1, onnotice: () => {} });
      const backupDir = createTempDir("dealdesk-db-restore-manual-");
      ...
      try {
        await fs.promises.writeFile(
          backupFile,
          [
            "-- DealDesk database backup",
            "-- Created: 2026-04-06T00:00:00.000Z",
            "",
            "BEGIN;",
            "-- dealdesk statement breakpoint 69f6f3f1-42fd-46a6-bf17-d1d85f8f3900",
