...
export type ResolvedDatabaseTarget =
  | {
      mode: "postgres";
      connectionString: string;
      source: "DATABASE_URL" | "dealdesk-env" | "config.database.connectionString";
      configPath: string;
      envPath: string;
    }
  | {
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".dealdesk", CONFIG_BASENAME);
...
function resolveDealDeskConfigPath(): string {
  if (process.env.DEALDESK_CONFIG?.trim()) {
    return path.resolve(process.env.DEALDESK_CONFIG.trim());
  }
  return findConfigFileFromAncestors(process.cwd()) ?? resolveDealDeskConfigPathForInstance();
}

function resolveDealDeskEnvPath(configPath: string): string {
  return resolveDealDeskEnvPathForConfig(configPath);
}
...
export function resolveDatabaseTarget(): ResolvedDatabaseTarget {
  ...
  const envPath = resolveDealDeskEnvPath(configPath);
