/**
 * esbuild configuration for building the dealdesk CLI for npm.
 *
 * Bundles all workspace packages (@dealdesk/*) into a single file.
...
// to npm and resolved at runtime (e.g. @dealdesk/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@dealdesk/")) {
      externals.add(name);
    }
  }
