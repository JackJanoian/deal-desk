...
function createExistingConfigFixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "dealdesk-onboard-"));
  const runtimeRoot = path.join(root, "runtime");
  const configPath = path.join(root, ".dealdesk", "config.json");
...
function createFreshConfigPath() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "dealdesk-onboard-fresh-"));
  return path.join(root, ".dealdesk", "config.json");
}
...
describe("onboard", () => {
  beforeEach(() => {
    ...
    delete process.env.DEALDESK_BIND;
    delete process.env.DEALDESK_BIND_HOST;
    delete process.env.DEALDESK_TAILNET_BIND_HOST;
  ...
  it("keeps --yes onboarding on local trusted loopback defaults", async () => {
    ...
    process.env.DEALDESK_BIND = "lan";

    await onboard({ config: configPath, yes: true, invokedByRun: true });

    const raw = JSON.parse(fs.readFileSync(configPath, "utf8")) as DealDeskConfig;
  ...
  it("creates instance-root config and data paths for a fresh DEALDESK_HOME", async () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "dealdesk-onboard-home-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "dealdesk-onboard-cwd-"));
    process.chdir(cwd);
  ...
  it("supports authenticated/private quickstart bind presets", async () => {
    const configPath = createFreshConfigPath();
    process.env.DEALDESK_TAILNET_BIND_HOST = "100.64.0.8";
