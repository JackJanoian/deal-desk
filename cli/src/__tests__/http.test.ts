...
import { ApiConnectionError, ApiRequestError, DealDeskApiClient } from "../client/http.js";

describe("DealDeskApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new DealDeskApiClient({
    ...
    expect(headers["x-dealdesk-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new DealDeskApiClient({ apiBase: "http://localhost:3100" });
