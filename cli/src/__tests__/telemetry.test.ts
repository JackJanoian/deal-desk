...
describe("cli telemetry", () => {
  ...
  it("respects telemetry.enabled=false from the config file", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "dealdesk-cli-telemetry-"));
    const configPath = makeConfigPath(root, false);
    process.env.DEALDESK_HOME = path.join(root, "home");
    process.env.DEALDESK_INSTANCE_ID = "telemetry-test";
