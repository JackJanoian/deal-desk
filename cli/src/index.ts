...
import { costsBackfillCommand } from "./commands/costs-backfill.js";
import { cliVersion } from "./version.js";

const program = new Command();
const DATA_DIR_OPTION_HELP =
  "DealDesk data directory root (isolates state from ~/.dealdesk)";

program
  .name("dealdesk")
  .description("DealDesk CLI — setup, diagnose, and configure your instance")
  .version(cliVersion);
...
program.hook("preAction", (_thisCommand, actionCommand) => {
  ...
  loadDealDeskEnvFile(options.config);
...
program
  ...
  .option("--run", "Start DealDesk immediately after saving config", false)
  .action(onboard);

program
  .command("doctor")
  .description("Run diagnostic checks on your DealDesk setup")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
program
  ...
  .option("--filename-prefix <prefix>", "Backup filename prefix", "dealdesk")
...
program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run DealDesk")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the DealDesk server API")
...
const costs = program.command("costs").description("Cost ledger maintenance commands");
costs
  .command("backfill")
  .description("Backfill cost_events from historical heartbeat runs")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
  .requiredOption("--company <companyId>", "Company ID to backfill")
  .option("--dry-run", "Preview rows without inserting", false)
  .option("--limit <count>", "Maximum runs to scan", (value) => Number.parseInt(value, 10))
  .action(async (opts) => {
    await costsBackfillCommand(opts);
  });
