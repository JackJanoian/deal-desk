...
export class DealDeskApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-dealdesk-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "Could not reach the DealDesk API.",
    "",
  ...
  lines.push(
    "",
    "This usually means the DealDesk server is not running, the configured URL is wrong, or the request is being blocked before it reaches DealDesk.",
    "",
    "Try:",
    "- Start DealDesk with `pnpm dev` or `pnpm dealdesk run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If DealDesk is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`DEALDESK_API_URL\`.`,
  );
  return lines.join("\n");
}
