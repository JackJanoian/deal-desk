...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@dealdesk/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://dealdesk:dealdesk@127.0.0.1:${port}/dealdesk`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" dealdesk db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveDealDeskInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "dealdesk";
