...
import { resolveDealDeskInstanceId, resolveDealDeskInstanceRoot } from "../config/home.js";
...
export function resolveEnvLabSshStatePath(instanceId?: string): string {
  ...
  return path.resolve(
    resolveDealDeskInstanceRoot(resolvedInstanceId),
    "env-lab",
    "ssh-fixture",
    "state.json",
  );
...
export async function envLabDoctorCommand(opts: { instance?: string; json?: boolean }) {
  ...
  p.log.message(`Cleanup: ${pc.dim("pnpm dealdesk env-lab down")}`);
}
...
export function registerEnvLabCommands(program: Command) {
  ...
  envLab
    ...
    .option("-i, --instance <id>", "DealDesk instance id (default: current/default)")
