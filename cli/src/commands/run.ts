...
export async function runCommand(opts: RunOptions): Promise<void> {
  const instanceId = resolveDealDeskInstanceId(opts.instance);
  process.env.DEALDESK_INSTANCE_ID = instanceId;

  const homeDir = resolveDealDeskHomeDir();
  ...
  p.intro(pc.bgCyan(pc.black(" dealdesk run ")));
  ...
  if (!configExists(configPath)) {
    if (!process.stdin.isTTY || !process.stdout.isTTY) {
      ...
      p.log.message(`Run ${pc.cyan("dealdesk onboard")} once, then retry ${pc.cyan("dealdesk run")}.`);
      process.exit(1);
    }
  ...
  p.log.step("Starting DealDesk server...");
...
function maybeEnableUiDevMiddleware(entrypoint: string): void {
  if (process.env.DEALDESK_UI_DEV_MIDDLEWARE !== undefined) return;
  const normalized = entrypoint.replaceAll("\\", "/");
  if (normalized.endsWith("/server/src/index.ts") || normalized.endsWith("@dealdesk/server/src/index.ts")) {
    process.env.DEALDESK_UI_DEV_MIDDLEWARE = "true";
  }
}
...
function ensureDevWorkspaceBuildDeps(projectRoot: string): void {
  ...
  if (result.error) {
    throw new Error(
      `Failed to prepare workspace build artifacts before starting the DealDesk dev server.\n${formatError(result.error)}`,
    );
  }
...
async function importServerEntry(): Promise<StartedServer> {
  ...
  // Production mode: import the published @dealdesk/server package
  try {
    const mod = await import("@dealdesk/server");
    return await startServerFromModule(mod, "@dealdesk/server");
  } catch (err) {
    const missingSpecifier = getMissingModuleSpecifier(err);
    const missingServerEntrypoint = !missingSpecifier || missingSpecifier === "@dealdesk/server";
    if (isModuleNotFoundError(err) && missingServerEntrypoint) {
      throw new Error(
        `Could not locate a DealDesk server entrypoint.\n` +
          `Tried: ${devEntry}, @dealdesk/server\n` +
          `${formatError(err)}`,
      );
    }
    throw new Error(
      `DealDesk server failed to start.\n` +
        `${formatError(err)}`,
    );
  }
}

function shouldGenerateBootstrapInviteAfterStart(config: DealDeskConfig): boolean {
...
async function startServerFromModule(mod: unknown, label: string): Promise<StartedServer> {
  ...
  if (typeof startServer !== "function") {
    throw new Error(`DealDesk server entrypoint did not export startServer(): ${label}`);
  }
