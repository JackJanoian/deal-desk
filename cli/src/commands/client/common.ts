...
import { ApiRequestError, DealDeskApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to DealDesk config file")
    .option("-d, --data-dir <path>", "DealDesk data directory root (isolates state from ~/.dealdesk)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the DealDesk API")
...
export function resolveCommandContext(
  ...
  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.DEALDESK_API_KEY?.trim() ||
  ...
  const companyId =
    ...
    process.env.DEALDESK_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set DEALDESK_COMPANY_ID, or set context profile companyId via `dealdesk context set`.",
    );
  }

  const api = new DealDeskApiClient({
    apiBase,
    apiKey,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.DEALDESK_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.DEALDESK_SERVER_PORT || "");
