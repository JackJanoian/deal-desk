...
import type { Company, FeedbackTrace, FeedbackTraceBundle } from "@dealdesk/shared";
import {
...
export async function resolveFeedbackCompanyId(
  ...
  if (!companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set DEALDESK_COMPANY_ID, or configure a CLI context default.",
    );
  }
  return companyId;
}
...
export function renderFeedbackReport(input: {
  ...
  const lines: string[] = [];
  lines.push("");
  lines.push(pc.bold(pc.magenta("DealDesk Feedback Report")));
...
export function renderFeedbackExportSummary(exported: FeedbackExportResult): string {
  const lines: string[] = [];
  lines.push("");
  lines.push(pc.bold(pc.magenta("DealDesk Feedback Export")));
