...
export const DEFAULT_WORKTREE_HOME = "~/.dealdesk-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".dealdesk");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: DealDeskConfig | null;
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "dealdesk",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    DEALDESK_HOME: paths.homeDir,
    DEALDESK_INSTANCE_ID: paths.instanceId,
    DEALDESK_CONFIG: paths.configPath,
    DEALDESK_CONTEXT: paths.contextPath,
    DEALDESK_IN_WORKTREE: "true",
    ...(branding?.name ? { DEALDESK_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { DEALDESK_WORKTREE_COLOR: branding.color } : {}),
  };
}
