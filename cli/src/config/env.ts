...
const JWT_SECRET_ENV_KEY = "DEALDESK_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by DealDesk CLI commands",
...
export function resolveDealDeskEnvFile(configPath?: string): string {
...
export function loadDealDeskEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeDealDeskEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readDealDeskEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeDealDeskEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeDealDeskEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readDealDeskEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeDealDeskEnvEntries(next, filePath);
  return next;
}
