...
export function secretsCheck(config: DealDeskConfig, configPath?: string): CheckResult {
  ...
  if (provider !== "local_encrypted") {
    return {
      ...
      canRepair: false,
      repairHint: "Run `dealdesk configure --section secrets` and choose local_encrypted or aws_secrets_manager",
    };
  }

  const envMasterKey = process.env.DEALDESK_SECRETS_MASTER_KEY;
  if (envMasterKey && envMasterKey.trim().length > 0) {
    if (!decodeMasterKey(envMasterKey)) {
      return {
        ...
        status: "fail",
        message:
          "DEALDESK_SECRETS_MASTER_KEY is invalid (expected 32-byte base64, 64-char hex, or raw 32-char string)",
        canRepair: false,
        repairHint: "Set DEALDESK_SECRETS_MASTER_KEY to a valid key or unset it to use a key file",
      };
    }
    ...
    return withStrictModeNote(
      {
        name: "Secrets adapter",
        status: "pass",
        message: "Local encrypted provider configured via DEALDESK_SECRETS_MASTER_KEY",
      },
      config,
    );
  }

  const keyFileOverride = process.env.DEALDESK_SECRETS_MASTER_KEY_FILE;
  ...
  try {
    ...
    return {
      ...
      canRepair: false,
      repairHint: "Check file permissions or set DEALDESK_SECRETS_MASTER_KEY",
    };
  }
...
function awsSecretsManagerCheck(): CheckResult {
  ...
  if (missingConfig.length > 0) {
    return {
      ...
      canRepair: false,
      repairHint:
        `Set ${missingConfig.join(", ")} in the DealDesk server runtime. ${AWS_CREDENTIAL_SOURCE_HINT}. Do not store AWS root credentials or long-lived IAM user keys in DealDesk secrets.`,
    };
  }
  ...
  const message =
    `AWS Secrets Manager provider configured for deployment ${process.env.DEALDESK_SECRETS_AWS_DEPLOYMENT_ID}; ` +
  ...
  if (staticEnvCredentials) {
    return {
      ...
      canRepair: false,
      repairHint:
        "AWS static environment credentials are visible. Use only short-lived shell credentials locally; prefer IAM role/workload identity for hosted deployments and never store AWS access keys in DealDesk company secrets.",
    };
  }
...
function missingAwsSecretsManagerConfig(): string[] {
  ...
  if (
    ...
    missing.push("DEALDESK_SECRETS_AWS_REGION or AWS_REGION/AWS_DEFAULT_REGION");
  }
  if (!process.env.DEALDESK_SECRETS_AWS_DEPLOYMENT_ID?.trim()) {
    missing.push("DEALDESK_SECRETS_AWS_DEPLOYMENT_ID");
  }
  if (!process.env.DEALDESK_SECRETS_AWS_KMS_KEY_ID?.trim()) {
    missing.push("DEALDESK_SECRETS_AWS_KMS_KEY_ID");
  }
  return missing;
}
