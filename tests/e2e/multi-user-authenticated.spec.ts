...
const BASE = process.env.DEALDESK_E2E_BASE_URL ?? "http://127.0.0.1:3105";
...
const ownerUser: HumanUser = {
  name: "Owner User",
  email: `owner-${runId}@dealdesk.local`,
  password: OWNER_PASSWORD,
};
const invitedUser: HumanUser = {
  name: "Invited User",
  email: `invitee-${runId}@dealdesk.local`,
  password: INVITED_PASSWORD,
};

function createBootstrapInvite() {
  if (!DATA_DIR) {
    throw new Error("DEALDESK_E2E_DATA_DIR or DEALDESK_HOME is required for authenticated bootstrap tests");
  }
...
async function signUp(page: Page, user: HumanUser) {
  ...
  await expect(page.getByRole("heading", { name: "Sign in to DealDesk" })).toBeVisible();
  await page.getByRole("button", { name: "Create one" }).click();
  await expect(page.getByRole("heading", { name: "Create your DealDesk account" })).toBeVisible();
...
async function acceptBootstrapInvite(page: Page, inviteUrl: string) {
  ...
  await expect(page.getByRole("heading", { name: "Bootstrap your DealDesk instance" })).toBeVisible();
