...
const PORT = Number(process.env.DEALDESK_E2E_PORT ?? 3199);
const BASE_URL = `http://127.0.0.1:${PORT}`;
const DEALDESK_HOME = fs.mkdtempSync(path.join(os.tmpdir(), "paperclip-e2e-home-"));
...
export default defineConfig({
  ...
  // `onboard --yes --run` works in a non-interactive temp DEALDESK_HOME.
  webServer: {
    command: `pnpm dealdesk onboard --yes --run`,
    url: `${BASE_URL}/api/health`,
    // Always boot a dedicated throwaway instance for e2e so browser tests
    // never attach to the developer's active DealDesk home/server.
    ...
    env: {
      ...
      PORT: String(PORT),
      DEALDESK_HOME,
      DEALDESK_INSTANCE_ID: "playwright-e2e",
      DEALDESK_BIND: "loopback",
      DEALDESK_DEPLOYMENT_MODE: "local_trusted",
