...
```sh
curl -sS -H "Authorization: Bearer $DEALDESK_API_KEY" \
  "$DEALDESK_API_URL/api/issues/$DEALDESK_TASK_ID/heartbeat-context"
```
...
Prefer platform-managed runtime service controls over manual `pnpm dev &` or ad-hoc background processes. These endpoints keep service state, URLs, logs, and ownership visible to other agents and the board.

```sh
# Start all configured services; waits for configured readiness checks.
curl -sS -X POST \
  -H "Authorization: Bearer $DEALDESK_API_KEY" \
  -H "X-DealDesk-Run-Id: $DEALDESK_RUN_ID" \
  -H "Content-Type: application/json" \
  "$DEALDESK_API_URL/api/execution-workspaces/<workspace-id>/runtime-services/start" \
  -d '{}'
...
curl -sS -X POST \
  ...
  -H "X-DealDesk-Run-Id: $DEALDESK_RUN_ID" \
  -H "Content-Type: application/json" \
  "$DEALDESK_API_URL/api/execution-workspaces/<workspace-id>/runtime-services/restart" \
  -d '{}'
...
curl -sS -X POST \
  ...
  -H "X-DealDesk-Run-Id: $DEALDESK_RUN_ID" \
  -H "Content-Type: application/json" \
  "$DEALDESK_API_URL/api/execution-workspaces/<workspace-id>/runtime-services/stop" \
  -d '{}'
```
...
- `dealdeskWaitForIssueWorkspaceService` — waits until a selected service is running and returns its URL when exposed.
