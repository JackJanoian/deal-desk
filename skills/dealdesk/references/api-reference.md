# Deal Desk API Reference

Detailed reference for the Deal Desk control plane API. For the core heartbeat procedure and critical rules, see the main `SKILL.md`.
...
There is **no separate execution-decision endpoint**. Review and approval decisions are submitted through `PATCH /api/issues/:issueId`, and Deal Desk records the decision row automatically.
...
Deal Desk writes the execution decision automatically. If another stage remains, the issue stays in `in_review` and is reassigned to the next participant. If this was the final stage, the issue reaches actual `done`.
...
Deal Desk converts that into a `changes_requested` decision, reassigns the issue to `returnAssignee`, and routes it back to the same stage when the executor resubmits.
...
# ^ Load tests depend on caching layer being done first. Deal Desk will auto-wake agent-55 when the blocker resolves.
...
POST /api/companies/{companyId}/projects
{
  ...
  "workspace": {
    "name": "acme-mobile",
    "cwd": "/Users/me/acme-mobile",
    "repoUrl": "https://github.com/acme/acme-mobile",
    "repoRef": "main",
    "isPrimary": true
  }
}
...
Use `dealdesk-create-agent` for the full hiring workflow (reflection + config comparison + prompt drafting).
...
- Use `blockedByIssueIds` for real work dependencies between issues so Deal Desk can wake the blocked assignee when all blockers resolve.
...
| Block on another task without `blockedByIssueIds` | No automatic wake when blocker resolves; manual follow-up needed | Set `blockedByIssueIds` so Deal Desk auto-wakes the assignee when all blockers are done |
