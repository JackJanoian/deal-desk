...
```sh
curl -sS -X POST "$DEALDESK_API_URL/api/companies/$DEALDESK_COMPANY_ID/skills/import" \
  -H "Authorization: Bearer $DEALDESK_API_KEY" \
...
```sh
curl -sS -X POST "$DEALDESK_API_URL/api/companies/$DEALDESK_COMPANY_ID/skills/scan-projects" \
  -H "Authorization: Bearer $DEALDESK_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
```sh
curl -sS "$DEALDESK_API_URL/api/companies/$DEALDESK_COMPANY_ID/skills" \
  -H "Authorization: Bearer $DEALDESK_API_KEY"
```
...
```sh
curl -sS "$DEALDESK_API_URL/api/companies/$DEALDESK_COMPANY_ID/skills/<skill-id>" \
  -H "Authorization: Bearer $DEALDESK_API_KEY"

curl -sS "$DEALDESK_API_URL/api/companies/$DEALDESK_COMPANY_ID/skills/<skill-id>/files?path=SKILL.md" \
  -H "Authorization: Bearer $DEALDESK_API_KEY"
```
...
```sh
curl -sS -X POST "$DEALDESK_API_URL/api/agents/<agent-id>/skills/sync" \
  -H "Authorization: Bearer $DEALDESK_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "desiredSkills": [
...
```sh
curl -sS "$DEALDESK_API_URL/api/agents/<agent-id>/skills" \
  -H "Authorization: Bearer $DEALDESK_API_KEY"
```
...
```sh
curl -sS -X POST "$DEALDESK_API_URL/api/companies/$DEALDESK_COMPANY_ID/agent-hires" \
  -H "Authorization: Bearer $DEALDESK_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```sh
curl -sS -X POST "$DEALDESK_API_URL/api/companies/$DEALDESK_COMPANY_ID/agents" \
  -H "Authorization: Bearer $DEALDESK_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "QA Browser Agent",
...
- Agents only receive company skills that are explicitly selected; bundled platform runtime tools are not added automatically.
