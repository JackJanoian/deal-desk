...
async function ensureWorkspaceLinksCurrent(workspaceDir: string) {
  ...
  console.log(`[dealdesk] detected stale workspace package links for ${workspaceDir}; relinking dependencies...`);
  for (const mismatch of mismatches) {
    console.log(
      `[dealdesk]   ${mismatch.packageName}: ${mismatch.actualPath ?? "missing"} -> ${mismatch.expectedPath}`,
    );
  }
