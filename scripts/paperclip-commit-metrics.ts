...
const DEFAULT_QUERY = "\"Co-Authored-By: DealDesk <noreply@paperclip.ing>\"";
const DEFAULT_CACHE_FILE = path.resolve("data/dealdesk-commit-metrics-cache.json");
...
const DEALDESK_EMAIL = "noreply@paperclip.ing";
const DEALDESK_NAME = "dealdesk";
...
function printHelp() {
  console.log(`Usage: tsx scripts/dealdesk-commit-metrics.ts [options]
...
function normalizeContributor(input: {
  ...
  if ((email && email === DEALDESK_EMAIL) || (displayName && displayName.toLowerCase() === DEALDESK_NAME)) {
    return null;
  }
...
function printSummary(summary: Summary) {
  console.log("DealDesk commit metrics");
    ...
    while (true) {
      const response = await fetch(`${this.apiBase}${pathname}`, {
        headers: {
          ...
          "User-Agent": "dealdesk-commit-metrics",
