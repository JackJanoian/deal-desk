...
Environment overrides:
  ...
  PC_DATA=/tmp/data           DealDesk data dir to use
  DEALDESK_HOST=127.0.0.1    Host passed to the onboarded server
  DEALDESK_PORT=3232         Port passed to the onboarded server
...
export DEALDESK_OPEN_ON_LISTEN="${DEALDESK_OPEN_ON_LISTEN:-false}"
...
echo "DEALDESK_HOST: $DEALDESK_HOST"
echo "DEALDESK_PORT: $DEALDESK_PORT"
...
env \
  ...
  HOST="$DEALDESK_HOST" \
  PORT="$DEALDESK_PORT" \
  DEALDESK_OPEN_ON_LISTEN="$DEALDESK_OPEN_ON_LISTEN" \
  pnpm dealdesk onboard --yes --data-dir "$PC_DATA"
