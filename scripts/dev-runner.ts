...
if (worktreeEnvBootstrap.missingEnv) {
  console.error(
    `[dealdesk] linked git worktree at ${repoRoot} is missing ${path.relative(repoRoot, worktreeEnvBootstrap.envPath)}. Run \`dealdesk worktree init\` in this worktree before \`pnpm dev\`.`,
  );
  process.exit(1);
}
...
const devServerStatusFilePath = path.join(repoRoot, ".dealdesk", "dev-server-status.json");
const devServerStatusToken = mode === "dev" ? randomUUID() : null;
const devServerStatusTokenHeader = "x-dealdesk-dev-server-status-token";
...
const ignoredRelativePaths = new Set([
  ".dealdesk/dev-server-status.json",
]);
...
for (let index = 0; index < cliArgs.length; index += 1) {
  ...
  if (arg === "--bind") {
    ...
    if (!value || value.startsWith("--") || !BIND_MODES.includes(value as BindMode)) {
      console.error(`[dealdesk] invalid --bind value. Use one of: ${BIND_MODES.join(", ")}`);
      process.exit(1);
    }
  ...
  if (arg === "--bind-host") {
    ...
    if (!value || value.startsWith("--")) {
      console.error("[dealdesk] --bind-host requires a value");
      process.exit(1);
    }
    bindHost = value;
    index += 1;
...
if (bindMode === "custom" && !bindHost) {
  console.error("[dealdesk] --bind custom requires --bind-host <host>");
  process.exit(1);
}

const env: NodeJS.ProcessEnv = {
  ...process.env,
  DEALDESK_UI_DEV_MIDDLEWARE: "true",
};

if (mode === "dev") {
  env.DEALDESK_DEV_SERVER_STATUS_FILE = devServerStatusFilePath;
  env.DEALDESK_DEV_SERVER_STATUS_TOKEN = devServerStatusToken ?? "";
  env.DEALDESK_MIGRATION_AUTO_APPLY ??= "true";
}

if (mode === "watch") {
  delete env.DEALDESK_DEV_SERVER_STATUS_TOKEN;
  env.DEALDESK_MIGRATION_PROMPT ??= "never";
  env.DEALDESK_MIGRATION_AUTO_APPLY ??= "true";
}
...
if (tailscaleAuth || bindMode) {
  ...
  if (tailscaleAuth) {
    console.log("[dealdesk] note: --tailscale-auth/--authenticated-private are legacy aliases for --bind lan");
  }
  env.DEALDESK_BIND = effectiveBind;
  if (bindHost) {
    env.DEALDESK_BIND_HOST = bindHost;
  } else {
    delete env.DEALDESK_BIND_HOST;
  }
  if (effectiveBind === "loopback" && !tailscaleAuth) {
    delete env.DEALDESK_DEPLOYMENT_MODE;
    delete env.DEALDESK_DEPLOYMENT_EXPOSURE;
    delete env.DEALDESK_AUTH_BASE_URL_MODE;
    console.log("[dealdesk] dev mode: local_trusted (bind=loopback)");
  } else {
    env.DEALDESK_DEPLOYMENT_MODE = "authenticated";
    env.DEALDESK_DEPLOYMENT_EXPOSURE = "private";
    env.DEALDESK_AUTH_BASE_URL_MODE = "auto";
    console.log(
      `[dealdesk] dev mode: authenticated/private (bind=${effectiveBind}${bindHost ? `:${bindHost}` : ""})`,
    );
  }
} else {
  delete env.DEALDESK_BIND;
  delete env.DEALDESK_BIND_HOST;
  delete env.DEALDESK_DEPLOYMENT_MODE;
  delete env.DEALDESK_DEPLOYMENT_EXPOSURE;
  delete env.DEALDESK_AUTH_BASE_URL_MODE;
  console.log("[dealdesk] dev mode: local_trusted (default)");
}
...
if (existingRunner) {
  console.log(
    `[dealdesk] ${devService.serviceName} already running (pid ${existingRunner.pid}${typeof existingRunner.metadata?.childPid === "number" ? `, child ${existingRunner.metadata.childPid}` : ""})`,
  );
  process.exit(0);
}
...
async function getMigrationStatusPayload() {
  ...
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[dealdesk] Command failed with code ${status.code}: pnpm --filter @dealdesk/db exec tsx src/migration-status.ts --json\n`,
    );
  ...
  try {
    ...
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        "[dealdesk] migration-status returned invalid JSON payload\n",
    );
...
async function maybePreflightMigrations(options: { interactive?: boolean; autoApply?: boolean; exitOnDecline?: boolean } = {}) {
  ...
  const autoApply = options.autoApply ?? env.DEALDESK_MIGRATION_AUTO_APPLY === "true";
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[dealdesk] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). Refusing to start watch mode against a stale schema.\n`,
      );
      process.exit(1);
    }
    return;
...
async function buildPluginSdk() {
  console.log("[dealdesk] building plugin sdk...");
  const result = await runPnpm(
    ["--filter", "@dealdesk/plugin-sdk", "build"],
    { stdio: "inherit" },
  );
  if (result.signal) {
  ...
  if (result.code !== 0) {
    console.error("[dealdesk] plugin sdk build failed");
...
async function startServerChild() {
  ...
  child = spawn(
    pnpmBin,
    ["--filter", "@dealdesk/server", serverScript, ...forwardedArgs],
