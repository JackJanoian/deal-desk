...
DEALDESK_HOST_PORT="${DEALDESK_HOST_PORT:-3100}"
DEALDESK_HOST_FROM_CONTAINER="${DEALDESK_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.dealdesk-openclaw.override.yml"
...
detect_dealdesk_base_url() {
  ...
  for candidate in "$DEALDESK_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${DEALDESK_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${DEALDESK_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
dealdesk_base_url="$(detect_dealdesk_base_url || true)"
...
DealDesk URL for OpenClaw container:
EOF
  if [[ -n "$dealdesk_base_url" ]]; then
    cat <<EOF
  $dealdesk_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${DEALDESK_HOST_PORT}
  (Do not use http://127.0.0.1:${DEALDESK_HOST_PORT} inside the container.)
  If DealDesk rejects the host, run on host machine:
    pnpm dealdesk allowed-hostname host.docker.internal
  Then restart DealDesk and re-run this script.
EOF
  fi
  cat <<EOF
