...
DEALDESK_API_URL="${DEALDESK_API_URL:-http://127.0.0.1:3100}"
API_BASE="${DEALDESK_API_URL%/}/api"
...
DEALDESK_API_URL_FOR_OPENCLAW="${DEALDESK_API_URL_FOR_OPENCLAW:-http://host.docker.internal:3100}"
...
AUTH_HEADERS=()
if [[ -n "${DEALDESK_AUTH_HEADER:-}" ]]; then
  AUTH_HEADERS+=( -H "Authorization: ${DEALDESK_AUTH_HEADER}" )
fi
if [[ -n "${DEALDESK_COOKIE:-}" ]]; then
  AUTH_HEADERS+=( -H "Cookie: ${DEALDESK_COOKIE}" )
  DEALDESK_BROWSER_ORIGIN="${DEALDESK_BROWSER_ORIGIN:-${DEALDESK_API_URL%/}}"
  AUTH_HEADERS+=( -H "Origin: ${DEALDESK_BROWSER_ORIGIN}" -H "Referer: ${DEALDESK_BROWSER_ORIGIN}/" )
fi
...
require_board_auth() {
  if [[ ${#AUTH_HEADERS[@]} -eq 0 ]]; then
    fail "board auth required. Set DEALDESK_COOKIE or DEALDESK_AUTH_HEADER."
  fi
...
create_and_approve_gateway_join() {
  ...
  join_payload="$(jq -nc \
    ...
    --arg dealDeskApiUrl "$DEALDESK_API_URL_FOR_OPENCLAW" \
...
persist_claimed_key_artifacts() {
  ...
  if [[ -n "$container" ]]; then
    ...
    docker cp "$claimed_file" "${container}:/home/node/.openclaw/workspace/dealdesk-claimed-api-key.json" >/dev/null 2>&1 || true
    docker exec "$container" sh -lc "chmod 644 /home/node/.openclaw/workspace/dealdesk-claimed-api-key.json" >/dev/null 2>&1 || true
  fi

  if [[ "$AUTO_INSTALL_SKILL" == "1" ]]; then
    api_request "GET" "/skills/dealdesk"
    assert_status "200"
    {
      echo "DEALDESK_API_URL=${DEALDESK_API_URL_FOR_OPENCLAW}"
      echo
      printf "%s\n" "$RESPONSE_BODY"
    } > "${skill_dir}/SKILL.md"
...
inject_agent_api_key_payload_template() {
  ...
  local base_message
  base_message="Set DEALDESK_API_KEY=${AGENT_API_KEY} in your run context before running DealDesk heartbeat steps."
