...
COMPANY_ID="${COMPANY_ID:-${DEALDESK_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-dealdesk-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-dealdesk-openclaw-smoke}"
...
Provide one of:
  DEALDESK_AUTH_HEADER=\"Bearer <board-token>\"
  DEALDESK_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "DealDesk OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.dealdesk.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
