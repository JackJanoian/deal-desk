...
base_cwd="${DEALDESK_WORKSPACE_BASE_CWD:?DEALDESK_WORKSPACE_BASE_CWD is required}"
worktree_cwd="${DEALDESK_WORKSPACE_CWD:?DEALDESK_WORKSPACE_CWD is required}"
dealdesk_home="${DEALDESK_HOME:-$HOME/.dealdesk}"
dealdesk_instance_id="${DEALDESK_INSTANCE_ID:-default}"
dealdesk_dir="$worktree_cwd/.dealdesk"
worktree_config_path="$dealdesk_dir/config.json"
worktree_env_path="$dealdesk_dir/.env"
worktree_name="${DEALDESK_WORKSPACE_BRANCH:-$(basename "$worktree_cwd")}"
...
source_config_path="${DEALDESK_CONFIG:-}"
if [[ -z "$source_config_path" && ( -e "$base_cwd/.dealdesk/config.json" || -L "$base_cwd/.dealdesk/config.json" ) ]]; then
  source_config_path="$base_cwd/.dealdesk/config.json"
fi
if [[ -z "$source_config_path" ]]; then
  source_config_path="$dealdesk_home/instances/$dealdesk_instance_id/config.json"
fi
source_env_path="$(dirname "$source_config_path")/.env"

mkdir -p "$dealdesk_dir"
...
run_isolated_worktree_init() {
  ...
  if command -v pnpm >/dev/null 2>&1 && pnpm dealdesk --help >/dev/null 2>&1; then
    (
      cd "$worktree_cwd"
      pnpm dealdesk worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    )
    return 0
  fi

  if command -v dealdesk >/dev/null 2>&1; then
    (
      cd "$worktree_cwd"
      dealdesk worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    )
    return 0
  fi
...
write_fallback_worktree_config() {
  ...
  DEALDESK_DIR="$dealdesk_dir" \
  SOURCE_CONFIG_PATH="$source_config_path" \
  SOURCE_ENV_PATH="$source_env_path" \
  DEALDESK_WORKTREES_DIR="${DEALDESK_WORKTREES_DIR:-}" \
  node <<'EOF'
...
async function main() {
  ...
  const dealdeskDir = process.env.DEALDESK_DIR;
  const sourceConfigPath = process.env.SOURCE_CONFIG_PATH;
  const sourceEnvPath = process.env.SOURCE_ENV_PATH;
  const worktreeHome = path.resolve(expandHomePrefix(nonEmpty(process.env.DEALDESK_WORKTREES_DIR) ?? "~/.dealdesk-worktrees"));
  const instanceId = sanitizeInstanceId(worktreeName);
  const instanceRoot = path.resolve(worktreeHome, "instances", instanceId);
  const configPath = path.resolve(dealdeskDir, "config.json");
  const envPath = path.resolve(dealdeskDir, ".env");
  ...
  const targetConfig = {
    ...
    storage: {
      ...
      s3: {
        bucket: sourceConfig?.storage?.s3?.bucket ?? "dealdesk",
  ...
  const inlineMasterKey = nonEmpty(sourceEnvEntries.DEALDESK_SECRETS_MASTER_KEY);
  if (inlineMasterKey) {
    ...
    const sourceKeyFilePath = nonEmpty(sourceEnvEntries.DEALDESK_SECRETS_MASTER_KEY_FILE)
      ? resolveRuntimeLikePath(sourceEnvEntries.DEALDESK_SECRETS_MASTER_KEY_FILE, sourceConfigPath)
  ...
  const envLines = [
    "DEALDESK_HOME=" + JSON.stringify(worktreeHome),
    "DEALDESK_INSTANCE_ID=" + JSON.stringify(instanceId),
    "DEALDESK_CONFIG=" + JSON.stringify(configPath),
    "DEALDESK_CONTEXT=" + JSON.stringify(path.resolve(worktreeHome, "context.json")),
    "DEALDESK_IN_WORKTREE=true",
    "DEALDESK_WORKTREE_NAME=" + JSON.stringify(worktreeName),
  ];

  const agentJwtSecret = nonEmpty(sourceEnvEntries.DEALDESK_AGENT_JWT_SECRET);
  if (agentJwtSecret) {
    envLines.push("DEALDESK_AGENT_JWT_SECRET=" + JSON.stringify(agentJwtSecret));
  }
...
if [[ -e "$worktree_config_path" && -e "$worktree_env_path" ]]; then
  echo "Reusing existing isolated DealDesk worktree config at $worktree_config_path" >&2
else
  if dealdesk_command_available; then
    run_isolated_worktree_init
  else
    echo "dealdesk CLI not available in this workspace; writing isolated fallback config without DB seeding." >&2
    write_fallback_worktree_config
  fi
fi
...
if [[ -f "$worktree_cwd/package.json" && -f "$worktree_cwd/pnpm-lock.yaml" ]]; then
  ...
  if [[ "$needs_install" -eq 1 ]]; then
    backup_suffix=".dealdesk-backup-${BASHPID:-$$}"
    moved_symlink_paths=()
