...
test("parseArgs recognizes publish and skip-build flags", () => {
  assert.deepEqual(parseArgs(["@dealdesk/adapter-acpx-local", "--publish", "--skip-build"]), {
    help: false,
    selector: "@dealdesk/adapter-acpx-local",
    publish: true,
    skipBuild: true,
    otp: null,
  });
...
test("resolveTargetPackage matches by package name or dir", () => {
  const packages = [
    { dir: "packages/a", name: "@dealdesk/a", pkg: {} },
    { dir: "packages/b", name: "@dealdesk/b", pkg: {} },
  ];

  assert.equal(resolveTargetPackage("@dealdesk/a", packages).dir, "packages/a");
  assert.equal(resolveTargetPackage("./packages/b", packages).name, "@dealdesk/b");
});
