...
To link a provider-owned secret without copying the value into DealDesk, create
...
DealDesk stores the provider reference and a non-sensitive fingerprint only.
...
credentials must not be stored in DealDesk `company_secrets`.

The equivalent CLI check is:

```sh
pnpm dealdesk secrets doctor --company-id {companyId}
```
...
Remote import links existing AWS Secrets Manager entries into DealDesk as
`external_reference` secrets. Import stores provider reference metadata only; it
does not copy the remote secret plaintext into DealDesk.
...
- `duplicate`: a DealDesk secret already links the same canonical provider
...
`provider_guardrail`. AWS refs under DealDesk's own managed namespace are
blocked as external references; use the DealDesk-managed secret flow for those
resources instead.
...
DealDesk `name`, `key`, optional DealDesk `description`,
`providerVersionRef`, and sanitized `providerMetadata`. Blank descriptions are
stored as `null`; AWS provider descriptions are not copied into DealDesk
...
- `imported`: DealDesk created an active `external_reference` secret and one
...
real value into the agent process environment. DealDesk's custody guarantees
...
```sh
pnpm dealdesk secrets declarations --company-id {companyId}
```
