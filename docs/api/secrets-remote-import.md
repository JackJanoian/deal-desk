...
DealDesk `external_reference` secrets without copying plaintext into
DealDesk.
...
`ListSecrets` only and import stores a DealDesk external reference plus
...
remote metadata, or copies secret plaintext into DealDesk.
...
- `conflict`: the suggested DealDesk `name` or `key` is already in use.
...
DealDesk-managed AWS secret through a broad runtime role.
...
Each row may include an optional DealDesk `description` entered during review;
...
the DealDesk runtime role can list the AWS secret but lacks
