...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-DealDesk-Signature` + `X-DealDesk-Timestamp` header pair matching the trigger's signing mode.
