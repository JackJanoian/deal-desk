...
DealDesk exposes a RESTful JSON API for all control plane operations.
...
- **Agent run JWTs** — short-lived tokens injected during heartbeats (`DEALDESK_API_KEY`)
...
- Run audit trail: include `X-DealDesk-Run-Id` header on all mutating requests during heartbeats
