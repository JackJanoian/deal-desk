...
summary: Build, package, and distribute adapters as plugins without modifying DealDesk source
---

DealDesk supports external adapter plugins that can be installed from npm packages or local directories. External adapters work exactly like built-in adapters — they execute agents, parse output, and render transcripts — but they live in their own package and don't require changes to DealDesk's source code.
...
| Updates | Requires DealDesk release | Independent versioning |
...
```json
{
  ...
  "dependencies": {
    "@dealdesk/adapter-utils": "^2026.325.0",
    "picocolors": "^1.1.0"
  },
  "devDependencies": {
    "@types/node": "^22.0.0",
...
```ts
import type { ServerAdapterModule } from "@dealdesk/adapter-utils";
...
export async function execute(
  ...
  // 2. Build environment with DealDesk vars injected
  const env = buildDealDeskEnv(agent);
...
#### Available Helpers from `@dealdesk/adapter-utils`

| Helper | Purpose |
|--------|---------|
| `runChildProcess(command, opts)` | Spawn a child process with timeout, grace period, and streaming callbacks |
| `buildDealDeskEnv(agent)` | Inject `DEALDESK_*` environment variables |
...
```sh
# Via the DealDesk UI
# Settings → Adapters → Install from npm → "my-dealdesk-adapter"

# Or via API
curl -X POST http://localhost:3102/api/adapters \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"packageName": "my-dealdesk-adapter"}'
```
...
Local adapters are symlinked into DealDesk's adapter directory. Changes to the source are picked up on server restart.

### Via adapter-plugins.json

For development, you can also edit `~/.dealdesk/adapter-plugins.json` directly:

```json
[
  {
    "packageName": "my-dealdesk-adapter",
...
```ts
import type { AdapterSessionCodec } from "@dealdesk/adapter-utils";
...
Other DealDesk users can then install your adapter by package name from the UI or API.
