...
- Running a Python script that calls the DealDesk API
...
1. DealDesk spawns the configured command as a child process
2. Standard DealDesk environment variables are injected (`DEALDESK_AGENT_ID`, `DEALDESK_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the DealDesk API and perform work.
