...
summary: What adapters are and how they connect agents to DealDesk
---

Adapters are the bridge between DealDesk's orchestration layer and agent runtimes. Each adapter knows how to invoke a specific type of AI agent and capture its results.

## How Adapters Work

When a heartbeat fires, DealDesk:
...
| Hermes Local | `hermes_local` | Runs Hermes CLI locally (`hermes-dealdesk-adapter`) |
...
|---------|---------|----------|-------------|
| Droid Local | `@henkey/droid-dealdesk-adapter` | `droid_local` | Runs Factory Droid locally |

## External Adapters

You can build and distribute adapters as standalone packages — no changes to DealDesk's source code required. External adapters are loaded at startup via the plugin system.
...
my-adapter/
  src/
    ...
    cli/
      format-event.ts   # Terminal output for `dealdesk run --watch`
```
...
External adapters can ship a self-contained UI parser that tells the DealDesk web UI how to render their stdout. Without it, the UI uses a generic shell parser. See the [UI Parser Contract](/adapters/adapter-ui-parser) for details.
