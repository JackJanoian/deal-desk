...
DealDesk supports two runtime modes with different security profiles. Reachability is configured separately with `bind`.
...
```sh
pnpm dealdesk allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, DealDesk emits a one-time claim URL at startup:
...
```sh
DEALDESK_DEPLOYMENT_MODE=authenticated DEALDESK_BIND=lan pnpm dealdesk run
```
