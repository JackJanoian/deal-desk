...
DealDesk encrypts secrets at rest using a local master key. Agent environment variables that contain sensitive values (API keys, tokens) are stored as encrypted secret references.

## Custody Boundaries

DealDesk protects secret values up to the moment they are handed to an agent
or workload:
...
- Transport: values are decrypted server-side and injected into the agent
  ...
  immediately before the call. DealDesk does not return decrypted values to
  the board UI.
...
Once a value reaches the consuming process, DealDesk can no longer guarantee
...
At runtime, DealDesk resolves the selected secret server-side and injects the
...
matches an agent env key, the project value wins before DealDesk injects its
own `DEALDESK_*` runtime variables.
...
```
~/.dealdesk/instances/default/secrets/master.key
```

This key is auto-created during onboarding. The key never leaves your machine.
DealDesk best-effort enforces `0600` permissions when it creates or loads the
key file. `dealdesk doctor` and the provider health API warn when the file is
...
|----------|-------------|
| `DEALDESK_SECRETS_MASTER_KEY` | 32-byte key as base64, hex, or raw string |
| `DEALDESK_SECRETS_MASTER_KEY_FILE` | Custom key file path |
| `DEALDESK_SECRETS_STRICT_MODE` | Set to `true` to enforce secret refs |
...
configuration or `DEALDESK_SECRETS_STRICT_MODE=false`.

## External References

Provider-owned secrets can be linked without copying values into DealDesk by
using `managedMode: "external_reference"` plus a provider `externalRef`.
DealDesk stores metadata and a non-sensitive fingerprint, never the value.
...
For hosted DealDesk Cloud on AWS, see the AWS Secrets Manager operational
...
identity, not in DealDesk company secrets**. Allowed credential sources are
workload identity attached to the DealDesk server (instance profile, IRSA, ECS
...
can override) the deployment-level `DEALDESK_SECRETS_AWS_*` env. Bootstrap
...
DealDesk `external_reference` secrets. This is a metadata-only link: DealDesk
...
4. Select an AWS vault, search the remote inventory, and load more pages as
   needed.
5. Check the rows to import, review/edit the DealDesk name and key, then
   submit.
...
cursors. Do not expect DealDesk to crawl a whole account in the background;
load pages deliberately and retry throttled requests with backoff.

Remote import exposes AWS secret metadata visible to the DealDesk runtime
...
- Keep managed create/rotate/delete permissions scoped to the DealDesk
...
dedicated DealDesk runtime roles per environment/account, AWS vaults pointed at
...
- `local_encrypted`: back up the local master key file and the DealDesk
  ...
  key bytes.
- `aws_secrets_manager`: back up DealDesk's database for vault metadata
  ...
  configured prefix; restore by pointing the same DealDesk company at the
...
- `gcp_secret_manager` and `vault`: while these are coming soon, only the
  draft vault config exists in DealDesk. Database backups capture it. There
  is nothing to restore on the provider side until runtime support lands.

### AWS Provider Bootstrap Boundary

The AWS Secrets Manager provider cannot bootstrap itself from DealDesk
...
For DealDesk Cloud, provision the server runtime IAM role/workload identity,
KMS key, deployment prefix, and non-secret `DEALDESK_SECRETS_AWS_*` environment
...
DealDesk secrets. Bootstrap material belongs in infrastructure IAM/workload
...
```sh
pnpm dealdesk secrets migrate-inline-env --company-id <company-id>
...
Use the CLI command for normal operations because it goes through the DealDesk
...
```sh
pnpm dealdesk secrets declarations --company-id <company-id> --kind secret
```
...
the value custodian; DealDesk stores metadata and provider version references,
