...
export DEALDESK_DOMAIN=dealdesk.example.com   # your domain
...
aws ecr create-repository \
  --repository-name dealdesk-server \
...
# Build
docker build -t dealdesk-server .

# Tag and push
docker tag dealdesk-server:latest \
  $AWS_ACCOUNT_ID.dkr.ecr.$AWS_REGION.amazonaws.com/dealdesk-server:latest
...
aws rds create-db-subnet-group \
  --db-subnet-group-name dealdesk-db-subnet \
  --db-subnet-group-description "DealDesk RDS subnets" \
  --subnet-ids $SUBNET_1 $SUBNET_2

aws rds create-db-instance \
  --db-instance-identifier dealdesk-db \
...
DATABASE_URL="postgresql://dealdesk:${DB_PASSWORD}@${RDS_ENDPOINT}:5432/dealdesk"
```
...
EFS_ID=$(aws efs create-file-system \
  ...
  --encrypted \
  --tags Key=Name,Value=dealdesk-data \
...
aws secretsmanager create-secret \
  --name dealdesk/database-url \
  --secret-string "$DATABASE_URL"

aws secretsmanager create-secret \
  --name dealdesk/anthropic-api-key \
  --secret-string "YOUR_ANTHROPIC_KEY"

aws secretsmanager create-secret \
  --name dealdesk/better-auth-secret \
  --secret-string "$AUTH_SECRET"

aws secretsmanager create-secret \
  --name dealdesk/openai-api-key \
  --secret-string "YOUR_OPENAI_KEY"

aws secretsmanager create-secret \
  --name dealdesk/github-token \
...
# Task execution role
aws iam create-role \
  --role-name dealdesk-ecs-execution \
...
aws iam put-role-policy \
  ...
  --policy-document '{
    ...
    "Statement": [{
      ...
      "Resource": "arn:aws:secretsmanager:'$AWS_REGION':'$AWS_ACCOUNT_ID':secret:dealdesk/*"
    }]
  }'

# Task role (application — add permissions as needed)
aws iam create-role \
  --role-name dealdesk-ecs-task \
  --assume-role-policy-document '{
    "Version": "2012-10-17",
    "Statement": [{
      "Effect": "Allow",
...
```bash
aws ecs create-cluster --cluster-name dealdesk

aws logs create-log-group --log-group-name /ecs/dealdesk
```
...
sed -e "s|<ACCOUNT_ID>|$AWS_ACCOUNT_ID|g" \
    ...
    docker/ecs-task-definition.json > /tmp/dealdesk-task-def.json

aws ecs register-task-definition \
  --cli-input-json file:///tmp/dealdesk-task-def.json
```
...
aws ecs create-service \
  --cluster dealdesk \
  --service-name dealdesk-server \
  --task-definition dealdesk-server \
  --desired-count 1 \
  --launch-type FARGATE \
...
aws ecs describe-services \
  --cluster dealdesk \
  --services dealdesk-server \
  --query 'services[0].{desired:desiredCount,running:runningCount,status:status}'

# Check task health
aws ecs list-tasks --cluster dealdesk --service-name dealdesk-server
TASK_ARN=$(aws ecs list-tasks --cluster dealdesk --service-name dealdesk-server --query 'taskArns[0]' --output text)
aws ecs describe-tasks --cluster dealdesk --tasks $TASK_ARN \
  --query 'tasks[0].{status:lastStatus,health:healthStatus}'

# Check logs
aws logs tail /ecs/dealdesk --since 10m --follow
...
aws ecs update-service \
  --cluster dealdesk \
  --service dealdesk-server \
  --task-definition dealdesk-server:<PREVIOUS_REVISION>
```
...
```bash
aws rds stop-db-instance --db-instance-identifier dealdesk-db
aws rds start-db-instance --db-instance-identifier dealdesk-db
```
...
aws ecs update-service --cluster dealdesk --service dealdesk-server --desired-count 0
aws ecs delete-service --cluster dealdesk --service dealdesk-server --force
aws ecs delete-cluster --cluster dealdesk
...
aws rds delete-db-instance \
  --db-instance-identifier dealdesk-db \
  --final-db-snapshot-identifier dealdesk-db-final
aws rds wait db-instance-deleted --db-instance-identifier dealdesk-db
aws rds delete-db-subnet-group --db-subnet-group-name dealdesk-db-subnet
...
for s in database-url anthropic-api-key better-auth-secret openai-api-key github-token; do
  aws secretsmanager delete-secret --secret-id dealdesk/$s --force-delete-without-recovery
done
...
# 7. ECR
aws ecr delete-repository --repository-name dealdesk-server --force

# 8. IAM roles
aws iam delete-role-policy --role-name dealdesk-ecs-execution --policy-name SecretsAccess
aws iam detach-role-policy --role-name dealdesk-ecs-execution \
  --policy-arn arn:aws:iam::aws:policy/service-role/AmazonECSTaskExecutionRolePolicy
aws iam delete-role --role-name dealdesk-ecs-execution
aws iam delete-role --role-name dealdesk-ecs-task

# 9. Log group
aws logs delete-log-group --log-group-name /ecs/dealdesk
```
