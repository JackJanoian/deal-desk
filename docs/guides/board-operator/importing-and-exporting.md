...
DealDesk companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .dealdesk.yaml     # Adapter config, env inputs, routines
```
...
- **.dealdesk.yaml** holds DealDesk-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
```sh
dealdesk company export <company-id> --out ./my-export
```
...
dealdesk company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
dealdesk company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
dealdesk company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.dealdesk.yaml`
...
```sh
# From a local folder
dealdesk company import ./my-export

# From a GitHub URL
dealdesk company import https://github.com/org/repo

# From a GitHub subfolder
dealdesk company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
dealdesk company import org/repo
dealdesk company import org/repo/companies/acme
```
...
If `--target` is not specified, DealDesk infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
```sh
dealdesk company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
dealdesk company import org/company-templates/engineering-team \
  --target new \
...
```sh
dealdesk company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
dealdesk company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
dealdesk company import ./package \
  --target new \
  --yes \
  --json
```
...
DealDesk supports several GitHub URL formats:
