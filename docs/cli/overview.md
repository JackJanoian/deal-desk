...
The DealDesk CLI handles instance setup, diagnostics, and control-plane operations.
...
| Flag | Description |
|------|-------------|
| `--data-dir <path>` | Local DealDesk data root (isolates from `~/.dealdesk`) |
...
```sh
# Set defaults
pnpm dealdesk context set --api-base http://localhost:3100 --company-id <id>
...
Secret operations are available under `dealdesk secrets`:
...
Context is stored at `~/.dealdesk/context.json`.
