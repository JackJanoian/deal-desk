...
Add first-class **documents** to DealDesk as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by DealDesk workflow/docs.
...
Update the DealDesk agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/dealdesk/SKILL.md`
