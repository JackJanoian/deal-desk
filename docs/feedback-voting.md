...
When you rate an agent's response with **Helpful** (thumbs up) or **Needs work** (thumbs down), DealDesk saves your vote locally alongside your running instance. This guide covers what gets stored, how to access it, and how to export it.
...
```bash
# Installed CLI
dealdesk feedback report

# Point to a different server or company
pnpm dealdesk feedback report --api-base http://127.0.0.1:3000 --company-id <company-id>

# Include raw payload dumps in the report
pnpm dealdesk feedback report --payloads
```
...
feedback-export-20260331T120000Z/
  ...
  traces/
    PAP-123-e5f6g7h8.json      # DealDesk feedback envelope (one per trace)
  full-traces/
    PAP-123-e5f6g7h8/
...
Exports are full by default. `traces/` keeps the DealDesk envelope, while `full-traces/` contains the richer per-trace bundle plus any recoverable adapter-native files.

```bash
# Custom server and output directory
pnpm dealdesk feedback export --api-base http://127.0.0.1:3000 --company-id <company-id> --out ./my-export
```
...
- Default endpoint: when no feedback export backend URL is configured, DealDesk falls back to `https://telemetry.dealdesk.ing`
