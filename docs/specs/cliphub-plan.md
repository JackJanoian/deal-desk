# ClipHub: Marketplace for DealDesk Team Configurations

> Supersession note: this marketplace plan predates the markdown-first company package direction. For the current package-format and import/export rollout plan, see `docs/companies/companies-spec.md`.

> The "app store" for whole-company AI teams — pre-built DealDesk configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for DealDesk-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | DealDesk company (agents, projects, governance) |
...
A complete DealDesk company configuration:
...
Single-agent configurations designed to plug into a DealDesk org:
...
Portable skill files that any DealDesk agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with DealDesk's skill loading system
...
interface Listing {
  ...
  dealdeskVersionMin: string;     // Minimum DealDesk version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target DealDesk company
...
| `POST` | `/api/listings/:id/install` | Install to DealDesk company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target DealDesk company (or create new)
  → ClipHub API calls DealDesk API to:
  ...
  → Redirect to DealDesk dashboard with new team running
```
...
### 5.3 Creator: Export from DealDesk → Publish

```
Running DealDesk company → "Export as Blueprint" (CLI or UI)
  → DealDesk exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (DealDesk brand blue/purple)
...
| Install flow | One-click deploy to DealDesk company |
...
{
  "targetCompanyId": "uuid",         // Existing DealDesk company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `dealdesk-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as DealDesk
- **Backend**: Node.js API (or extend DealDesk server)
- **Database**: Postgres (can share DealDesk's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with DealDesk auth (or OAuth2)

### 9.2 Integration with DealDesk

ClipHub can be:
- **Option A**: A separate app that calls DealDesk's API to install blueprints
- **Option B**: A built-in section of the DealDesk UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing DealDesk UI and API.
...
- [ ] Install flow: blueprint → DealDesk company
...
- [ ] "Export from DealDesk" CLI command
